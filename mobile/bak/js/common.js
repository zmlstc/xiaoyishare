...
function getSearchName() {
	...
	if (keyword == '') {
	    if(getCookie('deft_key_value') == null) {
	        $.getJSON(ApiUrl + '/index.php/index/search_hot_info', function(result) {
...
function getFreeVoucher(tid) {
    ...
    $.ajax({
        type:'post',
        url:ApiUrl+"/index.php/member_voucher/voucher_freeex",
...
function updateCookieCart(key) {
    ...
    if (cartlist) {
        $.ajax({
            type:'post',
            url:ApiUrl+'/index.php/member_cart/cart_batchadd',
...
function getCartCount(key, expireHours) {
    ...
    if (getCookie('key') !== null && getCookie('cart_count') === null) {
        ...
        $.ajax({
            type:'post',
            url:ApiUrl+'/index.php/member_cart/cart_count',
            data:{key:key},
            dataType:'json',
            async:false,
            success:function (result) {
...
function getChatCount() {
    if ($('#header').find('.message').length > 0) {
        var key = getCookie('key');
        if (key !== null) {
            $.getJSON(ApiUrl+'/index.php/member_chat/get_msg_count', {key:key}, function(result){
                if (result.datas > 0) {
     ...
     */
        areaSelected: function(options) {
            ...
            function _getAreaList() {
                $.ajax({//获取区域列表
                    type:'get',
                    url:ApiUrl+'/index.php/area/area_list',
                    data:{area_id:ASID},
                    dataType:'json',
                    async:false,
                    success:function(result){
...
function loadVercode(){
    ...
    $.ajax({
        type:'get',
        url:ApiUrl+"/index.php/vercode/makecodekey",
        async : false,
        dataType: 'json',
        success:function(result){
    ...
    $("#codeimage").attr('src',ApiUrl+'/index.php/vercode?k='+$("#codekey").val()+'&c=' + Math.random());
}
/**
 * 收藏店铺
 */
function favoriteStore(store_id){
    ...
    $.ajax({
        type: 'post',
        url: ApiUrl + '/index.php/member_favorites_store/favorites_add',
...
function dropFavoriteStore(store_id){
    ...
    $.ajax({
        type: 'post',
        url: ApiUrl + '/index.php/member_favorites_store/favorites_del',
        data: {key: key, store_id: store_id},
        dataType: 'json',
        async: false,
        success: function(result) {
...
function favoriteGoods(goods_id){
    ...
    $.ajax({
        type: 'post',
        url: ApiUrl + '/index.php/member_favorites/favorites_add',
...
function dropFavoriteGoods(goods_id){
    ...
    $.ajax({
        type: 'post',
        url: ApiUrl + '/index.php/member_favorites/favorites_del',
...
$(function() {
	...
	if(uawx.indexOf('micromessenger') > -1) {
		if (!wxout){
			$.getJSON(ApiUrl + '/index.php/connect/get_state', function(result){
				var allow_login = 0;
				if (result.datas.connect_wap_wx == '1' && result.datas.connect_wap_wxauto == '1') {
					...
					if (allow_login) {
						...
						if(!key){
							location.href = ApiUrl+'/index.php/connect/index';
						}
					}
				}
			});
