...
$(function(){
	...
	if (article_id=='') {
		...
		$.ajax({
			url:ApiUrl+"/index.php/news_article/news_show",
			type:'get',
...
function loadPLlist() {
  ...
  param.type = 'article';
	...
	$.getJSON(ApiUrl+"/index.php/news_article/comment_list?" + window.location.search.replace("?", "&"), param, function(e) {
	/*	if (!e) {
			e = [];
			e.datas = [];
	...
	var key = getCookie('key');
	 if (key === null) {
		...
		if (!conFlg) {
			conFlg = true;
			 $.ajax({
            url:ApiUrl+"/index.php/news_article/article_up",
            type:"post",
            dataType:"json",
...
function _itemz(id,his){
	...
	if (getCookie(keys) == 1) {
		return false;
					 ...
					 $.ajax({
            url:ApiUrl+"/index.php/news_article/comment_up",
            type:"post",
            dataType:"json",
