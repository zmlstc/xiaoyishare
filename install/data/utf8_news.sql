...
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='NEWS文章表';

CREATE TABLE `#__news_article_attitude` (
...
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='NEWS文章心情表';

CREATE TABLE `#__news_article_class` (
...
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='news文章分类表';
...
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='NEWS评论表';

CREATE TABLE `#__news_comment_up` (
  ...
  PRIMARY KEY (`up_id`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='NEWS评论顶表';

CREATE TABLE `#__news_index_module` (
...
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='NEWS首页模块表';

CREATE TABLE `#__news_module` (
  ...
  PRIMARY KEY (`module_id`)
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COMMENT='NEWS模板模块表';

INSERT INTO `#__news_module` (`module_id`, `module_title`, `module_name`, `module_type`, `module_class`) VALUES
(1, '综合模块', 'index', 'index', 1),
(2, '买什么模块', 'what', 'what', 1),
(3, '文章模块1', 'article1', 'article1', 1),
(4, '文章模块2', 'article2', 'article2', 1),
(5, '通栏广告模块', 'show', 'show', 1);

CREATE TABLE `#__news_module_assembly` (
...
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COMMENT='news模块组件表';

INSERT INTO `#__news_module_assembly` (`assembly_id`, `assembly_title`, `assembly_name`, `assembly_explain`) VALUES
(1, '文章', 'article', '文章组件'),
(2, '图片', 'picture', '图片组件'),
(3, '商品', 'goods', '商品组件'),
(4, '品牌', 'brand', '品牌组件'),
...
CREATE TABLE `#__news_module_frame` (
...
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COMMENT='news模块框架表';

INSERT INTO `#__news_module_frame` (`frame_id`, `frame_title`, `frame_name`, `frame_explain`, `frame_structure`) VALUES
...
CREATE TABLE `#__news_navigation` (
...
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COMMENT='NEWS导航表';

INSERT INTO `#__news_navigation` (`navigation_id`, `navigation_title`, `navigation_link`, `navigation_sort`, `navigation_open_type`) VALUES
(1, '商城', '', 255, 1),
(2, '社区', 'bbs', 255, 1),
(3, '买什么', 'what', 255, 1),
(4, '品牌', 'index.php/brand', 255, 1);

CREATE TABLE `#__news_picture` (
  ...
  `picture_comment_count` int(10) unsigned NULL DEFAULT '0' COMMENT '图刊评论数',
  `picture_title_short` varchar(50) NULL DEFAULT '' COMMENT '图刊短标题',
  `picture_image_count` tinyint(1) unsigned NULL DEFAULT '0'COMMENT '图刊图片总数',
  `picture_share_count` int(10) unsigned NULL DEFAULT '0' COMMENT '图刊分享数',
  `picture_verify_reason` varchar(255) DEFAULT NULL COMMENT '审核失败原因',
  PRIMARY KEY (`picture_id`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='NEWS图刊表';

CREATE TABLE `#__news_picture_class` (
  `class_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '分类编号 ',
  `class_name` varchar(50) NOT NULL COMMENT '分类名称',
  `class_sort` tinyint(1) unsigned NOT NULL DEFAULT '255' COMMENT '排序',
  PRIMARY KEY (`class_id`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='news图刊分类表';

CREATE TABLE `#__news_picture_image` (
...
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='NEWS图刊图片表';

CREATE TABLE `#__news_special` (
...
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='NEWS专题表';

CREATE TABLE `#__news_tag` (
  ...
  PRIMARY KEY (`tag_id`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='NEWS标签表';

CREATE TABLE `#__news_tag_relation` (
...
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='NEWS标签关系表';
