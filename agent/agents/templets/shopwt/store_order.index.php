...
$(function(){
    ...
    $('#wtsubmit').click(function(){
        $("#flexigrid").flexOptions({url: '<?php echo urlAgentAgents('order','get_xml');?>?'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#wtreset').click(function(){
        $("#flexigrid").flexOptions({url: '<?php echo urlAgentAgents('order','get_xml');?>'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
        url: '<?php echo urlAgentAgents('order','get_xml');?>',
        colModel : [
