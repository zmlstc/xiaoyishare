...
$(function(){
    $("#flexigrid").flexigrid({
        url: '<?php echo urlAgentAgents('store','get_xml');?>',
        colModel : [
    ...
    $('#wtsubmit').click(function(){
        $("#flexigrid").flexOptions({url: '<?php echo urlAgentAgents('store','get_xml');?>?'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });

    // 高级搜索重置
    $('#wtreset').click(function(){
        $("#flexigrid").flexOptions({url: '<?php echo urlAgentAgents('store','get_xml');?>'}).flexReload();
        $("#formSearch")[0].reset();
    });
});
...
function fg_operations(name, bDiv) {
    if (name == 'wtshop_add') {
       // window.location.href = 'store/wtshop_add';
    }
}
