...
//布局换色设置
        ...
        $("div.bgSelector").toggle(300, function() {
            if ($(this).html() == '') {
                $(this).sColor({
                    ...
                    curImg: AGENTS_TEMPLATES_URL + '/images/cur.png',  //必填，颜色选择对象图片路径
    ...
    if ($.cookie('wspeParam_agent') == null) {
        // 默认选择第一个菜单
        $('.wt-module-menu').find('li:first > a').click();
    } else {
        openItem($.cookie('wspeParam_agent'));
    }
    ...
    if ($.cookie('commonPendingMatters') == null) {
        $.ajax({
                url : 'index.php/common/ajax_pending_matters',
                async : false,
...
function openItem(param) {
    ...
    $('#workspace').attr('src', AGENTS_SITE_URL + '/' + data_str[0] + '/index.php/' + data_str[1]);
    $.cookie('wspeParam_agent', data_str[0] + '|' + data_str[1], { expires: 1 ,path:"/"});
}