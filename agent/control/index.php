...
class indexControl extends SystemControl{
    ...
    public function logoutWt(){
        setWtCookie('sys_agent_key','',-1,'',null);
    ...
    public function modifypwWt(){
        if (chksubmit()){
            ...
            //查询管理员信息
            $admin_model = Model('agencies');
            $admininfo = $admin_model->getAgenciesInfoByID($admininfo['id']);
            if (!is_array($admininfo) || count($admininfo)<= 0){
                showMessage(Language::get('index_modifypw_admin_error'));
            }
            //旧密码是否正确
            if ($admininfo['agent_passwd'] != md5(trim($_POST['old_pw']))){
                showMessage(Language::get('index_modifypw_oldpw_error'));
            }
            $new_pw = md5(trim($_POST['new_pw']));
            $update = array();
            $update['agent_passwd'] = $new_pw;
            $update['agent_id'] = $admininfo['admin_id'];
            //$result = $admin_model->updateAdmin($update);
            $result = $admin_model->editAgencies(array('agent_id'=>$admininfo['agent_id']),$update);
            if ($result){
                showDialog(Language::get('index_modifypw_success'), AGENTS_SITE_URL .'/index.php/index/logout', 'succ');
            }else{
