...
<div class="tabmenu">
  ...
  <a href="<?php echo urlSeller('order_vr', 'exchange');?>" class="wtbtn wtbtn-bittersweet"><i class="icon-edit"></i>兑换兑换码</a> </div>
<form method="get" action="<?php echo urlSeller('order_vr','index');?>" target="_self">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td class="bdl bdr"><p><?php echo $order['state_desc']; ?> </p>
        ...
        <p><a href="<?php echo urlSeller('order_vr','show_order',array('order_id'=>$order['order_id']));?>" target="_blank"><?php echo $lang['store_order_view_order'];?></a></p></td>
      <td class="bdl bdr"><!-- 取消订单 -->
        
        <?php if($order['if_cancel']) { ?>
        <p><a href="javascript:void(0)" class="wtbtn wtbtn-grapefruit mt5" wt_type="dialog" uri="<?php echo urlSeller('order_vr','change_state',array('state_type'=>'order_cancel','order_id'=>$order['order_id'],'order_sn'=>$order['order_sn']));?>" dialog_title="<?php echo $lang['store_order_cancel_order'];?>" dialog_id="seller_order_cancel_order" dialog_width="400" id="order<?php echo $order['order_id']; ?>_action_cancel" /><i class="icon-remove-bbs"></i><?php echo $lang['store_order_cancel_order'];?></a></p>
        <?php } ?>
    </tr>
    <?php } } else { ?>
    <tr>
