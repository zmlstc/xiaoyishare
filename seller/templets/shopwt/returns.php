...
<form method="get" action="<?php echo urlSeller('returns','index');?>">
...
<table class="wtsc-default-table">
  ...
  <tbody>
    ...
    <tr class="bd-line" >
        ...
        <dd><?php echo $lang['refund_order_ordersn'].$lang['wt_colon'];?><a href="<?php echo urlSeller('order','show_order',array('order_id'=>$val['order_id']));?>" target="_blank"><?php echo $val['order_sn'];?></a></dd>
      ...
      <td class="nscs-table-handle">
        ...
        <span><a href="<?php echo urlSeller('returns','edit',array('return_id'=>$val['refund_id']));?>" class="btn-bluejeans"><i class="icon-edit"></i><p>处理</p></a></span>
        <?php } else { ?>
       <span> <a href="<?php echo urlSeller('returns','view',array('return_id'=>$val['refund_id']));?>" class="btn-bittersweet"><i class="icon-eye-open"></i><p><?php echo $lang['wt_view'];?></p></a></span>
        <?php } ?>
        <?php if ($val['seller_state'] == 2 && $val['return_type'] == 2 && $val['goods_state'] == 2) { ?>
        <span><a href="javascript:void(0)" class="btn-mint" wt_type="dialog" dialog_title="<?php echo $lang['wt_edit'];?>" dialog_id="return_order" dialog_width="480" uri="<?php echo urlSeller('returns','receive',array('return_id'=>$val['refund_id']));?>"><i class="icon-check-sign"></i><p><?php echo '收货';?></p></a></span>
        <?php } ?>
        </td>
    </tr>
    <?php } ?>
