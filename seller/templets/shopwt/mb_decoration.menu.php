...
<div class="wtsc-form-default">
  <form method="post" action="<?php echo urlSeller('mb_decoration','decoration_menu');?>" enctype="multipart/form-data" id="my_store_form">
...
$(function() {
    ...
    $('input.input-file').change(function() {
        ...
        $.ajaxFileUpload({
            url: '<?php echo urlSeller('setting','store_mb_sliders');?>',
            secureuri: false,
            fileElementId: id,
            dataType: 'json',
            data: {id: id},
    ...
    $("a[data-slider-drop]").click(function() {
        ...
        $.getJSON('<?php echo urlSeller('setting','store_mb_sliders_drop');?>', {id: id}, function(d) {
            if (!d.success) {
                alert(d.error);
                return;
            }
