...
<form method="get" action="<?php echo urlSeller('refund','index');?>">
  ...
  <tbody>
    ...
    <tr class="bd-line" >
      ...
      <td class="nscs-table-handle"><?php if ($val['seller_state'] == 1) { ?>
    	<span><a href="<?php echo urlSeller('refund','edit',array('refund_id'=>$val['refund_id']));?>" class="btn-bluejeans"><i class="icon-edit"></i><p>处理</p></a></span>
    	<?php } else { ?>
    	<span><a href="<?php echo urlSeller('refund','view',array('refund_id'=>$val['refund_id']));?>" class="btn-bittersweet"><i class="icon-eye-open"></i><p><?php echo $lang['wt_view'];?></p></a></span><?php } ?>
       </td>
    </tr>
    <?php } ?>
    <?php } else { ?>
