...
<form method="get" action="<?php echo urlSeller('order','index');?>" target="_self">
  ...
  <tbody>
    ...
    <tr>
      <th colspan="20"><span class="ml10"><?php echo $lang['store_order_order_sn'].$lang['wt_colon'];?><em><?php echo $order['order_sn']; ?></em>
        ...
        <span>取货方式：门店自提</span>
        <?php } ?>
		<span class="fr mr5"> <a href="<?php echo urlSeller('order','order_print_small',array('order_id'=>$order_id));?>" class="wtbtn-mini" target="_blank" title="打印发货单"/><i class="icon-print"></i>小票打印</a></span>
        <span class="fr mr5"> <a href="<?php echo urlSeller('order','order_print',array('order_id'=>$order_id));?>" class="wtbtn-mini" target="_blank" title="打印发货单"/><i class="icon-print"></i><?php echo $lang['store_show_order_printorder'];?></a></span>
	</th>
    </tr>
    <?php $i = 0;?>
    <?php if(!empty($order['goods_list'])&& is_array($order['goods_list'])){ foreach($order['goods_list'] as $k => $goods) { ?>
    <?php $i++;?>
    <tr>
      <td class="bdl"></td>
      ...
      <td class="tl"><dl class="goods-name">
          <dt><a target="_blank" href="<?php echo $goods['goods_url'];?>"><?php echo $goods['goods_name']; ?></a><a target="_blank" class="blue ml5" href="<?php echo urlSeller('snapshot', 'index', array('rec_id' => $goods['rec_id']));?>">[交易快照]</a></dt>
