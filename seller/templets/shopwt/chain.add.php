...
<div class="wtsc-form-default">
  <form method="post" action="<?php echo empty($output['chain_info']) ? urlSeller('chain', 'chain_add') : urlSeller('chain', 'chain_edit', array('chain_id' => $output['chain_info']['chain_id']));?>" id="chain_form" enctype="multipart/form-data">
    ...
    $('#chain_form').validate({
        ...
        rules : {
            chain_user : {
                required : true,
                minlength: 3,
                remote   : '<?php echo urlSeller('chain','check_user',array('no_id'=>$output['chain_info']['chain_id']));?>'
            },
            chain_pwd : {
                <?php if (empty($output['chain_info'])) {?>
                required : true,
