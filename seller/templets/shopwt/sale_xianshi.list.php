...
<div class="tabmenu">
  ...
  <a class="wtbtn wtbtn-mint" href="<?php echo urlSeller('sale_xianshi', 'xianshi_add');?>"><i class="icon-plus-sign"></i><?php echo $lang['xianshi_add'];?></a>
  <?php } else { ?>
  <?php if(!empty($output['current_xianshi_quota'])) { ?>
  <a class="wtbtn wtbtn-mint" style="right:100px" href="<?php echo urlSeller('sale_xianshi', 'xianshi_add');?>"><i class="icon-plus-sign"></i><?php echo $lang['xianshi_add'];?></a> <a class="wtbtn wtbtn-aqua" href="<?php echo urlSeller('sale_xianshi', 'xianshi_quota_add');?>" title=""><i class="icon-money"></i>套餐续费</a>
  <?php } else { ?>
  <a class="wtbtn wtbtn-aqua" href="<?php echo urlSeller('sale_xianshi', 'xianshi_quota_add');?>" title=""><i class="icon-money"></i><?php echo $lang['xianshi_quota_add'];?></a>
  <?php } ?>
  <?php } ?>
</div>
<?php if ($output['isOwnShop']) { ?>
...
<?php } ?>
<form method="get" action="<?php echo urlSeller('sale_xianshi','xianshi_list');?>">
  ...
  <tbody id="xianshi_list">
    <tr class="bd-line">
      ...
      <td class="nscs-table-handle tr"><?php if($val['editable']) { ?>
        <span> <a href="<?php echo urlSeller('sale_xianshi','xianshi_edit',array('xianshi_id'=>$val['xianshi_id']));?>" class="btn-bluejeans"> <i class="icon-edit"></i>
        <p><?php echo $lang['wt_edit'];?></p>
        </a> </span>
        <?php } ?>
        <span> <a href="<?php echo urlSeller('sale_xianshi','xianshi_manage',array('xianshi_id'=>$val['xianshi_id']));?>" class="btn-mint"> <i class="icon-cog"></i>
    ...
    $(document).ready(function(){
        $('[wttype="btn_del_xianshi"]').on('click', function() {
            if(confirm('<?php echo $lang['wt_ensure_del'];?>')) {
                var action = "<?php echo urlSeller('sale_xianshi', 'xianshi_del');?>";
