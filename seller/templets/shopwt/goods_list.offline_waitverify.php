  ...
  <thead>
    ...
    <tr>
      ...
      <td colspan="10"><label for="all"><?php echo $lang['wt_select_all'];?></label>
        <a href="javascript:void(0);" class="wtbtn-mini" wt_type="batchbutton" uri="<?php echo urlSeller('goods_online', 'drop_goods');?>" name="commonid" confirm="<?php echo $lang['wt_ensure_del'];?>"><i class="icon-trash"></i><?php echo $lang['wt_del'];?></a> 
    </tr>
    <?php } ?>
  </thead>
  <tbody>
    ...
    <tr>
      ...
      <td class="nscs-table-handle"><?php if ($val['goods_lock'] == 0) {?>
        ...
        <?php } else {?>
        <span class="tip" title="该商品参加抢购活动期间不能进行编辑及删除等操作,可以编辑赠品和推荐组合"><a href="<?php if ($val['is_virtual'] ==1 ) {echo 'javascript:void(0);';} else {echo urlSeller('goods_online', 'add_gift', array('commonid' => $val['goods_commonid']));}?>" class="btn-darkgray"><i class="icon-lock"></i>
        <p>锁定</p>
        </a></span>
        <?php }?></td>
    </tr>
