...
<?php if ($output['isOwnShop']) { ?>
    ...
    <a class="wtbtn wtbtn-mint" href="<?php echo urlSeller('voucher', 'templateadd');?>"><i class="icon-plus-sign"></i><?php echo $lang['voucher_templateadd']; ?></a>
<?php } else { ?>
    ...
    <a class="wtbtn wtbtn-mint" style="right:100px" href="<?php echo urlSeller('voucher', 'templateadd');?>"><i class="icon-plus-sign"></i><?php echo $lang['voucher_templateadd']; ?></a>
	<a class="wtbtn wtbtn-aqua" href="<?php echo urlSeller('voucher', 'quotaadd');?>" title=""><i class="icon-money"></i>套餐续费</a>
    <?php } else { ?>
    <a class="wtbtn wtbtn-aqua" href="<?php echo urlSeller('voucher', 'quotaadd');?>" title=""><i class="icon-money"></i>购买套餐</a>
    <?php } ?>
...
<form method="get" action="<?php echo urlSeller('voucher','templatelist');?>">
      ...
      <tr class="bd-line">
        	...
        	<?php if($val['voucher_t_state']==$output['templatestate_arr']['usable'][0] && (!$val['voucher_t_giveout']) && (!$val['voucher_t_isbuild'])) {//代金券模板有效并且没有领取时可以编辑?>
        		<span> <a class="btn-bluejeans"
					href="<?php echo urlSeller('voucher','templateedit',array('tid'=>$val['voucher_t_id']));?>">
						<i class="icon-edit"></i>
						<p>编辑</p>
				</a>
			</span>
        	...
        	<!-- 代金券详细 -->
        	<span>
        	   <a class="btn-bluejeans" href="<?php echo urlSeller('voucher','templateinfo',array('tid'=>$val['voucher_t_id']));?>"><i class="icon-th-list"></i><p>详细</p></a>
			</span>
        	<?php if ((!$val['voucher_t_giveout']) && (!$val['voucher_t_isbuild'])){//该模板没有发放过代金券时可以删除?>
        	<span> <a class="btn-grapefruit" href="javascript:void(0)" onclick="ajax_get_confirm('<?php echo $lang['wt_ensure_del'];?>','<?php echo urlSeller('voucher','templatedel',array('tid'=>$val['voucher_t_id']));?>');"><i class="icon-trash"></i><p>删除</p></a></span>
        	<?php }?>
        </td>
		</tr>
      <?php }?>
