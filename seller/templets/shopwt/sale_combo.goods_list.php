...
<?php } else { ?>
  ...
  <a class="wtbtn wtbtn-aqua" href="<?php echo urlSeller('sale_combo', 'combo_quota_add');?>" title="购买套餐"><i class="icon-money"></i>购买套餐</a>
  <?php } else { ?>
  <a class="wtbtn wtbtn-mint" href="javascript:void(0);" wttype="select_goods" style="right:100px"><i class="icon-plus-sign"></i>添加商品</a>
  <a class="wtbtn wtbtn wtbtn-aqua" href="<?php echo urlSeller('sale_combo', 'combo_renew');?>"><i class="icon-money"></i>套餐续费</a>
  <?php } ?>
<?php } ?>
</div>
...
<table class="wtsc-default-table">
  ...
  <tbody wttype="choose_goods_list">
	...
	<td class="nscs-table-handle">
        <span>
          <a class="btn-bluejeans" href="<?php echo urlSeller('sale_combo', 'choosed_goods', array('gid' => $val['goods_id']));?>">
            <i class="icon-edit"></i>
            <p>编辑</p>
          </a>
        </span>
...
$(function(){
    ...
    $('input[wttype="btn_search_goods"]').click(function(){
        _url = '<?php echo urlSeller('sale_combo', 'combo_select_goods');?>';
    ...
    $('#mainContent').on('click', 'a[wttype="a_choose_goods"]', function(){
        ...
        CUR_DIALOG =ajax_form('choose_goods', '设置推荐组合商品', '<?php echo urlSeller('sale_combo', 'choosed_goods');?>&gid='+_gid, 1000);
    });

    // 删除商品
    $('a[wttype="del_choosed"]').click(function(){
        $this = $(this);
        _url = '<?php echo urlSeller('sale_combo', 'del_choosed_goods');?>';
        _gid = $this.attr('data-gid');
