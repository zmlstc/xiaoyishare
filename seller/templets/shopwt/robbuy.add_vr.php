...
<div class="wtsc-form-default">
  <form id="add_form" action="<?php echo urlSeller('robbuy','robbuy_save',array('vr'=>'1'));?>" method="post" enctype="multipart/form-data">
    <dl>
...
<?php } ?>
    ...
    <dl>
      ...
      <dd>
        ...
        <p class="hr8"><a class="des_demo wtbtn" href="<?php echo urlSeller('album','pic_list',array('item'=>'robbuy'));?>"><i class="icon-picture"></i><?php echo $lang['store_goods_album_insert_users_photo'];?></a></p>
...
$("select[name=class]").change(function(){
    ...
    $.ajax({
        type:'GET',
        url:'<?php echo urlSeller('robbuy','ajax_vr_class');?>?class_id='+class_id,
...
$("select[name=city]").change(function(){
    ...
    $.ajax({
        type:'GET',
        url:'<?php echo urlSeller('robbuy','ajax_vr_area');?>?area_id='+area_id,
        success:function(json){
            var html = '<option value="">'+'请选择...'+'</option>';
...
$("select[name=area]").change(function(){
    ...
    $('#btn_search_goods').on('click', function() {
        var url = "<?php echo urlSeller('robbuy', 'search_vr_goods'); ?>";
    ...
    $('#div_goods_search_result').on('click', '[wttype="btn_add_robbuy_goods"]', function() {
        ...
        $.get('<?php echo urlSeller('robbuy', 'robbuy_goods_content'); ?>', {goods_commonid: goods_commonid}, function(data) {
            if(data.result) {
    ...
    $('[wttype="btn_upload_image"]').fileupload({
        dataType: 'json',
            url: "<?php echo urlSeller('robbuy', 'image_upload');?>",
            add: function(e, data) {
                ...
                $img.attr('src', "<?php echo SELLER_TEMPLATES_URL.'/images/loading.gif';?>");
                data.submit();
            },
    ...
    jQuery.validator.methods.checkRobbuyGoods = function(value, element) {
        ...
        $.ajax({
            type:"GET",
            url:'<?php echo urlSeller('robbuy', 'check_robbuy_goods');?>',
            async:false,
...
<?php } ?>
    ...
    $('.des_demo').ajaxContent({
        event:'click', //mouseover
            loaderType:"img",
            loadingMsg:"<?php echo SELLER_TEMPLATES_URL;?>/images/loading.gif",
            target:'#des_demo'
    });
});
