  ...
  <div class="wtsc-order-info">
    ...
    <div class="wtsc-order-condition">
      ...
      <ul>
        ...
        <li>2. 可以在<a href="<?php echo urlSeller('evaluate','list');?>" class="wtbtn-mini">评价管理</a>查看详细内容。</li>
      </ul>
    </div>
    <?php } else { ?>
    <div class="wtsc-order-condition">
      ...
      <tbody>
        ...
        <tr class="bd-line">
          ...
          <td class="tl"><dl class="goods-name">
              <dt><a target="_blank" href="<?php echo $goods['goods_url']; ?>"><?php echo $goods['goods_name']; ?></a><a target="_blank" href="<?php echo urlSeller('snapshot', 'index', array('rec_id' => $goods['rec_id']));?>"  class="blue ml5">[交易快照]</a></dt>
              ...
              <dd>
                ...
                退款单号：<a target="_blank" href="<?php echo urlSeller('refund','view',array('refund_id'=>$output['refund_all']['refund_id']));?>"><?php echo $output['refund_all']['refund_sn'];?></a>
                <?php }else if($goods['extend_refund']['refund_type'] == 1) {?>
                退款单号：<a target="_blank" href="<?php echo urlSeller('refund','view',array('refund_id'=>$goods['extend_refund']['refund_id']));?>"><?php echo $goods['extend_refund']['refund_sn'];?></a></dd>
              <?php }else if($goods['extend_refund']['refund_type'] == 2) {?>
              退货单号：<a target="_blank" href="<?php echo urlSeller('returns','view',array('return_id'=>$goods['extend_refund']['refund_id']));?>"><?php echo $goods['extend_refund']['refund_sn'];?></a>
              </dd>
              <?php } ?>
            </dl></td>
          ...
          <td class="bdl bdr" rowspan="<?php echo $output['order_info']['goods_count'];?>"><?php echo $output['order_info']['state_desc']; ?>
            ...
            <p><a href="javascript:void(0)" class="wtbtn" uri="<?php echo urlSeller('order','change_state',array('state_type'=>'modify_price','order_sn'=>$output['order_info']['order_sn'],'order_id'=>$output['order_info']['order_id']));?>" dialog_width="480" dialog_title="<?php echo $lang['store_order_modify_price'];?>" wt_type="dialog"  dialog_id="seller_order_adjust_fee" id="order<?php echo $output['order_info']['order_id']; ?>_action_adjust_fee" />修改运费</a></p>
            <?php }?>
            
            <!-- 取消订单 -->
            
            <?php if ($output['order_info']['if_cancel']) { ?>
            <p><a href="javascript:void(0)" style="color:#F30; text-decoration:underline;" wt_type="dialog" uri="<?php echo urlSeller('order','change_state',array('state_type'=>'order_cancel','order_sn'=>$output['order_info']['order_sn'],'order_id'=>$output['order_info']['order_id']));?>" dialog_title="<?php echo $lang['store_order_cancel_order'];?>" dialog_id="seller_order_cancel_order" dialog_width="400" id="order<?php echo $output['order_info']['order_id']; ?>_action_cancel" /><?php echo $lang['store_order_cancel_order'];?></a></p>
            <?php } ?>
            
            <!-- 发货 -->
            
            <?php if ($output['order_info']['if_store_send']) { ?>
            <p><a class="wtbtn" href="<?php echo urlSeller('deliver','send',array('order_id'=>$output['order_info']['order_id']));?>"/><i class="icon-truck"></i><?php echo $lang['store_order_send'];?></a></p>
            <?php } ?></td>
          <?php } ?>
          <!-- E 合并TD --> 
        </tr>
...
$(function(){
    $('#show_shipping').on('hover',function(){
    	...
    	$.getJSON('<?php echo urlSeller('index','get_express',array('e_code'=>$output['order_info']['express_info']['e_code'],'shipping_code'=>$output['order_info']['shipping_code'],'c'=>random(7)));?>',function(data){
    		if(data){
    			data = var_send+data;
