...
<table class="wtsc-default-table">
  ...
  <tbody>
    ...
    <tr class="bd-line">
      ...
      <span><a href="javascript:void(0)" class="btn-grapefruit" onclick="ajax_get_confirm('<?php echo $lang['wt_ensure_del'];?>', '<?php echo urlSeller('info','reopen_del',array('re_id'=>$val['re_id']));?>');"><i class="icon-trash"></i><p><?php echo $lang['wt_del'];?></p></a></span>
     <?php } ?>
      </td>
    </tr>
    <?php } ?>
...
<?php if ($output['upload_cert']) {?>
  <form method="post" action="<?php echo urlSeller('info','reopen_upload');?>" name="upload_form" id="upload_form" enctype="multipart/form-data">
...
<?php if ($output['applay_reopen']) {?>
  <form method="post" action="<?php echo urlSeller('info','reopen_add');?>" name="add_form" id="add_form">
    <input type="hidden" name="form_submit" value="ok" />
    <dl>
