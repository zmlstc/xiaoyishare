...
<form method="get" action="<?php echo urlSeller('deliver','index');?>" target="_self">
  ...
  <tbody>
    ...
    <tr>
      <th colspan="21"><span class="ml5"><?php echo $lang['store_order_order_sn'].$lang['wt_colon'];?><strong><?php echo $order['order_sn']; ?></strong></span><span><?php echo $lang['store_order_add_time'].$lang['wt_colon'];?><em class="goods-time"><?php echo date("Y-m-d H:i:s",$order['add_time']); ?></em></span>
        ...
        <a href="<?php echo urlSeller('deliver','search_deliver',array('order_sn'=>$order['order_sn']));?>" class="wtbtn-mini"><i class="icon-compass"></i><?php echo $lang['store_order_show_deliver'];?></a>
        <?php }?>
        <a href="<?php echo urlSeller('order','order_print',array('order_id'=>$order['order_id']));?>" target="_blank"  class="wtbtn-mini" title="<?php echo $lang['store_show_order_printorder'];?>"/><i class="icon-print"></i><?php echo $lang['store_show_order_printorder'];?></a>
	<a href="<?php echo urlSeller('order','order_print_small',array('order_id'=>$order['order_id']));?>" target="_blank"  class="wtbtn-mini" title="<?php echo $lang['store_show_order_printorder'];?>"/><i class="icon-print"></i>小票打印</a>
	</span></th>
    </tr>
    <?php $i = 0; ?>
      ...
      <td class="bdl bdr order-info w500" rowspan="<?php echo $order['goods_count'];?>"><dl>
        ...
        <dl>
          ...
          <dd>
            ...
            <span><a href="<?php echo urlSeller('deliver','send',array('order_id'=>$order['order_id']));?>" class="wtbtn-mini wtbtn-mint fr"><i class="icon-truck"></i><?php echo $lang['store_order_send'];?></a></span>
            <?php } elseif ($order['order_state'] == ORDER_STATE_SEND){?>
            <span>
            <a href="javascript:void(0)" class="wtbtn-mini wtbtn-bittersweet ml5 fr" uri="<?php echo urlSeller('deliver','delay_receive',array('order_id'=>$order['order_id']));?>" dialog_width="480" dialog_title="延迟收货" wt_type="dialog" dialog_id="seller_order_delay_receive" id="order<?php echo $order['order_id']; ?>_action_delay_receive" /><i class="icon-time"></i></i>延迟收货</a>
            <a href="<?php echo urlSeller('deliver','send',array('order_id'=>$order['order_id']));?>" class="wtbtn-mini wtbtn-aqua fr"><i class="icon-edit"></i><?php echo $lang['store_deliver_modify_info'];?></a>
            </span>
            <?php }?>
            <?php }?>
          </dd>
