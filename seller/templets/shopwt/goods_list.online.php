...
<div class="tabmenu">
  <?php include template('layout/submenu');?>
    <a title="批量生成商品二维码" class="wtbtn wtbtn-aqua" href="<?php echo urlSeller('goods_online','maker_qrcode');?>" style="right:100px" onclick="return confirm('全部生成二维码可能超时失败，确定要生成吗？');">生成所有商品二维码</a>
  <a href="<?php echo urlSeller('goods_add');?>" class="wtbtn wtbtn-mint" title="<?php echo $lang['store_goods_index_add_goods'];?>"> <?php echo $lang['store_goods_index_add_goods'];?></a> </div>
<form method="get" action="<?php echo urlSeller('goods_online','index');?>">
  <table class="search-form">
    <tr>
      ...
      <td class="w160"><select name="stc_id" class="w150">
          <option value="0"><?php echo $lang['wt_please_choose'];?></option>
          <?php if(is_array($output['goods_class']) && !empty($output['goods_class'])){?>
  ...
  <thead>
    ...
    <tr>
      ...
      <td colspan="20"><label for="all" ><?php echo $lang['wt_select_all'];?></label>
        <a href="javascript:void(0);" class="wtbtn-mini" wt_type="batchbutton" uri="<?php echo urlSeller('goods_online', 'drop_goods');?>" name="commonid" confirm="<?php echo $lang['wt_ensure_del'];?>"><i class="icon-trash"></i><?php echo $lang['wt_del'];?></a> <a href="javascript:void(0);" class="wtbtn-mini" wt_type="batchbutton" uri="<?php echo urlSeller('goods_online', 'goods_unshow');?>" name="commonid"><i class="icon-level-down"></i><?php echo $lang['store_goods_index_unshow'];?></a> <a href="javascript:void(0);" class="wtbtn-mini" wttype="batch" data-param="{url:'<?php echo urlSeller('goods_online', 'edit_jingle');?>', sign:'jingle'}"><i></i>设置广告词</a> <a href="javascript:void(0);" class="wtbtn-mini" wttype="batch" data-param="{url:'<?php echo urlSeller('goods_online', 'edit_plate');?>', sign:'plate'}"><i></i>设置关联版式</a> <a href="javascript:void(0);" class="wtbtn-mini" wt_type="batchbutton" uri="<?php echo urlSeller('goods_online', 'edit_qrcode');?>" name="commonid" confirm="你确定要更新嘛？">更新二维码</a></td>
    </tr>
    <?php } ?>
  </thead>
  <tbody>
    ...
    <tr>
      ...
      <td class="tl"><dl class="goods-name">
          ...
          <dd class="serve"> <span class="<?php if ($val['goods_commend'] == 1) { echo 'open';}?>" title="店铺推荐商品"><i class="commend">荐</i></span> <span class="<?php if ($val['mobile_body'] != '') { echo 'open';}?>" title="手机端商品详情"><i class="icon-tablet"></i></span> <span class="" title="商品页面二维码"><i class="icon-qrcode"></i>
            ...
            <?php if ($val['is_fcode'] ==1) {?>
            <span><a class="wtbtn-mini wtbtn-grapefruit" href="<?php echo urlSeller('goods_online', 'download_f_code_excel', array('commonid' => $val['goods_commonid']));?>">下载F码</a></span>
            <?php }?>
          </dd>
        </dl></td>
      ...
      <td class="nscs-table-handle"><?php if ($val['goods_lock'] == 0) {?>
        <span><a href="<?php echo urlSeller('goods_online', 'edit_goods', array('commonid' => $val['goods_commonid']));?>" class="btn-bluejeans"><i class="icon-edit"></i>
        <p><?php echo $lang['wt_edit'];?></p>
        </a></span> <span><a href="javascript:void(0);" onclick="ajax_get_confirm('<?php echo $lang['wt_ensure_del'];?>', '<?php echo urlSeller('goods_online', 'drop_goods', array('commonid' => $val['goods_commonid']));?>');" class="btn-grapefruit"><i class="icon-trash"></i>
        <p><?php echo $lang['wt_del'];?></p>
        </a></span>
        <?php } else {?>
        <span class="tip" title="该商品参加抢购活动期间不能进行编辑及删除等操作,但可以编辑赠品"><a href="<?php if ($val['is_virtual'] ==1 ) {echo 'javascript:void(0);';} else {echo urlSeller('goods_online', 'add_gift', array('commonid' => $val['goods_commonid']));}?>" class="btn-darkgray"><i class="icon-lock"></i>
        <p>锁定</p>
        </a></span>
        <?php }?></td>
    </tr>
  ...
  <tfoot>
    ...
    <tr>
      ...
      <th colspan="10"><label for="all2"><?php echo $lang['wt_select_all'];?></label>
        <a href="javascript:void(0);" wt_type="batchbutton" uri="<?php echo urlSeller('goods_online', 'drop_goods');?>" name="commonid" confirm="<?php echo $lang['wt_ensure_del'];?>" class="wtbtn-mini"><i class="icon-trash"></i><?php echo $lang['wt_del'];?></a> <a href="javascript:void(0);" wt_type="batchbutton" uri="<?php echo urlSeller('goods_online', 'goods_unshow');?>" name="commonid" class="wtbtn-mini"><i class="icon-level-down"></i><?php echo $lang['store_goods_index_unshow'];?></a> <a href="javascript:void(0);" class="wtbtn-mini" wttype="batch" data-param="{url:'<?php echo urlSeller('goods_online', 'edit_jingle');?>', sign:'jingle'}"><i></i>设置广告词</a> <a href="javascript:void(0);" class="wtbtn-mini" wttype="batch" data-param="{url:'<?php echo urlSeller('goods_online', 'edit_plate');?>', sign:'plate'}"><i></i>设置关联版式</a> </th>
    </tr>
    <tr>
