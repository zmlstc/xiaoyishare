...
<?php } else { ?>
  ...
  <a class="wtbtn wtbtn-aqua" href="<?php echo urlSeller('sale_book', 'book_quota_add');?>" title="购买套餐"><i class="icon-money"></i>购买套餐</a>
  <?php } else { ?>
  <a class="wtbtn wtbtn-mint" href="javascript:void(0);" wttype="select_goods" style="right:100px"><i class="icon-plus-sign"></i>添加商品</a>
  <a class="wtbtn wtbtn wtbtn-aqua" href="<?php echo urlSeller('sale_book', 'book_renew');?>"><i class="icon-money"></i>套餐续费</a>
  <?php } ?>
<?php } ?>
</div>
...
$(function(){
    ...
    $('input[wttype="btn_search_goods"]').click(function(){
        _url = '<?php echo urlSeller('sale_book', 'book_select_goods');?>';
    ...
    $('div[wttype="div_goods_select"]').on('click', 'a[wttype="a_choose_goods"]', function(){
        ...
        CUR_DIALOG = ajax_form('choose_goods', '预定商品规则设定', '<?php echo urlSeller('sale_book', 'choosed_goods');?>&gid='+_gid+'&type='+_type, 640);
    });

    // 删除商品
    $('tbody[wttype="choose_goods_list"]').on('click','a[wttype="del_choosed"]', function(){
        $this = $(this);
        _url = '<?php echo urlSeller('sale_book', 'del_choosed_goods');?>';
