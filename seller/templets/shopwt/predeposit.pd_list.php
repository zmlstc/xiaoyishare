...
<div class="wrap">
  <div class="tabmenu">
    ...
    <!--<a class="wtbtn wtbtn-bittersweet" title="在线充值" href="<?php echo urlShop('predeposit','recharge_add');?>"> <i class="icon-shield"></i> 在线充值 </a> </div>-->
  ...
  <form method="get" action="<?php echo urlShop('predeposit','index');?>">
    ...
    <tbody>
      ...
      <tr class="bd-line">
        ...
        <td class="wtm-table-handle"><?php if (!intval($val['pdr_payment_state'])){?>
          ...
          <p>支付</p>
          </a></span> <span><a class="btn-grapefruit" href="javascript:void(0)" onclick="ajax_get_confirm('<?php echo $lang['wt_ensure_del'];?>', '<?php echo urlShop('predeposit','recharge_del',array('id'=>$val['pdr_id']));?>');"><i class="icon-trash"></i>
          <p><?php echo $lang['wt_del'];?></p>
          </a></span>
          <?php }else{?>
          <span><a href="<?php echo urlSeller('predeposit_tx','recharge_show',array('id'=>$val['pdr_id']));?>" class="btn-bluejeans"><i class="icon-eye-open"></i>
