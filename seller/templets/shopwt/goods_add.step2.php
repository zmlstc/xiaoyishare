...
<div class="item-publish">
  <form method="post" enctype="multipart/form-data" id="goods_form" action="<?php if ($output['edit_goods_sign']) { echo urlSeller('goods_online', 'edit_save_goods');} else { echo urlSeller('goods_add', 'save_goods');}?>">
    ...
    <div class="wtsc-form-goods">
      ...
      <dl>
        ...
        <dd id="gcategory"> <?php echo $output['goods_class']['gc_tag_name'];?> <a class="wtbtn" href="<?php if ($output['edit_goods_sign']) { echo urlSeller('goods_online', 'edit_class', array('commonid' => $output['goods']['goods_commonid'])).'?ref_url='.getReferer(); } else { echo urlSeller('goods_add', 'add_step_one'); }?>"><?php echo $lang['wt_edit'];?></a>
      ...
      <dl wt_type="spec_group_dl_<?php echo $i;?>" wttype="spec_group_dl" <?php if ($output['goods']['is_bat'] == 1) {?>style="display:none;"<?php }?> class="spec-bg" <?php if($k == '1'){?>spec_img="t"<?php }?>>
        ...
        <dd <?php if($k == '1'){?>wttype="sp_group_val"<?php }?>>
          <ul class="spec">
            ...
            <?php }?>
            <?php }?>
            <li data-param="{gc_id:<?php echo $output['goods_class']['gc_id'];?>,sp_id:<?php echo $k;?>,url:'<?php echo urlSeller('goods_add', 'ajax_add_spec');?>'}">
          ...
          <div class="wtsc-goods-default-pic">
            <div class="goodspic-uplaod">
              ...
              <div class="handle">
                ...
                <a class="wtbtn mt5" wttype="show_image" href="<?php echo urlSeller('album', 'pic_list', array('item'=>'goods'));?>"><i class="icon-picture"></i>从图片空间选择</a> <a href="javascript:void(0);" wttype="del_goods_demo" class="wtbtn mt5" style="display: none;"><i class="icon-bbs-arrow-up"></i>关闭相册</a></div>
            </div>
          </div>
          <div id="demo"></div>
        </dd>
        ...
        <dd>
          <div class="wtsc-brand-select">
            ...
            <div class="wtsc-brand-select-container">
              <div class="brand-index" data-tid="<?php echo $output['goods_class']['type_id'];?>" data-url="<?php echo urlSeller('goods_add', 'ajax_get_brand');?>">
          ...
          <div class="tabs">
            ...
            <div id="panel-1" class="ui-tabs-panel">
              ...
              <div class="hr8">
                ...
                <a class="wtbtn mt5" wttype="show_desc" href="<?php echo urlSeller('album','pic_list',array('item'=>'des'));?>"><i class="icon-picture"></i><?php echo $lang['store_goods_album_insert_users_photo'];?></a> <a href="javascript:void(0);" wttype="del_desc" class="wtbtn mt5" style="display: none;"><i class=" icon-bbs-arrow-up"></i>关闭相册</a> </div>
              <p id="des_demo"></p>
            </div>
          ...
          <ul class="wtsc-form-radio-list">
            ...
            <li>
              ...
              <div wttype="div_freight" <?php if (intval($output['goods']['transport_id']) == 0) {?>style="display: none;"<?php }?>>
                ...
                <a href="JavaScript:void(0);" onclick="window.open('<?php echo urlSeller('transport','',array('type'=>'select'));?>')" class="wtbtn" id="postageButton"><i class="icon-truck"></i><?php echo $lang['store_goods_index_select_tpl'];?></a> </div>
            </li>
          </ul>
			 ...
			 .find('i[id="newbatprice"]').click(function(){
                ...
                $(this).next().show();				
					...
					$.ajax({   
						type: "POST",  
						dataType: "text",  
						url: '<?php echo urlSeller('goods_spec_step','TempGetSpecStepPrice');?>',    
		...
		if(parseInt($("#g_step_num_"+sprc_s+"_"+aaN).val())<parseInt(_s_add_step_num)||parseInt($("#g_step_price_"+sprc_s+"_"+aaN).val())>parseInt(_s_add_step_price))
               ...
               for(var ssp_=1;ssp_<=<?=$output['bat_max_num']?>;ssp_++)
			   {
				...
				$.ajax({   
					type: "POST",  
					dataType: "text",  
					url: '<?php echo urlSeller('goods_spec_step','TempSaveSpecStepPrice');?>',      //提交到一般处理程序请求数据   
					data: urldata,                          
					success: function(msg) {
						if(msg<1)
						{
