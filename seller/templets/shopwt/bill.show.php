...
.bill_alert {
  ...
  <div class="bill_alert bill-alert-block mt10">
    ...
    <a class="wtbtn wtbtn-grapefruit mt5" onclick="ajax_get_confirm('一旦确认将无法恢复，系统将自动进入结算环节,<BR/>确认系统出账单计算无误吗?', '<?php echo urlSeller('bill','confirm_bill',array('ob_id'=>$_GET['ob_id']));?>');" href="javascript:void(0)">本期结算无误，我要确认</a>
    <?php } elseif ($output['bill_info']['ob_state'] == BILL_STATE_SUCCESS) {?>
    <a class="wtbtn mt5" target="_blank" href="<?php echo urlSeller('bill','bill_print',array('ob_id'=>$_GET['ob_id']));?>">打印结算单</a>
    <?php } ?>
    </div>
