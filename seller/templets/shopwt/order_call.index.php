...
<form method="get" action="<?php echo urlSeller('order_call','index');?>" target="_self">
...
<span class="fr mr5"> <a href="<?php echo urlSeller('order_print','',array('order_id'=>$order_id));?>" class="wtsc-btn-mini" target="_blank" title="打印发货单"/><i class="icon-print"></i>A5打印</a></span>
<span class="fr mr5"> <a href="<?php echo urlSeller('order_print','small',array('order_id'=>$order_id));?>" class="wtsc-btn-mini" target="_blank" title="打印发货单"/><i class="icon-print"></i>小票打印</a></span>
 </th>
    </tr>
    <?php $i = 0;?>
    ...
    <tr>
      ...
      <td class="bdl bdr" rowspan="<?php echo $order['goods_count'];?>"><p><?php echo $order['state_desc']; ?>
        ...
        <!-- 订单查看 -->
        <p><a href="<?php echo urlSeller('order','show_order',array('order_id'=>$order_id));?>" target="_blank"><?php echo $lang['store_order_view_order'];?></a></p>
        
        <!-- 物流跟踪 -->
        <p>
          <?php if ($order['if_deliver']) { ?>
          <a href='<?php echo urlSeller('deliver','search_deliver',array('order_sn'=>$order['order_sn']));?>'><?php echo $lang['store_order_show_deliver'];?></a>
          <?php } ?>
        </p>
	...
	<td><?php if($order['is_print'] == 1){ echo "已打印"; }else{ echo "未打印";} ?></td>
      ...
      <td class="bdl bdr" rowspan="<?php echo $order['goods_count'];?>">
        ...
        <p><a href="javascript:void(0)" class="wtsc-btn wtsc-btn-red mt5" wt_type="dialog" uri="<?php echo urlSeller('order','change_state',array('state_type'=>'order_cancel','order_sn'=>$order['order_sn'],'order_id'=>$order['order_id']));?>" dialog_title="<?php echo $lang['store_order_cancel_order'];?>" dialog_id="seller_order_cancel_order" dialog_width="400" id="order<?php echo $order['order_id']; ?>_action_cancel" /><i class="icon-remove-bbs"></i><?php echo $lang['store_order_cancel_order'];?></a></p>
        <?php } ?>
        
        <!-- 修改运费 -->
        <?php if ($order['if_modify_price']) { ?>
        <p><a href="javascript:void(0)" class="wtsc-btn-mini wtsc-btn-orange mt10" uri="<?php echo urlSeller('order','change_state',array('state_type'=>'modify_price','order_sn'=>$order['order_sn'],'order_id'=>$order['order_id']));?>" dialog_width="480" dialog_title="<?php echo $lang['store_order_modify_price'];?>" wt_type="dialog"  dialog_id="seller_order_adjust_fee" id="order<?php echo $order['order_id']; ?>_action_adjust_fee" /><i class="icon-pencil"></i>修改运费</a></p>
        <?php }?>
        <!-- 修改价格 -->
		<?php if ($order['if_spay_price']) { ?>
        <p><a href="javascript:void(0)" class="wtsc-btn-mini wtsc-btn-green mt10" uri="<?php echo urlSeller('order','change_state',array('state_type'=>'spay_price','order_sn'=>$order['order_sn'],'order_id'=>$order['order_id']));?>" dialog_width="480" dialog_title="<?php echo $lang['store_order_modify_price'];?>" wt_type="dialog"  dialog_id="seller_order_adjust_fee" id="order<?php echo $order['order_id']; ?>_action_adjust_fee" /><i class="icon-pencil"></i>修改价格</a></p>
		<?php }?>
        
        <!-- 发货 -->
        <?php if ($order['if_send']) { ?>
        <p><a class="wtsc-btn wtsc-btn-green mt10" href="<?php echo urlSeller('deliver','send',array('order_id'=>$order['order_id']));?>"/><i class="icon-truck"></i><?php echo $lang['store_order_send'];?></a></p>
        <?php } ?>
...
var timeval = 15;
var timeinterval;
var SELLER_SITE_URL = '<?php echo SELLER_SITE_URL;?>';	
...
function refresh()
{
	$.ajax({
		type: "GET",
		url: '<?php echo urlSeller('order_call','getajaxorder');?>',
		dataType:"json",
		async: true,
				...
				for(var i = 0; i < order_list.length; i++)
				{
					...
					for(var k = 0; k < order_list[i].goods_list.length; k++)
					{
						...
						if ((order_list[i].goods_count > 0 && k==0) || (order_list[i].goods_count) == 1 )
						{
							...
							//订单查看
							html_str += '	<p><a href="<?php echo urlSeller('order','show_order');?>?order_id='+order_list[i].order_id+'" target="_blank">订单详情</a></p>';
								
							//物流跟踪
							html_str += '		<p>';
							if(order_list[i].if_deliver)
							{
								html_str += '<a href=\'<?php echo urlSeller('deliver','search_deliver');?>?order_sn='+order_list[i].order_sn+'\'>查看物流</a>';
							}
							...
							if(order_list[i].if_send){
								html_str += '<p>';
								html_str += '<a class="wtsc-btn wtsc-btn-green mt10" href="<?php echo urlSeller('deliver','send');?>?order_id='+order_list[i].order_id+'">';
...
{
    if(length > 0){
		var url = '<?php echo urlSeller('order_call_print');?>';
...
{
	if(length > 0){
		var url = '<?php echo urlSeller('order_call_print','small');?>';
	    window.open(url,'_blank');
	}
}
...
function select_type(obj)
{
	...
	$.ajax({
		type: "GET",
		url: '<?php echo urlSeller('order_call','select_type');?>?type='+type,
		dataType:"json",
		async: true,
		success: function(data){
			if(data == 1)
