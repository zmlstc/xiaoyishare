...
<?php } else { ?>
  ...
  <a class="wtbtn wtbtn-aqua" href="<?php echo urlSeller('sale_fcode', 'fcode_quota_add');?>" title="购买套餐"><i class="icon-money"></i>购买套餐</a>
  <?php } else { ?>
  <a class="wtbtn wtbtn-mint" href="javascript:void(0);" wttype="select_goods" style="right:100px"><i class="icon-plus-sign"></i>添加商品</a>
  <a class="wtbtn wtbtn wtbtn-aqua" href="<?php echo urlSeller('sale_fcode', 'fcode_renew');?>"><i class="icon-money"></i>套餐续费</a>
  <?php } ?>
<?php } ?>
</div>
  ...
  <tbody wttype="choose_goods_list">
    ...
    <tr class="bd-line">
      ...
      <td class="nscs-table-handle">
        <span><a class="btn-bittersweet" href='<?php echo urlSeller('sale_fcode', 'download_f_code_excel', array('gid' => $val['goods_id']));?>' title="下载F码"><i class="icon-download"></i><p>下载</p></a></span>
...
$(function(){
    ...
    $('input[wttype="btn_search_goods"]').click(function(){
        _url = '<?php echo urlSeller('sale_fcode', 'fcode_select_goods');?>';
    ...
    $('div[wttype="div_goods_select"]').on('click', 'a[wttype="a_choose_goods"]', function(){
        ...
        CUR_DIALOG =ajax_form('choose_goods', 'F码商品规则设定', '<?php echo urlSeller('sale_fcode', 'choosed_goods');?>&gid='+_gid, 640);
    });

    // 删除商品
    $('tbody[wttype="choose_goods_list"]').on('click','a[wttype="del_choosed"]', function(){
        $this = $(this);
        _url = '<?php echo urlSeller('sale_fcode', 'del_choosed_goods');?>';
...
function choose_goods(data) {
    ...
    $('<tr class="bd-line"></tr>')
        ...
        .append('<td class="nscs-table-handle"><span><a class="btn-bittersweet" href="<?php echo urlSeller('sale_fcode', 'download_f_code_excel');?>&gid=' + data.goods_id + '" title="下载F码"><i class="icon-download"></i><p>下载</p></a></span><span><a class="btn-grapefruit" href="javascript:void(0);" data-param="{gid:'+ data.goods_id +'}" wttype="del_choosed"><i class="icon-trash"></i><p>删除</p></a></span></td>')
