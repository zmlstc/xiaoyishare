...
<div class="tabmenu">
  ...
  <a class="wtbtn wtbtn-mint" href="<?php echo urlSeller('sale_pingou', 'pingou_add');?>"><i class="icon-plus-sign"></i>添加活动</a>
  <?php } else { ?>
  <?php if(!empty($output['current_quota'])) { ?>
  <a class="wtbtn wtbtn-mint" style="right:100px" href="<?php echo urlSeller('sale_pingou', 'pingou_add');?>"><i class="icon-plus-sign"></i>添加活动</a> <a class="wtbtn wtbtn-aqua" href="<?php echo urlSeller('sale_pingou', 'quota_add');?>" title=""><i class="icon-money"></i>套餐续费</a>
  <?php } else { ?>
  <a class="wtbtn wtbtn-aqua" href="<?php echo urlSeller('sale_pingou', 'quota_add');?>" title=""><i class="icon-money"></i>购买套餐</a>
  <?php } ?>
  <?php } ?>
</div>
<?php if ($output['isOwnShop']) { ?>
...
<?php } ?>
<form method="get" action="<?php echo urlSeller('sale_pingou','list');?>">
  ...
  <tbody id="list">
    <tr class="bd-line">
      ...
      <td class="nscs-table-handle tr"><?php if($val['state'] && $val['start_time'] > TIMESTAMP) { ?>
        <span> <a href="<?php echo urlSeller('sale_pingou','pingou_edit',array('pingou_id'=>$val['pingou_id']));?>" class="btn-bluejeans"> <i class="icon-edit"></i>
        <p><?php echo $lang['wt_edit'];?></p>
        </a> </span>
        <?php } ?>
        <?php if($val['state'] && $val['end_time'] > TIMESTAMP) { ?>
        <span> <a href="<?php echo urlSeller('sale_pingou','pingou_manage',array('pingou_id'=>$val['pingou_id']));?>" class="btn-mint"> <i class="icon-cog"></i>
    ...
    $(document).ready(function(){
        $('[wttype="btn_del_pingou"]').on('click', function() {
            if(confirm('<?php echo $lang['wt_ensure_del'];?>')) {
                var action = "<?php echo urlSeller('sale_pingou', 'pingou_del');?>";
