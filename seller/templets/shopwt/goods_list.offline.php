...
<form method="get" action="<?php echo urlSeller('goods_offline','index');?>">
...
<table class="wtsc-default-table">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><a href="javascript:void(0)" onclick="ajax_get_confirm('','<?php echo urlSeller('goods_offline', 'goods_show', array('commonid' => $val['goods_commonid']));?>')" class="wtbtn"><?php echo $lang['store_goods_index_show'];?></a></td>
      ...
      <td class="nscs-table-handle"><?php if ($val['goods_lock'] == 0) {?>
        ...
        <?php } else {?>
        <span class="tip" title="该商品参加抢购活动期间不能进行编辑及删除等操作,但可以编辑赠品"><a href="<?php if ($val['is_virtual'] ==1 ) {echo 'javascript:void(0);';} else {echo urlShop('goods_online', 'add_gift', array('commonid' => $val['goods_commonid']));}?>" class="btn-darkgray"><i class="icon-lock"></i>
        <p>锁定</p>
        </a></span>
        <?php }?></td>
    </tr>
