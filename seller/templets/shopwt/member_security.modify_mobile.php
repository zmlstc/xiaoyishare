...
<div class="wrap">
  ...
  <div class="wtm-default-form">
    <form method="post" id="mobile_form" action="<?php echo urlShop('member_security','modify_mobile');?>">
...
$(function(){
	...
	$('#send_auth_code').on('click',function(){
		...
		ALLOW_SEND = !ALLOW_SEND;
		$('#sending').show();
		$.getJSON('<?php echo urlShop('member_security','send_modify_mobile');?>',{mobile:$('#mobile').val()},function(data){
			if (data.state == 'true') {
				$('#sending').hide();
