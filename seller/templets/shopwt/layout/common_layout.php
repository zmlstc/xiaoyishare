...
$wapurl = WAP_SITE_URL;
	...
	if(strpos($agent,"comFront") || strpos($agent,"iPhone") || strpos($agent,"MIDP-2.0") || strpos($agent,"Opera Mini") || strpos($agent,"UCWEB") || strpos($agent,"Android") || strpos($agent,"Windows CE") || strpos($agent,"SymbianOS")){
        ...
        if(!empty($config['wap_site_url'])){
            ...
            switch ($_GET['w']){
			case 'goods':
			  $url .= '/html/goods_detail.html?goods_id=' . $_GET['goods_id'];
			  break;
			case 'store':
			  $url .= '/shop.html';
			  break;
...
$("img[rel='lazy']").scrollLoading();
	...
	$('#keyword').focus(function(){
        ...
        source: function (request, response) {
            $.getJSON('<?php echo urlShop('search','auto_complete');?>', request, function (data, status, xhr) {
	...
	$('#button').click(function(){
	    if ($('#keyword').val() == '') {
		    if ($('#keyword').attr('data-value') == '') {
				...
				window.location.href="<?php echo urlShop('search','');?>?keyword="+$('#keyword').attr('data-value');
			    return false;
			}
	    }
	});
  ...
  <header class="wt-head wrapper">
    ...
    <div class="wt-head-search">
     ...
     <div id="search">
        ...
        <form action="<?php echo urlShop('search','index');?>" method="get" class="search-form" id="head_search_form">
    ...
    <div class="head-my-menu">
      <dl class="my-cart">
        ...
        <dd>
          ...
          <div class="checkout"> <span class="total-price">共<i><?php echo $output['cart_goods_num'];?></i><?php echo $lang['wt_kindof_goods'];?></span><a href="<?php echo urlShop('cart');?>" class="btn-cart"><?php echo $lang['wt_bill_goods'];?></a> </div>
        </dd>
      </dl>
    </div>
  </header>
