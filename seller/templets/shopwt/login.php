...
$(document).ready(function() {
    ...
    function change_vercode() {
        $('#codeimage').attr('src', '<?php echo urlSeller('vercode','',array('type'=>'30x92'));?>?c=' + Math.random());
        $('#captcha').select();
    }
    ...
    $("#form_login").validate({
        ...
        rules:{
            ...
            captcha:{
                required:true,
                remote:{
                    url:"<?php echo urlSeller('vercode','check');?>",
                    type:"get",
                    data:{
                        captcha:function() {
                            return $("#captcha").val();
...
.logo .link i{display:inline-block;vertical-align:middle;background:url(<?php echo SELLER_TEMPLATES_URL;?>/images/seller/login/dlgj.png) no-repeat;width:28px;height:26px}
		...
		<div class="wtsc-login-layout" style="background: <?php echo $output['lpic']['color'];?> url(<?php echo $output['lpic']['pic'];?>) no-repeat scroll 0px center;">
			...
			<div class="right-login">
				<div class="wtsc-login-container">
					...
					<form id="form_login" action="<?php echo urlSeller('login','login');?>" method="post">
						...
						<div class="input">
							...
							<div class="code">
								<div class="arrow"></div>
								<div class="code-img"><a href="javascript:void(0)" wttype="btn_change_vercode"><img src="<?php echo urlSeller('vercode');?>" name="codeimage" border="0" id="codeimage"></a>
								</div>
