...
<dl>
  ...
  <dd>
    <input type="text" class="w340" onclick="oCopy(this)" value="<?php echo urlShop('voucher','getvoucher',array('tid'=>$output['t_info']['voucher_t_id']));?>" readonly/>
<script>
	function oCopy(obj){
      obj.select();
...
<h3>已生成代金券
  <a id="voucher_exportbtn" class="wtbtn-mini" href="javascript:void(0);" onclick="javascript:go('<?php echo urlSeller('voucher','voucher_export',array('tid'=>$output['t_info']['voucher_t_id']));?>');" title="导出Excel" style="float: right; margin-right: 10px; display: none;">导出Excel</a>
</h3>
...
<script type="text/javascript">
  $("#voucher_list_div").load('<?php echo urlSeller('voucher','voucherlist',array('tid'=>$output['t_info']['voucher_t_id']));?>');
</script>
<?php } ?>
<div class="bottom">
  <a href="javascript:void(0);" class="submit" onclick="window.location='<?php echo urlSeller('voucher','templatelist');?>'" > <?php echo $lang['voucher_template_backlist'];?></a>
</div>
<script type="text/javascript">
$(document).ready(function(){
	$("#build_voucher").click(function(){
		ajaxget('<?php echo urlSeller('voucher','bulidvoucher',array('tid'=>$output['t_info']['voucher_t_id']));?>');
	});
});
</script>