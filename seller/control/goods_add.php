...
class goods_addControl extends BaseSellerControl {
    /**
     * 三方店铺验证，商品数量，有效期
     */
    private function checkStore(){
        ...
        if ($goodsLimit > 0) {
            ...
            if ($goods_num >= $goodsLimit) {
                showMessage(L('store_goods_index_goods_limit') . $goodsLimit . L('store_goods_index_goods_limit1'), urlSeller('goods_online','goods_list'), 'html', 'error');
            }
        }
    }
    ...
    public function save_goodsWt() {
        ...
        if(!$result['state']) {
            showMessage(L('error') . $result['msg'], urlSeller('index'), 'html', 'error');
        }

        redirect(urlSeller('goods_add', 'add_step_three', array('commonid' => $result['data'])));
    }
    ...
    public function save_imageWt(){
        if (chksubmit()) {
            ...
            if ($rs) {
                redirect(urlSeller('goods_add', 'add_step_four', array('commonid' => $common_id)));
            } else {
