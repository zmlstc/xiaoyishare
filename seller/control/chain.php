...
class chainControl extends BaseSellerControl {
    private $model_chain;
    public function __construct() {
        parent::__construct();

        if (!checkPlatformStore()) {
            showMessage('该功能只有平台自营店铺使用', urlSeller('seller_center', 'index'), '', 'error');
        }
    ...
    public function chain_addWt() {
        ...
        if (chksubmit()){
            ...
            if ($result) {
                showDialog('操作成功', urlSeller('chain', 'index'), 'succ');
            } else {
                showDialog('操作失败', 'reload');
            }
        }
        $this->profile_menu('chain_add', 'chain_add');
        Tpl::showpage('chain.add');
    }
    ...
    public function chain_editWt() {
        ...
        if (chksubmit()){
            ...
            if ($result) {
                showDialog('编辑成功', urlSeller('chain', 'index'), 'succ');
            } else {
    ...
    public function chain_delWt() {
        ...
        if ($result) {
            showDialog('删除成功', urlSeller('chain', 'index'), 'succ');
        } else {
    ...
    private function profile_menu($menu_type,$menu_key,$array=array()) {
        ...
        switch ($menu_type) {
            case 'chain_list':
                $menu_array = array(
                    array('menu_key' => 'chain_list', 'menu_name' => '门店列表', 'menu_url' => urlSeller('chain', 'chain_list'))
                );
                break;
            case 'chain_add':
                $menu_array = array(
                    array('menu_key' => 'chain_list', 'menu_name' => '门店列表', 'menu_url' => urlSeller('chain', 'chain_list')),
                    array('menu_key' => 'chain_add', 'menu_name' => '添加门店', 'menu_url' => urlSeller('chain', 'chain_add'))
                );
                break;
            case 'chain_edit':
                $menu_array = array(
                    array('menu_key' => 'chain_list', 'menu_name' => '门店列表', 'menu_url' => urlSeller('chain', 'chain_list')),
                    array('menu_key' => 'chain_add', 'menu_name' => '添加门店', 'menu_url' => urlSeller('chain', 'chain_add')),
                    array('menu_key' => 'chain_edit', 'menu_name' => '编辑门店', 'menu_url' => urlSeller('chain', 'chain_edit'))
                );
                break;
        }
        if(!empty($array)) {
