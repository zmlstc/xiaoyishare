...
class voucherControl extends BaseSellerControl{
    //定义代金券类常量
    ...
    public function __construct() {
        ...
        if (C('voucher_allow') != 1){
            showMessage(Language::get('voucher_unavailable'),urlSeller('store'),'html','error');
        }
        //申请记录状态
    ...
    public function templatelistWt(){
        ...
        Tpl::showpage('voucher_template.index') ;
    }
    ...
    public function voucherlistWt(){
        ...
        if($t_id <= 0){
            Tpl::showpage('voucher.list','null_layout');
        }
        $model_voucher = Model('voucher');
        //查询代金券模板
        $where = array();
    ...
    public function voucher_exportWt(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){//显示下载链接
                ...
                Tpl::output('murl',urlSeller('voucher','voucher_export'));
    ...
    public function quotaaddWt(){
        if (chksubmit()){
            ...
            if($reault){
                showDialog(Language::get('voucher_apply_buy_succ'),urlSeller('voucher','quotalist'),'succ');
            } else {
                showDialog(Language::get('wt_common_op_fail'),urlSeller('voucher','quotalist'));
            }
        }else {
            //输出导航
            self::profile_menu('quota_add','quotaadd');
            Tpl::showpage('voucher_quota.add');
        }
    }
    /*
     * 代金券模版添加
    ...
    public function templateaddWt(){
        ...
        if ($isOwnShop = checkPlatformStore()) {
            ...
            if(empty($quotainfo)){
                showMessage(Language::get('voucher_template_quotanull'),urlSeller('voucher','quotaadd'),'html','error');
            }
            ...
            if ($count >= C('sale_voucher_storetimes_limit')){
                ...
                showMessage($message,urlSeller('voucher','templatelist'),'html','error');
            }
        }
        ...
        if(empty($pricelist)){
            showMessage(Language::get('voucher_template_pricelisterror'),urlSeller('voucher','templatelist'),'html','error');
        }
        //获取领取方式
        ...
        if(chksubmit()){
            ...
            if ($error){
                ...
                if($rs){
                    ...
                    showDialog(Language::get('wt_common_save_succ'),urlSeller('voucher','templatelist'),'succ');
                }else{
                    showDialog(Language::get('wt_common_save_fail'),urlSeller('voucher','templatelist'),'error');
                }
            }
        }else{
            //店铺分类
            ...
            Tpl::showpage('voucher_template.add');
        }
    }
    /*
     * 代金券模版编辑
    ...
    public function templateeditWt(){
        ...
        if ($t_id <= 0){
            showMessage(Language::get('wrong_argument'),urlSeller('voucher','templatelist'),'html','error');
        }
        $model = Model('voucher');
        //查询模板信息
        $param = array();
        ...
        if(chksubmit()){
            ...
            if ($error){
                ...
                if($rs){
                    ...
                    showDialog(Language::get('wt_common_op_succ'),urlSeller('voucher','templatelist'),'succ');
                }else{
                    showDialog(Language::get('wt_common_op_fail'),urlSeller('voucher','templatelist'),'error');
                }
            }
        }else{
    ...
    public function templateinfoWt(){
        ...
        Tpl::showpage('voucher_template.info');
    }
    /*
     * 把代金券模版设为失效
     */
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'voucher':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('wt_member_path_store_voucher'), 'menu_url'=>urlSeller('voucher','templatelist')),
                );
                break;
            case 'quota_add':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('wt_member_path_store_voucher'), 'menu_url'=>urlSeller('voucher','templatelist')),
                4=>array('menu_key'=>'quotaadd','menu_name'=>Language::get('voucher_applyadd'), 'menu_url'=>urlSeller('voucher','quotaadd'))
                );
                break;
            case 'template':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('wt_member_path_store_voucher'), 'menu_url'=>urlSeller('voucher','templatelist')),
                2=>array('menu_key'=>'templateadd','menu_name'=>Language::get('voucher_templateadd'),   'menu_url'=>urlSeller('voucher','templateadd')),
                );
                break;
            case 'templateedit':
                $menu_array = array(
