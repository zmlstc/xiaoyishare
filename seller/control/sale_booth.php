...
class sale_boothControl extends BaseSellerControl {
    ...
    public function booth_goods_listWt() {
            ...
            if (!empty($boothgoods_list)) {
                ...
                if (!empty($goods_list)) {
                    ...
                    foreach ($goods_list as $key => $val) {
                        ...
                        $goods_list[$key]['url'] = urlSeller('goods', 'index', array('goods_id' => $val['goods_id']));
                    }
        ...
        Tpl::showpage('sale_booth.goods_list');
    }
    ...
    public function booth_select_goodsWt() {
        ...
        Tpl::showpage('sale_booth.select_goods', 'null_layout');
    }
    ...
    public function booth_quota_addWt() {
        if (chksubmit()) {
            ...
            if ($quantity <= 0 || $quantity > 12) {
                showDialog('参数错误，购买失败。', urlSeller('sale_booth', 'booth_quota_add'), '', 'error' );
            }
            // 实例化模型
            $model_booth = Model('p_booth');
            ...
            if ($return) {
                ...
                showDialog('购买成功', urlSeller('sale_booth', 'booth_goods_list'), 'succ');
            } else {
                showDialog('购买失败', urlSeller('sale_booth', 'booth_quota_add'));
            }
        }
        // 输出导航
        self::profile_menu('booth_quota_add', 'booth_quota_add');
        Tpl::showpage('sale_booth.quota_add');
    }
    ...
    public function booth_renewWt() {
        if (chksubmit()) {
            ...
            if ($return) {
                ...
                showDialog('购买成功', urlSeller('sale_booth', 'booth_list'), 'succ');
            } else {
                showDialog('购买失败', urlSeller('sale_booth', 'booth_quota_add'));
            }
        }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'booth_goods_list':
                $menu_array = array(
                    1=>array('menu_key'=>'booth_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlSeller('sale_booth', 'booth_goods_list'))
                );
                break;
            case 'booth_quota_add':
                $menu_array = array(
                    1=>array('menu_key'=>'booth_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlSeller('sale_booth', 'booth_goods_list')),
                    2=>array('menu_key'=>'booth_quota_add', 'menu_name'=>'购买套餐', 'menu_url'=>urlSeller('sale_booth', 'booth_quota_add'))
                );
                break;
            case 'booth_renew':
                $menu_array = array(
                    1=>array('menu_key'=>'booth_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlSeller('sale_booth', 'booth_goods_list')),
                    2=>array('menu_key'=>'booth_renew', 'menu_name'=>'套餐续费', 'menu_url'=>urlSeller('sale_booth', 'booth_renew'))
                );
                break;
        }
