...
class infoControl extends BaseSellerControl {
    ...
    public function bind_class_addWt() {
        ...
        self::profile_menu('index','bind_class');
        Tpl::showpage('bind_class.add','null_layout');
    }
    ...
    public function bind_class_saveWt() {
        ...
        if($result) {
            showDialog('申请成功，请等待系统审核',urlSeller('info','bind_class'),'succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
        }else {
            showDialog($lang['wt_common_save_fail']);
        }
    }
...
//              $model_store->editStore(array('store_end_time'=>$data['re_end_time']),array('store_id'=>$_SESSION['store_id']));
    ...
    private function profile_menu($menu_type,$menu_key='',$array=array()) {
        ...
        switch ($menu_type) {
            case 'index':
                $menu_array[] = array('menu_key'=>'bind_class', 'menu_name'=>$lang['wt_member_path_bind_class'], 'menu_url'=>urlSeller('info','bind_class'));
                if (!checkPlatformStore()) {
                    $menu_array[] = array('menu_key'=>'index', 'menu_name'=>$lang['wt_member_path_store_info'], 'menu_url'=>urlSeller('info'));
                    $menu_array[] = array('menu_key'=>'reopen', 'menu_name'=>$lang['wt_member_path_store_reopen'], 'menu_url'=>urlSeller('info','reopen'));
                }
                break;
            case 'bind_class':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>$lang['wt_member_path_bind_class'], 'menu_url'=>urlSeller('store_bind_class')),
                );
                break;
            case 'add':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>$lang['wt_member_path_bind_class'], 'menu_url'=>urlSeller('store_bind_class')),
                array('menu_key'=>'add', 'menu_name'=>$lang['wt_member_path_bind_class_add'], 'menu_url'=>urlSeller('store_bind_class','add'))
                );
                break;
        }
        if(!empty($array)) {
