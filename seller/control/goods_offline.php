...
class goods_offlineControl extends BaseSellerControl {
    ...
    public function goods_storageWt() {
        ...
        switch ($_GET['type']) {
            // 违规的商品
            case 'lock_up':
                Tpl::showpage('goods_list.offline_lockup');
                break;
            // 等待审核或审核失败的商品
            case 'wait_verify':
                Tpl::output('verify', array('0' => '未通过', '10' => '等待审核'));
                Tpl::showpage('goods_list.offline_waitverify');
                break;
            // 仓库中的商品
            default:
    ...
    private function profile_menu($menu_key = '') {
        $menu_array = array(
            array('menu_key' => 'goods_storage',    'menu_name' => L('wt_member_path_goods_storage'),   'menu_url' => urlSeller('goods_offline', 'index')),
            array('menu_key' => 'goods_lockup',     'menu_name' => L('wt_member_path_goods_state'),     'menu_url' => urlSeller('goods_offline', 'index', array('type' => 'lock_up'))),
            array('menu_key' => 'goods_verify',     'menu_name' => L('wt_member_path_goods_verify'),    'menu_url' => urlSeller('goods_offline', 'index', array('type' => 'wait_verify')))
        );
