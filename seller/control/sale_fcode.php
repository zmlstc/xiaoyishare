...
class sale_fcodeControl extends BaseSellerControl {
    ...
    public function fcode_listWt() {
        ...
        Tpl::showpage('sale_fcode.goods_list');
    }
    ...
    public function fcode_quota_addWt() {
        if (chksubmit()) {
            ...
            if ($quantity <= 0 || $quantity > 12) {
                showDialog('参数错误，购买失败。', urlSeller('sale_fcode', 'fcode_quota_add'), '', 'error' );
            }
            // 实例化模型
            $model_fcode = Model('p_fcode');
            ...
            if ($return) {
                ...
                showDialog('购买成功', urlSeller('sale_fcode', 'fcode_goods_list'), 'succ');
            } else {
                showDialog('购买失败', urlSeller('sale_fcode', 'fcode_quota_add'));
            }
        }
        // 输出导航
        $this->profile_menu('fcode_quota_add', 'fcode_quota_add');
        Tpl::showpage('sale_fcode.quota_add');
    }
    ...
    public function fcode_renewWt() {
        if (chksubmit()) {
            ...
            if ($return) {
                ...
                showDialog('购买成功', urlSeller('sale_fcode', 'fcode_list'), 'succ');
            } else {
                showDialog('购买失败', urlSeller('sale_fcode', 'fcode_quota_add'));
            }
        }
    ...
    public function fcode_select_goodsWt() {
        ...
        Tpl::showpage('sale_fcode.select_goods', 'null_layout');
    }
    ...
    public function choosed_goodsWt() {
        ...
        Tpl::showpage('sale_fcode.choosed_goods', 'null_layout');
    }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'fcode_goods_list':
                $menu_array = array(
                    1=>array('menu_key'=>'fcode_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlSeller('sale_fcode', 'fcode_goods_list'))
                );
                break;
            case 'fcode_quota_add':
                $menu_array = array(
                    1=>array('menu_key'=>'fcode_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlSeller('sale_fcode', 'fcode_goods_list')),
                    2=>array('menu_key'=>'fcode_quota_add', 'menu_name'=>'购买套餐', 'menu_url'=>urlSeller('sale_fcode', 'fcode_quota_add'))
                );
                break;
            case 'fcode_renew':
                $menu_array = array(
                    1=>array('menu_key'=>'fcode_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlSeller('sale_fcode', 'fcode_goods_list')),
                    2=>array('menu_key'=>'fcode_renew', 'menu_name'=>'套餐续费', 'menu_url'=>urlSeller('sale_fcode', 'fcode_renew'))
                );
                break;
        }
