...
class robbuyControl extends BaseSellerControl {
    ...
    public function __construct() {
        ...
        if (intval(C('robbuy_allow')) !== 1){
            showMessage(Language::get('robbuy_unavailable'),urlSeller('index'),'','error');
        }
    }
    /**
     * 默认显示抢购列表
    ...
    public function robbuy_quota_add_saveWt() {
        ...
        showDialog(Language::get('robbuy_quota_add_success'), urlSeller('robbuy', 'robbuy_list'), 'succ');
    }
    ...
    public function robbuy_addWt() {
        ...
        if (checkPlatformStore()) {
            ...
            $current_robbuy_quota = $model_robbuy_quota->getRobbuyQuotaCurrent($_SESSION['store_id']);
            if(empty($current_robbuy_quota)) {
                showMessage('当前没有可用套餐，请先购买套餐',urlSeller('robbuy', 'robbuy_quota_add'),'','error');
            }
            Tpl::output('current_robbuy_quota', $current_robbuy_quota);
        }
    ...
    public function robbuy_saveWt() {
        ...
        if (!checkPlatformStore()) {
            ...
            $current_robbuy_quota = $model_robbuy_quota->getRobbuyQuotaCurrent($_SESSION['store_id']);
            if(empty($current_robbuy_quota)) {
                showDialog('当前没有可用套餐，请先购买套餐',urlSeller('robbuy', 'robbuy_quota_add'),'error');
            }
        }
        ...
        if($result) {
            ...
            showDialog(Language::get('robbuy_add_success'),urlSeller('robbuy'),'succ');
        }else {
            showDialog(Language::get('robbuy_add_fail'),urlSeller('robbuy'));
        }
    }
    ...
    public function search_goodsWt() {
        ...
        Tpl::showpage('robbuy.goods', 'null_layout');
    }
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            1=>array('menu_key'=>'robbuy_list','menu_name'=>L('wt_member_path_group_list'),'menu_url'=>urlSeller('robbuy', 'robbuy_list'))
        );
        switch ($menu_key){
        case 'robbuy_add':
            $menu_array[] = array('menu_key'=>'robbuy_add','menu_name'=>L('wt_member_path_new_group'),'menu_url'=>urlSeller('robbuy','robbuy_add'));
            break;
        case 'robbuy_add_vr':
            $menu_array[] = array('menu_key'=>'robbuy_add_vr','menu_name'=>'新增虚拟抢购','menu_url'=>urlSeller('robbuy','robbuy_add_vr'));
            break;
        case 'robbuy_quota_add':
            $menu_array[] = array('menu_key'=>'robbuy_quota_add','menu_name'=>'购买套餐','menu_url'=>urlSeller('robbuy', 'robbuy_quota_add'));
            break;
        case 'robbuy_edit':
            $menu_array[] = array('menu_key'=>'robbuy_edit','menu_name'=>L('wt_member_path_edit_group'),'menu_url'=>urlSeller('robbuy'));
            break;
        case 'cancel':
