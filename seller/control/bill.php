...
class billControl extends BaseSellerControl {
    /**
     * 每次导出多少条记录
     * @var unknown
     */
    ...
    public function show_billWt(){
        ...
        if ($_GET['type'] =='refund'){
            ...
            $sub_tpl_name = 'bill.show.refund_list';
            ...
            $sub_tpl_name = 'bill.show.cost_list';
            ...
            $sub_tpl_name = 'bill.show.fx_list';
            ...
            $sub_tpl_name = 'bill.show.order_book_list';
    ...
    public function export_orderWt(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){
                ...
                Tpl::output('murl',urlSeller('bill','show_bill',array('ob_id'=>$_GET['ob_id'])));
    ...
    public function export_refund_orderWt(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('list',$array);
                Tpl::output('murl',urlSeller('bill','show_bill',array('query_type'=>'refund','ob_id'=>$_GET['ob_id'])));
                Tpl::showpage('export.excel');
                exit();
            }else{
                //如果数量小，直接下载
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'list':
                $menu_array = array(
                1=>array('menu_key'=>'list','menu_name'=>'实物订单结算', 'menu_url'=>urlSeller('bill','list')),
                );
                break;
            case 'show':
                $menu_array = array(
                array('menu_key'=>'order_list','menu_name'=>'订单列表', 'menu_url'=>urlSeller('bill','show_bill',array('ob_id'=>$_GET['ob_id']))),
                array('menu_key'=>'refund_list','menu_name'=>'退款订单','menu_url'=>urlSeller('bill','show_bill',array('type'=>'refund','ob_id'=>$_GET['ob_id']))),
                array('menu_key'=>'cost_list','menu_name'=>'促销费用','menu_url'=>urlSeller('bill','show_bill',array('type'=>'cost','ob_id'=>$_GET['ob_id'])))
                );
                if (floatval($this->_bill_info['ob_order_book_totals']) > 0) {
                    array_push($menu_array,array('menu_key'=>'book_list','menu_name'=>'未退定金', 'menu_url'=>urlSeller('bill','show_bill',array('type'=>'book','ob_id'=>$_GET['ob_id']))));
                }
                if (C('fenxiao_isuse') == 1) {
                    array_push($menu_array,array('menu_key'=>'fx_list','menu_name'=>'分销佣金', 'menu_url'=>urlSeller('bill','show_bill',array('type'=>'dis','ob_id'=>$_GET['ob_id']))));
                }
                break;
        }
