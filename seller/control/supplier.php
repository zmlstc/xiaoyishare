...
class supplierControl extends BaseSellerControl {
    ...
    public function sup_addWt() {
        ...
        Tpl::showpage('supplier.add','null_layout');
    }
    ...
    public function sup_delWt() {
        ...
        if ($sup_id > 0){
            ...
            showDialog(Language::get('wt_common_del_succ'),urlSeller('supplier','sup_list'),'succ');
        }else {
    ...
    private function profile_menu($menu_type, $menu_key = '', $array = array()) {
        ...
        switch ($menu_type) {
        	case 'sup_list':
        	    $menu_array = array(
        	    array('menu_key'=>'sup_list', 'menu_name'=>'供货商', 'menu_url'=>urlSeller('supplier','sup_list'))
        	    );
        	    break;
        }
        if(!empty($array)) {
