...
class sale_bookControl extends BaseSellerControl {
    ...
    public function book_listWt() {
        ...
        Tpl::showpage('sale_book.goods_list');
    }
    ...
    public function book_quota_addWt() {
        if (chksubmit()) {
            ...
            if ($quantity <= 0 || $quantity > 12) {
                showDialog('参数错误，购买失败。', urlSeller('sale_book', 'book_quota_add'), '', 'error' );
            }
            // 实例化模型
            $model_book = Model('p_book');
            ...
            if ($return) {
                ...
                showDialog('购买成功', urlSeller('sale_book', 'book_goods_list'), 'succ');
            } else {
                showDialog('购买失败', urlSeller('sale_book', 'book_quota_add'));
            }
        }
        // 输出导航
        $this->profile_menu('book_quota_add', 'book_quota_add');
        Tpl::showpage('sale_book.quota_add');
    }
    ...
    public function book_renewWt() {
        if (chksubmit()) {
            ...
            if ($return) {
                ...
                showDialog('购买成功', urlSeller('sale_book', 'book_list'), 'succ');
            } else {
                showDialog('购买失败', urlSeller('sale_book', 'book_quota_add'));
            }
        }
    ...
    public function book_select_goodsWt() {
        ...
        Tpl::showpage('sale_book.select_goods', 'null_layout');
    }
    ...
    public function choosed_goodsWt() {
        ...
        Tpl::showpage('sale_book.choosed_goods', 'null_layout');
    }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'book_goods_list':
                $menu_array = array(
                    1=>array('menu_key'=>'book_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlSeller('sale_book', 'book_goods_list'))
                );
                break;
            case 'book_quota_add':
                $menu_array = array(
                    1=>array('menu_key'=>'book_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlSeller('sale_book', 'book_goods_list')),
                    2=>array('menu_key'=>'book_quota_add', 'menu_name'=>'购买套餐', 'menu_url'=>urlSeller('sale_book', 'book_quota_add'))
                );
                break;
            case 'book_renew':
                $menu_array = array(
                    1=>array('menu_key'=>'book_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlSeller('sale_book', 'book_goods_list')),
                    2=>array('menu_key'=>'book_renew', 'menu_name'=>'套餐续费', 'menu_url'=>urlSeller('sale_book', 'book_renew'))
                );
                break;
        }
