...
class goods_onlineControl extends BaseSellerControl {
    ...
    public function goods_listWt() {
        ...
        // 商品分类
        $goods_class = Model('store_goods_class')->getClassTree(array('store_id' => $_SESSION['store_id'], 'stc_state' => '1'));
    ...
    public function edit_save_goodsWt() {
        ...
        if ($result['state']) {
            ...
            //回滚事务
            showDialog($result['msg'], urlSeller('goods_online', 'index'));
        }
    }
    ...
    public function edit_imageWt() {
        ...
        if ($common_id <= 0) {
            showMessage(L('wrong_argument'), urlSeller('index'), 'html', 'error');
        }
        $model_goods = Model('goods');
    ...
    public function edit_save_imageWt() {
        if (chksubmit()) {
            ...
            if ($rs['state']) {
                ...
                showDialog(L('wt_common_save_fail'), urlSeller('goods_online', 'index'));
            }
        }
    }
    ...
    public function goods_unshowWt() {
        ...
        if ($result['state']) {
            showDialog(L('store_goods_index_goods_unshow_success'), getReferer() ? getReferer() : urlSeller('goods_online','goods_list'), 'succ', '', 2);
        } else {
    ...
    public function add_giftWt() {
        ...
        Tpl::showpage('goods_edit.add_gift');
    }
   ...
   * 设置商品佣金
    ...
    public function edit_invite_priceWt() {
        ...
        Tpl::showpage('list.edit_invite_price', 'null_layout');
    }
    ...
    public function search_goodsWt() {
        ...
        Tpl::output('goods_list', $goods_list);
        Tpl::showpage('goods_edit.search_goods', 'null_layout');
    }
    ...
    public function edit_iframe_ajaxWt() {
        ...
        Tpl::showpage('goods_list.edit_iframe', 'null_layout');
    }
    ...
    public function edit_body_ajaxWt() {
        ...
        Tpl::showpage('goods_list.edit_body', 'null_layout');
    }
    ...
    private function profile_menu($menu_type,$menu_key, $allow_sale = array()) {
        ...
        switch ($menu_type) {
            case 'goods_list':
                $menu_array = array(
                   array('menu_key' => 'goods_list',    'menu_name' => '出售中的商品', 'menu_url' => urlSeller('goods_online', 'index'))
                );
                break;
            case 'edit_detail':
                if ($allow_sale['lock'] === false) {
                    $menu_array = array(
                        array('menu_key' => 'edit_detail',  'menu_name' => '编辑商品', 'menu_url' => urlSeller('goods_online', 'edit_goods', array('commonid' => $_GET['commonid'])).'?ref_url='. $_GET['ref_url']),
                        array('menu_key' => 'edit_image',   'menu_name' => '编辑图片', 'menu_url' => urlSeller('goods_online', 'edit_image', array('commonid' => $_GET['commonid'])).'?ref_url='. ($_GET['ref_url'] ? $_GET['ref_url'] : getReferer())),
                    );
                }
                if ($allow_sale['gift'] == true) {
                    $menu_array[] = array('menu_key' => 'add_gift', 'menu_name' => '赠送赠品', 'menu_url' => urlSeller('goods_online', 'add_gift', array('commonid' => $_GET['commonid'])).'?ref_url='. ($_GET['ref_url'] ? $_GET['ref_url'] : getReferer()));
                }
                break;
            case 'edit_class':
                $menu_array = array(
                    array('menu_key' => 'edit_class',   'menu_name' => '选择分类', 'menu_url' => urlSeller('goods_online', 'edit_class', array('commonid' => $_GET['commonid'])).'?ref_url='. $_GET['ref_url']),
                    array('menu_key' => 'edit_detail',  'menu_name' => '编辑商品', 'menu_url' => urlSeller('goods_online', 'edit_goods', array('commonid' => $_GET['commonid'])).'?ref_url='.$_GET['ref_url']),
                    array('menu_key' => 'edit_image',   'menu_name' => '编辑图片', 'menu_url' => urlSeller('goods_online', 'edit_image', array('commonid' => $_GET['commonid'])).'?ref_url=' .($_GET['ref_url'] ? $_GET['ref_url'] : getReferer())),
                );
                break;
	...
	public function maker_qrcodeWt()
	{
		...
		foreach($lst as $k=>$v)
		{
			$goods_id=$v['goods_id'];
			//生成二维码
			$qrcode_url=WAP_SITE_URL . '/html/goods_detail.html?goods_id='.$goods_id;
