...
class navigationControl extends BaseSellerControl {
    ...
    public function navigation_listWt() {
        ...
        self::profile_menu('navigation');
        Tpl::showpage('navigation.list');
    }

    public function navigation_addWt() {
        $this->profile_menu('navigation_add');
        Tpl::showpage('navigation.form');
    }

    public function navigation_editWt() {
        $sn_id = intval($_GET['sn_id']);
        if($sn_id <= 0) {
           showMessage(L('wrong_argument'), urlSeller('navigation', 'navigation_list'), '', 'error');
        }
        $model_store_navigation = Model('store_navigation');
    ...
    public function navigation_saveWt() {
        ...
        showDialog(L('wt_common_op_succ'), urlSeller('navigation', 'navigation_list'), 'succ');
    }
    ...
    public function navigation_delWt() {
        ...
        if($sn_id > 0) {
            ...
            showDialog(L('wt_common_op_fail'), urlSeller('navigation', 'navigation_list'), 'error');
        }
    }
    ...
    private function profile_menu($menu_key = '') {
        ...
        $menu_array[] = array(
            'menu_key' => 'navigation',
            'menu_name' => '导航列表',
            'menu_url' => urlSeller('navigation', 'navigation_list')
        );
        if($menu_key == 'navigation_add') {
            $menu_array[] = array(
                'menu_key' => 'navigation_add',
                'menu_name' => '添加导航',
                'menu_url' => urlSeller('navigation', 'navigation_add')
            );
        }
        if($menu_key == 'navigation_edit') {
            $menu_array[] = array(
                ...
                'menu_name' => '编辑导航',
                'menu_url' => urlSeller('navigation', 'navigation_edit')
            );
        }
