...
class deliver_setControl extends BaseSellerControl {
    ...
    public function daddress_listWt() {
       ...
       Tpl::showpage('deliver_set.daddress_list');
    }
    ...
    public function daddress_addWt() {
        ...
        if (chksubmit()) {
            ...
            if (empty($address_info) && !is_array($address_info)){
                showMessage($lang['store_daddress_wrong_argument'],urlSeller('deliver_set','daddress_list'),'html','error');
            }
            Tpl::output('address_info',$address_info);
        }
        Tpl::showpage('deliver_set.daddress_add','null_layout');
    }
    ...
    public function daddress_delWt() {
        ...
        if ($delete){
            showDialog(Language::get('store_daddress_del_succ'),urlSeller('deliver_set','daddress_list'),'succ');
        }else {
            showDialog(Language::get('store_daddress_del_fail'),'','error');
        }
    }
   ...
   public function daddress_default_setWt() {
    ...
    public function free_freightWt(){
        ...
        Tpl::showpage('free_freight.index');
    }
    ...
    public function deliver_regionWt(){
        ...
        Tpl::showpage('deliver_region.index');
    }
    ...
    public function print_setWt(){
        ...
        if(empty($store_info)){
            showDialog(Language::get('store_storeinfo_error'),urlSeller('store_printsetup'),'error');
        }
        if(chksubmit()){
            $obj_validate = new Validate();
            $obj_validate->validateparam = array(
    ...
    private function profile_menu($menu_type, $menu_key = '') {
        ...
        switch ($menu_type) {
            case 'daddress':
                $menu_array = array(
                array('menu_key'=>'daddress',   'menu_name'=>Language::get('store_deliver_daddress_list'),  'menu_url'=>urlSeller('deliver_set','daddress_list')),
                array('menu_key'=>'express',        'menu_name'=>Language::get('store_deliver_default_express'),    'menu_url'=>urlSeller('deliver_set','express')),
                array('menu_key'=>'free_freight',   'menu_name'=>'免运费额度',      'menu_url'=>urlSeller('deliver_set','free_freight')),
				array('menu_key'=>'free_time',   'menu_name'=>'预计送达时间',      'menu_url'=>urlSeller('deliver_set','free_time')),
                array('menu_key'=>'deliver_region',   'menu_name'=>'默认配送地区',      'menu_url'=>urlSeller('deliver_set','deliver_region')),
                array('menu_key'=>'print_set', 'menu_name'=>'发货单打印设置', 'menu_url'=>urlSeller('deliver_set','print_set'))
                );
                break;
        }
