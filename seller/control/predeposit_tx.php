...
class predeposit_txControl extends BaseSellerControl {
    ...
    public function pd_cash_addWt(){
        if (chksubmit()){
            ...
            if (floatval($member_info['available_predeposit']) < $pdc_amount){
                showDialog(Language::get('predeposit_cash_shortprice_error'),urlSeller('predeposit_tx','pd_cash_list'),'error');
            }
            try {
                ...
                $model_pd->commit();
                showDialog(Language::get('predeposit_cash_add_success'),urlSeller('predeposit_tx','pd_cash_list'),'succ');
            } catch (Exception $e) {
                $model_pd->rollback();
                showDialog($e->getMessage(),urlSeller('predeposit_tx','pd_cash_list'),'error');
            }
        }
    }
    ...
    public function pd_cash_infoWt(){
        ...
        if ($pdc_id <= 0){
            showMessage(Language::get('predeposit_parameter_error'),urlSeller('predeposit_tx','pd_cash_list'),'html','error');
        }
        $model_pd = Model('predeposit');
        $condition = array();
        $condition['pdc_member_id'] = $_SESSION['member_id'];
        ...
        if (empty($info)){
            showMessage(Language::get('predeposit_record_error'),urlSeller('predeposit_tx','pd_cash_list'),'html','error');
        }
    ...
    private function profile_menu($menu_type,$menu_key=''){
        $menu_array = array(
           array('menu_key'=>'cashlist',       'menu_name'=>'提现记录',    'menu_url'=>urlSeller('predeposit_tx','pd_cash_list')),
        );
        switch ($menu_type) {
            case 'rechargeinfo':
