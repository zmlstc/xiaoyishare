...
class albumControl extends BaseSellerControl {
...
//      Tpl::output('show_page',$page->show());
    ...
    public function album_addWt(){
        ...
        Tpl::showpage('album.class_add','null_layout');
    }
    /**
     * 相册保存
     *
    ...
    public function album_add_saveWt(){
        if (chksubmit()){
            ...
            if($class_count['count'] >= 40){
                showDialog(Language::get('album_class_save_max_20'),urlSeller('album'),'error',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
            }
            /**
             * 实例化相册模型
             */
            ...
            if($return){
                showDialog(Language::get('album_class_save_succeed'),urlSeller('album'),'succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
            }
        }
    ...
    public function album_editWt(){
        ...
        Tpl::showpage('album.class_edit','null_layout');
    }
    /**
     * 相册分类编辑保存
     */
    public function album_edit_saveWt(){
        ...
        if($return){
            ...
            if($re){
                showDialog(Language::get('album_class_edit_succeed'),urlSeller('album'),'succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
            }
        }else{
    ...
    public function album_delWt(){
        ...
        if($return){
            showDialog(Language::get('album_class_file_del_succeed'),urlSeller('album'),'succ');
        }else{
            showDialog(Language::get('album_class_file_del_lose'));
        }
    }
    ...
    public function pic_listWt(){
        ...
        case 'des':
            ...
            break;
        case 'robbuy':
            Tpl::showpage('robbuy.album','null_layout');
            break;
    ...
    public function album_ad_ajaxWt(){
        ...
        Tpl::showpage('album.pic_scroll_ajax','null_layout');
    }
    ...
    public function album_pic_moveWt(){
        ...
        Tpl::output('class_list',$class_list);
        Tpl::showpage('album.move','null_layout');
    }
	...
	public function album_pic_watermarkWt(){
		...
		if ($store_wm_info['wm_image_name'] == '' && $store_wm_info['wm_text'] == ''){
			showDialog(Language::get('album_class_setting_wm'),urlSeller('album','store_watermark'),'error','CUR_DIALOG.close();');//"请先设置水印"
		}
		import('bin.gdimage');
    ...
    private function profile_menu($menu_type,$menu_key=''){
        ...
        switch ($menu_type) {
            case 'album':
                $menu_array = array(
                1=>array('menu_key'=>'album','menu_name'=>Language::get('wt_member_path_my_album'),'menu_url'=>urlSeller('album')),
                2=>array('menu_key'=>'watermark','menu_name'=>Language::get('wt_member_path_watermark'),'menu_url'=>urlSeller('album','store_watermark'))
                );
                break;
            case 'album_pic':
                $menu_array = array(
                1=>array('menu_key'=>'album','menu_name'=>Language::get('wt_member_path_my_album'),'menu_url'=>urlSeller('album')),
                3=>array('menu_key'=>'pic_list','menu_name'=>Language::get('wt_member_path_album_pic_list'),'menu_url'=>urlSeller('album','album_pic_list',array('id'=>intval($_GET['id'])))),
                2=>array('menu_key'=>'watermark','menu_name'=>Language::get('wt_member_path_watermark'),'menu_url'=>urlSeller('album','store_watermark'))
                );
                break;
            case 'album_pic_info':
                $menu_array = array(
                1=>array('menu_key'=>'album','menu_name'=>Language::get('wt_member_path_my_album'),'menu_url'=>urlSeller('album')),
                3=>array('menu_key'=>'pic_info','menu_name'=>Language::get('wt_member_path_album_pic_info'),'menu_url'=>urlSeller('album','album_pic_info',array('class_id'=>intval($_GET['class_id']),'id'=>intval($_GET['id'])))),
                2=>array('menu_key'=>'watermark','menu_name'=>Language::get('wt_member_path_watermark'),'menu_url'=>urlSeller('album','store_watermark'))
                );
                break;
        }
