...
class transportControl extends BaseSellerControl {
    ...
    public function saveWt(){
        ...
        if ($result){
            header('location: '.urlSeller('transport','',array('type'=>$_POST['type'])));exit;
        }else{
            showMessage(Language::get('transport_op_fail'),getReferer(),'html','error');
        }
    }
    ...
    public function offpay_areaWt()
    {
        ...
        Tpl::showpage('transport.offpay_area');
    }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            ...
            case 'offpay_area':
                $menu_array = array(
                1=>array('menu_key'=>'transport',   'menu_name'=>Language::get('wt_member_path_postage'),       'menu_url'=>urlSeller('transport')),
                );
                if (checkPlatformStore()) {
                    $menu_array[] = array(
                        'menu_key' => 'offpay_area',
                        'menu_name' => '货到付款地区',
                        'menu_url' => urlSeller('transport','offpay_area')
                    );
                }
                break;
        }
