...
class brandControl extends BaseSellerControl {
    ...
    public function brand_addWt() {
        ...
        Tpl::showpage('brand.add','null_layout');
    }
    ...
    public function brand_saveWt() {
        ...
        if (chksubmit()) {
            ...
            if ($result){
                showDialog($lang['store_goods_brand_apply_success'],urlSeller('brand','brand_list'),'succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
            }else {
        ...
        if ($_POST['form_submit'] == 'ok' and intval($_POST['brand_id']) != 0) {
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    showDialog($lang['wt_common_save_succ'],urlSeller('brand','brand_list'),'succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
                }else {
                    showDialog($lang['wt_common_save_fail']);
                }
            }
    ...
    public function drop_brandWt() {
        ...
        if ($brand_id > 0){
            ...
            showDialog(Language::get('wt_common_del_succ'),urlSeller('brand','brand_list'),'succ');
        }else {
    ...
    private function profile_menu($menu_type,$menu_key='',$array=array()) {
        ...
        switch ($menu_type) {
            case 'brand_list':
                $menu_array = array(
                    1=>array('menu_key'=>'brand_list', 'menu_name'=>$lang['wt_member_path_brand_list'], 'menu_url'=>urlSeller('brand','brand_list'))
                );
                break;
        }
        if(!empty($array)) {
