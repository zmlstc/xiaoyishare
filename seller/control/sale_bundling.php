...
class sale_bundlingControl extends BaseSellerControl {
    ...
    public function __construct() {
        ...
        if (intval(C('sale_allow')) !== 1) {
            showMessage("商品促销功能尚未开启", urlSeller('index'),'','error');
        }
    ...
    public function bundling_quota_addWt() {
        if (chksubmit()) {
            ...
            if ($quantity <= 0 || $quantity > 12) {
                showDialog(Language::get('bundling_quota_price_fail'), urlSeller('sale_bundling', 'bundling_quota_add'), '', 'error' );
            }
            // 实例化模型
            $model_bundling = Model('p_bundling');
            ...
            if ($return) {
                ...
                showDialog(L('bundling_quota_price_succ'), urlSeller('sale_bundling', 'bundling_list'), 'succ');
            } else {
                showDialog(L('bundling_quota_price_fail'), urlSeller('sale_bundling', 'bundling_quota_add'));
            }
        }
        // 输出导航
        self::profile_menu('bundling_quota_add', 'bundling_quota_add');
        Tpl::showpage('sale_bundling.quota_add');
    }
    ...
    public function bundling_addWt() {
        ...
        if (chksubmit()) {
            ...
            showDialog(L('wt_common_op_succ'), urlSeller('sale_bundling', 'bundling_list'), 'succ');
        }
        ...
        if (intval($_GET['bundling_id']) > 0) {
            ...
            if (empty($bundling_info['store_id'])) {
                showMessage(L( 'wrong_argument'), urlSeller('sale_bundling', 'bundling_list'), '', 'error' );
            }
        ...
        Tpl::showpage('sale_bundling.add');
    }
    ...
    public function bundling_add_goodsWt() {
        ...
        Tpl::showpage('sale_bundling.add_goods', 'null_layout');
    }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            ...
            case 'bundling_quota_list':
                $menu_array = array(
                    1=>array('menu_key'=>'bundling_list', 'menu_name'=>Language::get('bundling_list'), 'menu_url'=>urlSeller('sale_bundling', 'bundling_list'))
                );
                break;
            case 'bundling_quota_add':
                $menu_array = array(
                    1=>array('menu_key'=>'bundling_list', 'menu_name'=>Language::get('bundling_list'), 'menu_url'=>urlSeller('sale_bundling', 'bundling_list')),
                    2=>array('menu_key'=>'bundling_quota_add', 'menu_name'=>Language::get('bundling_quota_add'), 'menu_url'=>urlSeller('sale_bundling', 'bundling_quota_add'))
                );
                break;
            case 'bundling_renew':
                $menu_array = array(
                    1=>array('menu_key'=>'bundling_list', 'menu_name'=>Language::get('bundling_list'), 'menu_url'=>urlSeller('sale_bundling', 'bundling_list')),
                    2=>array('menu_key'=>'bundling_renew', 'menu_name'=>'套餐续费', 'menu_url'=>urlSeller('sale_bundling', 'bundling_renew'))
                );
                break;
            case 'bundling_add':
                $menu_array = array(
                    1=>array('menu_key'=>'bundling_list', 'menu_name'=>Language::get('bundling_list'), 'menu_url'=>urlSeller('sale_bundling', 'bundling_list')),
                    2=>array('menu_key'=>'bundling_add', 'menu_name'=>Language::get('bundling_add'), 'menu_url'=>urlSeller('sale_bundling', 'bundling_add'))
                );
                break;
            case 'bundling_edit':
                $menu_array = array(
                    1=>array('menu_key'=>'bundling_list', 'menu_name'=>Language::get('bundling_list'), 'menu_url'=>urlSeller('sale_bundling', 'bundling_list')),
                    2=>array('menu_key'=>'bundling_edit', 'menu_name'=>Language::get('bundling_edit'), 'menu_url'=>urlSeller('sale_bundling', 'bundling_edit'))
                );
            break;
        }
