...
class vr_billControl extends BaseSellerControl {
    /**
     * 每次导出多少条记录
     * @var unknown
     */
    ...
    public function show_billWt(){
        ...
        Tpl::output('sub_tpl_name','vr_bill.show.code_list');
        Tpl::output('bill_info',$bill_info);
        $this->profile_menu('show',$_GET['type']);
        Tpl::showpage('vr_bill.show');
    }
    ...
    public function export_orderWt(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){
                ...
                Tpl::output('murl',urlSeller('vr_bill','show_bill',array('ob_id'=>$_GET['ob_id'])));
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'list':
                $menu_array = array(
                1=>array('menu_key'=>'list','menu_name'=>'虚拟订单结算', 'menu_url'=>urlSeller('vr_bill','list')),
                );
                break;
            case 'show':
                $menu_array = array(
                array('menu_key'=>'','menu_name'=>'已使用', 'menu_url'=>urlSeller('vr_bill','show_bill',array('ob_id'=>$_GET['ob_id']))),
                array('menu_key'=>'timeout','menu_name'=>'已过期',  'menu_url'=>urlSeller('vr_bill','show_bill',array('type'=>'timeout','ob_id'=>$_GET['ob_id'])))
                );
                break;
        }
