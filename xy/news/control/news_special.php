...
class news_specialControl extends SystemControl{
        ...
        if (!empty($list) && is_array($list)){
            ...
            foreach ($list as $k => $v){
                ...
                if($v['special_state'] == '2') {
                    ...
                    $operation .= '<li><a href="'.urlAdminCms('news_special','news_special_detail',array('special_id'=>$v['special_id'])).'" target="_blank">预览专题页面</a></li>';
                }
                $operation .= '<li><a href="'.urlAdminCms('news_special','news_special_edit',array('special_id'=>$v['special_id'])).'">编辑专题内容</a></li>';
                $operation .= '</ul></span>';
    ...
    public function news_special_saveWt() {
        ...
        if($result) {
            ...
            showMessage(Language::get('wt_common_save_succ'), urlAdminCms('news_special','news_special_list'));
        } else {
            $this->log(Language::get('news_log_special_save').$result, 0);
            showMessage(Language::get('wt_common_save_fail'), urlAdminCms('news_special','news_special_list'));
        }
    }
    /**
     * 专题安全过滤
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'special_list'=>array('menu_type'=>'link','menu_name'=>Language::get('wt_manage'),'menu_url'=>urlAdminCms('news_special','news_special_list')),
            'special_add'=>array('menu_type'=>'link','menu_name'=>Language::get('wt_new'),'menu_url'=>urlAdminCms('news_special','news_special_add')),
