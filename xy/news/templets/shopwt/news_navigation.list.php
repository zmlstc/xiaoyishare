    ...
    <table class="flex-table">
      ...
      <tbody>
        ...
        <tr data-id="<?php echo $val['navigation_id']; ?>">
          ...
          <td class="handle-s"><a href="<?php echo urlAdminCms('news_navigation','news_navigation_drop',array('navigation_id'=>$val['navigation_id']));?>" class="btn red confirm-del"><i class="fa fa-trash-o"></i><?php echo $lang['wt_del'];?></a></td>
          ...
          <td class="yes-onoff"><a href="JavaScript:void(0);" class=" <?php echo $val['navigation_open_type']? 'enabled':'disabled'?>" ajax_branch='navigation_open_type'  wt_type="inline_edit" fieldname="navigation_open_type" fieldid="<?php echo $val['navigation_id']?>" fieldvalue="<?php echo $val['navigation_open_type']?'1':'0'?>" title="<?php echo $lang['wt_editable'];?>"><img src="<?php echo ADMIN_TEMPLATES_URL;?>/images/lucid.gif"></a></td>
          <td></td>
        </tr>
        <?php } ?>
        <?php }else { ?>
        ...
        buttons : [
            ...
            },
            {
                ...
                onpress : function() {
                    ...
                    location.href = '<?php echo urlAdminCms('news_navigation','news_navigation_drop');?>?navigation_id=__IDS__'.replace('__IDS__', ids.join(','));
                }
            }
        ]
    });
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = '<?php echo urlAdminCms('news_navigation','news_navigation_add');?>';
    }
}
</script>