...
function update_flex(){
    $("#flexigrid").flexigrid({
        url: '<?php echo urlAdminCms('news_special','news_special_xml');?>',
        colModel : [
...
function fg_operation_del(special_id){
    if (confirm('确定删除？')) {
        window.location.href = '<?php echo urlAdminCms('news_special','news_special_drop');?>?special_id='+special_id;
    }
}
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = '<?php echo urlAdminCms('news_special','news_special_add');?>';
    }
}
</script>