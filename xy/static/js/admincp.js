...
//布局换色设置
    ...
    if ($.cookie('shopwtWorkParam') == null) {
        // 默认选择第一个菜单
        $('.wt-module-menu').find('li:first > a').click();
    } else {
        openItem($.cookie('shopwtWorkParam'));
    }
    ...
    $('div[data-param^="map-"]').find('i').click(function(){
        ...
        if ($this.parent().hasClass('selected')) {
            $.getJSON('index.php/common/common_operations', {type : 'del', value : _value}, function(data){
                if (data) {
            ...
            $.getJSON('index.php/common/common_operations', {type : 'add', value : _value}, function(data){
                if (data) {
    ...
    if ($.cookie('commonPendingMatters') == null) {
        $.ajax({
                url : ADMIN_SITE_URL+'/index.php/common/ajax_pending_matters',
                async : false,
...
function openItem(param) {
    ...
    $('#workspace').attr('src', ADMIN_SITE_URL + '/' + data_str[0] + '/index.php/' + data_str[1]);
    $.cookie('shopwtWorkParam', data_str[0] + '|' + data_str[1], { expires: 1 ,path:"/"});
}