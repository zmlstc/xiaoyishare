...
var CURRENT_SITEURL = '<?php echo APP_SITE_URL;?>';
</script>
...
$(function(){
	...
	function ajaxFileUpload()
	{
		...
		(
			{
				url:'<?php echo urlAdmin('common','pic_upload',array('type'=>'admin_avatar'));?>?form_submit=ok&uploadpath=<?php echo ATTACH_ADMIN_AVATAR;?>',
				secureuri:false,
				fileElementId:'_pic',
				dataType: 'json',
				...
				{
					if (data.status == 1){
						ajax_form('cutpic','<?php echo $lang['wt_cut'];?>','<?php echo urlAdmin('common','pic_cut',array('type'=>'admin_avatar'));?>?x=100&y=100&resize=1&ratio=1&url='+data.url,690);
					}else{
						alert(data.msg);
					}
...
function call_back(picname){
    $.getJSON('<?php echo urlAdmin('index','save_avatar');?>?avatar=' + picname, function(data){
        if (data) {
...
<div class="admiwtp-header">
  ...
  <div class="admiwtp-header-r">
    ...
    <ul class="operate wt-row">
      ...
      <li><a class="time" href="<?php echo urlAdminSystem('setting','exetarget');?>" target="workspace"  title="手动计划任务触发">&nbsp;</a></li>
      ...
      <li><a class="login-out show-option" href="<?php echo urlAdmin('index','logout');?>" title="安全退出管理中心">&nbsp;</a></li>
    </ul>
  </div>
  <div class="clear"></div>
</div>
