...
EOB;
    ...
    public function prod_addWt(){
        ...
        if (chksubmit()){
            ...
            $validate_arr[] = array("input"=>$_POST["store_id"],"require"=>"true","message"=>'请输入店铺ID');
			...
			$store_info = Model('store')->getStoreInfo(array('store_id'=>intval($_POST["store_id"])));
			if(empty($store_info)||!is_array($store_info)) {
				 showDialog('输入的店铺ID不存在！','','error');
			}

            $model_pointprod = Model('pointprod');
            $goods_array = array();
            $goods_array['store_id']     = $store_info['store_id'];
            $goods_array['store_name']     = $store_info['store_name'];
            $goods_array['city_id']     = $store_info['city_id'];
            $goods_array['area_id']     = $store_info['area_id'];
            ...
            $goods_array['pgoods_islimittime'] = 1;//intval($_POST['islimittime']);
		...
		/* 	else {
            ...
            if($state){
                ...
                showDialog(L('admin_pointprod_add_success'),urlAdminShop('pointprod','pointprod'),'succ');
            }
        }
        //模型实例化
        $where = array();
    ...
    public function prod_editWt(){
        ...
        if (!$pg_id){
            showDialog(L('admin_pointprod_parameter_error'),urlAdminShop('pointprod','pointprod'),'error');
        }
        $model_pointprod = Model('pointprod');
        //查询礼品记录是否存在
        $prod_info = $model_pointprod->getPointProdInfo(array('pgoods_id'=>$pg_id));
        if (!$prod_info){
            showDialog(L('admin_pointprod_record_error'),urlAdminShop('pointprod','pointprod'),'error');
        }
        if (chksubmit()){
            //验证表单
            $obj_validate = new Validate();
            ...
            if($state){
                ...
                showDialog(L('admin_pointprod_edit_success'),urlAdminShop('pointprod','pointprod'),'succ');
            }
        }else {
            $where = array();
            $where['upload_type'] = '6';
...
Tpl::setDirquna('shop');
    ...
    public function order_dropWt(){
        ...
        if ($data['state']){
            showDialog(L('admin_pointorder_del_success'),urlAdminShop('pointprod','pointorder_list'),'succ');
        } else {
            showDialog($data['msg'],urlAdminShop('pointprod','pointorder_list'),'error');
        }
    }
    ...
    public function order_cancelWt(){
        ...
        if ($data['state']){
            showDialog(L('admin_pointorder_cancel_success'),urlAdminShop('pointprod','pointorder_list'),'succ');
        }else {
            showDialog($data['msg'],urlAdminShop('pointprod','pointorder_list'),'error');
        }
    }
    ...
    public function order_shipWt(){
        ...
        if ($order_id <= 0){
            showDialog(L('admin_pointorder_parameter_error'),urlAdminShop('pointprod','pointorder_list'),'error');
        }
        $model_pointorder = Model('pointorder');
        //获取订单状态
        ...
        if (!$order_info){
            showDialog(L('admin_pointorderd_record_error'),urlAdminShop('pointprod','pointorder_list'),'error');
        }
        if (chksubmit()){
            $obj_validate = new Validate();
            ...
            $error = $obj_validate->validate();
            if ($error != ''){
                showDialog(L('error').$error,urlAdminShop('pointprod','pointorder_list'),'error');
            }
            //发货
            $data = $model_pointorder->shippingPointOrder($order_id, $_POST, $order_info);
            if ($data['state']){
                showDialog('发货修改成功',urlAdminShop('pointprod','pointorder_list'),'succ');
            }else {
                showDialog($data['msg'],urlAdminShop('pointprod','pointorder_list'),'error');
            }
        } else {
