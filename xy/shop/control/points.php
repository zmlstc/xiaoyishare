...
class pointsControl extends SystemControl{
    ...
    public function __construct(){
        ...
        //判断系统是否开启积分功能
        if (C('points_isuse') != 1){
            showMessage(Language::get('admin_points_unavailable'),urlAdminShop('setting'),'','error');
        }
    }
    ...
    public function addpointsWt(){
        if (chksubmit()){
            ...
            if (!is_array($member_info) || count($member_info)<=0){
                showMessage(Language::get('admin_points_userrecord_error'),urlAdminShop('points','addpoints'),'','error');
            }

            $pointsnum = intval($_POST['pointsnum']);
            if ($_POST['operatetype'] == 2 && $pointsnum > intval($member_info['member_points'])){
                showMessage(Language::get('admin_points_points_short_error').$member_info['member_points'],urlAdminShop('points','addpoints'),'','error');
            }
            ...
            if ($result){
                ...
                showMessage(Language::get('wt_common_save_succ'),urlAdminShop('points','addpoints'));
            }else {
                showMessage(Language::get('wt_common_save_fail'),urlAdminShop('points','addpoints'),'','error');
            }
        }else {
			Tpl::setDirquna('shop');
    ...
    public function settingWt() {
        ...
        if (chksubmit()){
            ...
            $update_array['cost_points_bl'] = intval($_POST['cost_points_bl'])?$_POST['cost_points_bl']:0;
