...
class stat_memberControl extends SystemControl{
    private $links = array(
        array('url'=>'stat_member/newmember','lang'=>'stat_newmember'),
        array('url'=>'stat_member/analyze','lang'=>'stat_memberanalyze'),
        array('url'=>'stat_member/scale','lang'=>'stat_scaleanalyze'),
        array('url'=>'stat_member/area','lang'=>'stat_areaanalyze'),
        array('url'=>'stat_member/buying','lang'=>'stat_buying'),
	array('url'=>'stat_member/area_xq','text'=>'县级分析'),
    );
    ...
    public function newmemberWt(){
        ...
        if($this->search_arr['search_type'] == 'day'){
            ...
            Tpl::output('actionurl',urlAdminShop('stat_member','newmember',array('search_type'=>'day','search_time'=>date('Y-m-d',$this->search_arr['day']['search_time']))));
        }
        ...
        if($this->search_arr['search_type'] == 'week'){
            ...
            Tpl::output('actionurl',urlAdminShop('stat_member','newmember',array('search_type'=>'week','searchweek_year'=>$this->search_arr['week']['current_year'],'searchweek_month'=>$this->search_arr['week']['current_month'],'searchweek_week'=>$this->search_arr['week']['current_week'])));
        }
        ...
        if($this->search_arr['search_type'] == 'month'){
            ...
            Tpl::output('actionurl',urlAdminShop('stat_member','newmember',array('search_type'=>'month','searchmonth_year'=>$this->search_arr['month']['current_year'],'searchmonth_month'=>$this->search_arr['month']['current_month'])));
        }
    ...
    public function showmemberWt(){
        ...
        $this->links[] = array('url'=>'stat_member/showmember','lang'=>'stat_memberlist');
    ...
    public function get_arealist_xq_xmlWt(){
        ...
        if (!empty($list) && is_array($list)){
            foreach ($list as $k => $v){
                ...
                $out_array['operation'] = '--';
				if($area_deep==1){
					$out_array['operation'] = '<a href="'.urlAdminShop('stat_member','area_xq',array('areadeep'=>($area_deep+1),'province_id'=>$province_id)).'" >查看市级</a>';
				}
				if($area_deep==2){
					$out_array['operation'] = '<a href="'.urlAdminShop('stat_member','area_xq',array('areadeep'=>($area_deep+1),'city_id'=>$province_id)).'" >查看县级</a>';
				}
                $out_array['provincename'] = ($t = $province_array[$province_id]) ? $t : '其他';
                $out_array = getFlexigridArray($out_array,$order_type,$v);
                $out_array['orderamount'] = wtPriceFormat($out_array['orderamount']);
