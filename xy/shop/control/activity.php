    ...
    public function activity_xmlWt()
    {
        ...
        foreach ($list as $val) {
            ...
            if ($val['activity_state'] == 0 || $val['activity_end_date'] < time()) {
                $o .= '<a class="btn red confirm-del-on-click" href="javascript:;" data-href="'.urlAdminShop('activity','del',array('activity_id'=>$val['activity_id'])).                   
                    '"><i class="fa fa-trash-o"></i>删除</a>';
            }

            $o .= '<span class="btn"><em><i class="fa fa-cog"></i>设置<i class="arrow"></i></em><ul>';

            $o .= '<li><a href="'.urlAdminShop('activity','edit',array('activity_id'=>$val['activity_id'])).
                    '">编辑活动</a></li>';

            $o .= '<li><a href="'.urlAdminShop('activity','detail',array('id'=>$val['activity_id'])).
                '">处理申请</a></li>';
...
EOB;
    ...
    public function newWt(){
        ...
        if($result){
            ...
            showMessage(Language::get('wt_common_op_succ'),urlAdminShop('activity','activity'));
        }else{
    ...
    public function editWt(){
        ...
        if($result){
            ...
            showMessage(Language::get('wt_common_save_succ'),urlAdminShop('activity','activity'));
        }else{
            if($_FILES['activity_banner']['name']!=''){
                @unlink(BASE_UPLOAD_PATH.DS.ATTACH_ACTIVITY.DS.$upload->file_name);
            }
