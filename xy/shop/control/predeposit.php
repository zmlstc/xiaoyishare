...
class predepositControl extends SystemControl{
    ...
    public function recharge_editWt(){
        $id = intval($_GET['id']);
        if ($id <= 0){
            showMessage(Language::get('admin_predeposit_parameter_error'),urlAdminShop('predeposit','predeposit'),'','error');
        }
        //查询充值信息
        ...
        if (empty($info)){
            showMessage(Language::get('admin_predeposit_record_error'),urlAdminShop('predeposit','predeposit'),'','error');
        }
        if (!chksubmit()) {
            //显示支付接口列表
        ...
        try {
            ...
            showMessage(Language::get('admin_predeposit_recharge_edit_success'),urlAdminShop('predeposit','predeposit'));
        } catch (Exception $e) {
            $model_pd->rollback();
            $this->log($log_msg,0);
            showMessage($e->getMessage(),urlAdminShop('predeposit','predeposit'),'html','error');
        }
    }
    ...
    public function pd_cash_payWt(){
        $id = intval($_GET['id']);
        if ($id <= 0){
            showMessage(Language::get('admin_predeposit_parameter_error'),urlAdminShop('predeposit','pd_cash_list'),'','error');
        }
        $model_pd = Model('predeposit');
        $condition = array();
        $condition['pdc_id'] = $id;
        ...
        if (!is_array($info) || count($info)<0){
            showMessage(Language::get('admin_predeposit_record_error'),urlAdminShop('predeposit','pd_cash_list'),'','error');
        }
        ...
        try {
            ...
            $model_pd->commit();
			$this->pd_cash_pay_Post($info);
            $this->log($log_msg,1);
            showMessage(Language::get('admin_predeposit_cash_edit_success'),urlAdminShop('predeposit','pd_cash_list'));
        } catch (Exception $e) {
            $model_pd->rollback();
            $this->log($log_msg,0);
            showMessage($e->getMessage(),urlAdminShop('predeposit','pd_cash_list'),'html','error');
        }
    }
	
	public function pd_cash_pay_Post($cash_info){
		$handle_trans = Handle('trans_api');
				
		if($cash_info['pdc_type_code']=='wxpay'){			
			
			$t_data = array();
			$t_data['order_sn'] = $cash_info['pdc_sn'];
			$t_data['pay_account'] = $cash_info['pdc_openid'];
			$t_data['amount'] = $cash_info['pdc_amount'];
			$t_data['real_name'] = $cash_info['pdc_bank_user'];
			$t_data['remark'] = '会员账户提现';
			
			$trans_data = $handle_trans->wxpayTransfer($t_data);
				Log::record('会员账户提现--微信--data==:'.json_encode($t_data));
				Log::record('会员账户提现--微信--trans_api==:'.json_encode($trans_data));
			if(!empty($trans_data)&&$trans_data['code'] == 10000){
				$updata = Model('predeposit')->editPdCash(array('pdc_pay_state'=>1,'trade_no'=>$trans_data['pay_order_id'],'pdc_pay_time'=>$trans_data['pay_date']),array('pdc_id'=>$cash_info['pdc_id']));
				
			} else {
				 showMessage('微信转账失败:'.$trans_data['sub_msg'].'['.$trans_data['code'].']',urlAdminShop('predeposit','pd_cash_list'),'','error');
				Log::record('---会员账户提现--微信 ==失败--- '.json_encode($trans_data));
			...
			$t_data = array();
			$t_data['order_sn'] = $cash_info['pdc_sn'];
			$t_data['pay_account'] = $cash_info['pdc_bank_no'];
			$t_data['amount'] = $cash_info['pdc_amount'];
			$t_data['real_name'] = $cash_info['pdc_bank_user'];
			$t_data['remark'] = '会员账户提现';
			...
			$trans_data = $handle_trans->alipayTransfer($t_data);
				Log::record('会员账户提现--微信--data==:'.json_encode($t_data));
				Log::record('会员账户提现--支付宝--trans_api==:'.json_encode($trans_data));
			if(!empty($trans_data)&&$trans_data['code'] == 10000){
				$updata = Model('predeposit')->editPdCash(array('pdc_pay_state'=>1,'trade_no'=>$trans_data['pay_order_id'],'pdc_pay_time'=>$trans_data['pay_date']),array('pdc_id'=>$cash_info['pdc_id']));
				
			} else {			
				 showMessage('支付宝转账失败:'.$trans_data['sub_msg'].'['.$trans_data['code'].']',urlAdminShop('predeposit','pd_cash_list'),'','error');
				Log::record('---会员账户提现--支付宝 ==失败--- '.json_encode($trans_data));
    ...
    public function export_step1Wt(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('murl',urlAdminShop('predeposit','predeposit'));
				Tpl::setDirquna('shop');
    ...
    public function export_cash_step1Wt(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('list',$array);
                Tpl::output('murl',urlAdminShop('predeposit','pd_cash_list'));
				Tpl::setDirquna('shop');
                Tpl::showpage('export.excel');
            }else{  //如果数量小，直接下载
    ...
    public function export_mx_step1Wt(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('list',$array);
                Tpl::output('murl',urlAdminShop('predeposit','pd_log_list'));
				Tpl::setDirquna('shop');
                Tpl::showpage('export.excel');
            }else{  //如果数量小，直接下载
    ...
    public function get_xmlWt() {
        ...
        foreach ($recharge_list as $value) {
            ...
            $operation .= "<a class='btn green' href='javascript:void(0)' onclick=\"ajax_form('recharge_info','查看充值编号“". $value['pdr_sn'] ."”的明细','".urlAdminShop('predeposit','recharge_info',array('id'=>$value['pdr_id']))."', '640')\"><i class='fa fa-list-alt'></i>查看</a>";
    ...
    public function get_cash_xmlWt() {
        ...
        foreach ($cash_list as $value) {
            ...
            $param['operation'] .= "<a class='btn green' href='javascript:void(0)' onclick=\"ajax_form('cash_info','查看提现编号“". $value['pdc_sn'] ."”的明细', '".urlAdminShop('predeposit','pd_cash_view',array('id'=>$value['pdc_id']))."', 640)\" ><i class='fa fa-list-alt'></i>查看</a>";
