...
class help_storeControl extends SystemControl{
    ...
    public function get_xmlWt() {
        ...
        foreach ($help_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn red' href=\"javascript:void(0);\" onclick=\"fg_del('".$value['help_id']."')\"><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='".urlAdminShop('help_store','edit_help',array('help_id'=>$value['help_id']))."' class='url'><i class='fa fa-pencil-square-o'></i>编辑</a>";
    ...
    public function get_type_xmlWt() {
        ...
        foreach ($type_list as $value) {
            ...
            $operation .= "<a class='btn blue' href='".urlAdminShop('help_store','edit_type',array('type_id'=>$value['type_id']))."' class='url'><i class='fa fa-pencil-square-o'></i>编辑</a>";
    ...
    public function add_helpWt() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('wt_common_save_succ'),urlAdminShop('help_store','help_store'));
            } else {
    ...
    public function add_typeWt() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('wt_common_save_succ'),urlAdminShop('help_store','help_type'));
            } else {
                showMessage(Language::get('wt_common_save_fail'));
            }
        }
