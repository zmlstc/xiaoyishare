...
class couponControl extends SystemControl{
    ...
    public function __construct(){
        parent::__construct();
        if (C('coupon_allow') != 1){
            showDialog('需开启“平台优惠券”功能',urlAdminShop('operation'),'succ');
        }
        ...
        if (chksubmit()){
            ...
            if ($error){
                ...
                if($rs){
                    ...
                    showDialog(L('wt_common_save_succ'),urlAdminShop('coupon','rptlist'),'succ');
                }else{
    ...
    public function rpteditWt(){
        ...
        if ($t_id <= 0){
            showDialog(L('param_error'),urlAdminShop('coupon','rptlist'));
        }
        $model_coupon = Model('coupon');
        //查询模板信息
        $where = array();
    ...
    public function export_step1Wt(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){//显示下载链接
                ...
                Tpl::output('murl',urlAdminShop('coupon','rptinfo',array('tid'=>$t_id)));
				Tpl::setDirquna('shop');
