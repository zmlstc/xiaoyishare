...
class storeControl extends SystemControl{
    ...
    private $_links = array(
       ...
       // array('url'=>'store/reopen_list','text'=>'续签申请'),
        array('url'=>'store/sname_modify','text'=>'店铺名申请')
        //array('url'=>'store/store_bind_class_applay_list','text'=>'经营类目申请')
    );
...
/*         $model_store_class = Model('store_class');
        ...
        foreach ($store_list as $value) {
            ...
            $operation = "<a class='btn green' href='".urlAdminShop('store','store_joinin_detail',array('sj_id'=>$value['join_id']))."'><i class='fa fa-list-alt'></i>查看</a><span class='btn'><em><i class='fa fa-cog'></i>" . L('wt_set') . " <i class='arrow'></i></em><ul><li><a href='".urlAdminShop('store','store_edit',array('store_id'=>$value['store_id']))."'>编辑店铺信息</a></li><li><a href='".urlAdminShop('store','del',array('store_id'=>$value['store_id']))."'>删除该店铺</a></li>";
          /*   if (str_cut($store_state, 6) == 'expire'  && cookie('remindRenewal'.$value['store_id']) == null) {
            ...
            $store_name = "<a class='open' href='". urlShop('show_store', 'index', array('store_id' => $value['store_id'])) ."' target='blank'>";
           /*  if ($store_state == 'expired') {
            ...
            $param['isshow'] = $value['isshow']?'是':'否';
            $param['scinfo'] = $value['scinfo'];
     ...
     * 店铺名称修改申请
     */
    public function sname_modifyWt(){
        //店铺等级
        /* $model_grade = Model('store_grade');
        $grade_list = $model_grade->getGradeList(array());
        Tpl::output('grade_list', $grade_list); */
        ...
        //输出子菜单
        Tpl::output('top_link',$this->sublink($this->_links,'sname_modify'));
		Tpl::setDirquna('shop');

        Tpl::showpage('sname_modify.index');
    }	
	
    /**
     * 输出XML数据
     */
    public function get_sname_xmlWt() {
        $model_store = Model('store_modify');
        // 设置页码参数名称
        $condition = array();
        ...
        //店铺列表
        $store_list = $model_store->getList($condition, $page);
        ...
        foreach ($store_list as $value) {
            ...
            if(intval($value['state'])==0) {
                $operation = "<a class='btn orange' href=\"".urlAdminShop('store','sname_detail',array('id'=>$value['id']))."\"><i class=\"fa fa-check-bbs\"></i>审核</a>";
            } else {
                $operation = "<a class='btn green' href=\"".urlAdminShop('store','sname_detail',array('id'=>$value['id']))."\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }
            $param['operation'] = $operation;
			$param['store_name'] = $value['store_name'];
            $param['store_name_old'] = $value['store_name_old'];
            $param['state'] = $value['state']==0?'新申请':($value['state']==1?'审核通过':'审核失败');
            $param['addtime'] = date('Y-m-d', $value['addtime']);
            $param['verify_time'] =$value['verify_time']==0?'':date('Y-m-d', $value['verify_time']);
            $param['curr_num'] = $value['curr_num'];
     ...
     * 店铺名称审核详细页
     */
    public function sname_detailWt(){
        $model_store = Model('store_modify');
        $detail = $model_store->getInfo(array('id'=>$_GET['id']));
        /* $detail_title = '查看';
        if(in_array(intval($joinin_detail['joinin_state']), array(STORE_JOIN_STATE_NEW, STORE_JOIN_STATE_PAY))) {
            $detail_title = '审核';
        }

        Tpl::output('detail_title', $detail_title); */
        Tpl::output('detail', $detail);
	
		Tpl::setDirquna('shop');
		

		Tpl::showpage('sname_modify.detail');
		
    }
    /**
     * 店铺名称审核
     */
    public function sname_verifyWt() {
        $model_store_modify = Model('store_modify');
        $detail = $model_store_modify->getInfo(array('id'=>$_POST['id']));
		if(empty($detail)||$detail['state']!=0){
            showMessage('参数错误','');
        }
        $model_store    = Model('store');
        if($_POST['verify_type'] === 'pass') {
			$storeinfo = $model_store->getStoreInfo(array('store_name'=>$detail['store_name']));

			//验证商家账号是否已经存在
			if(!empty($storeinfo)&&is_array($storeinfo)&&$storeinfo['store_id']!=$detail['store_id']) {
				showMessage('该店铺名称已存在','');
			}
	    }

		$update     = array();
		$update['verify_time']     = time();
		$update['state']   = $_POST['verify_type'] === 'pass' ? 1 : 2;
        
		$state = $model_store_modify->edit($update,array('id'=>intval($_POST['id'])));
        if($_POST['verify_type'] === 'pass') {
            if($state) {
				$model_store->editStore(array('store_name'=>$detail['store_name']),array('store_id'=>$detail['store_id']));
                showMessage('审核成功',urlAdminShop('store','sname_modify'));
            } else {
                showMessage('审核失败',urlAdminShop('store','sname_modify'));
            }
        } else {
            showMessage('审核拒绝通过',urlAdminShop('store','sname_modify'));
        }
    }
    ...
    public function export_csvWt() {
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('murl',urlAdminShop('store'));
				Tpl::setDirquna('shop');
    ...
    public function store_editWt(){
        ...
        //保存
        if (chksubmit()){
			if(floatval($_POST['store_rate'])>20){
				 showMessage('店铺佣金比列不能大于20%');
			}
            //取店铺等级的审核
          /*   $model_grade = Model('store_grade');
	    ...
	    //编辑店铺地址 公司名称
            ...
            $city_id = intval($_POST['store_city_id']);
            $area_id = intval($_POST['store_area_id']);
            if ($province_id > 0) {
                $update_array['province_id'] = $province_id;
            }
            if ($city_id > 0) {
                $update_array['city_id'] = $city_id;
            }
            if ($area_id > 0) {
                $update_array['area_id'] = $area_id;
            }
			if(intval($_POST['scid1'])>0){
				$update_array['scid1'] = intval($_POST['scid1']);
				$update_array['scid2'] = intval($_POST['scid2']);
			}
            $update_array['store_rate'] = floatval($_POST['store_rate']);
            ...
            if ($result){
                $url = array(
                array(
                'url'=>urlAdminShop('store','store'),
                'msg'=>$lang['back_store_list'],
                ),
                array(
                'url'=>urlAdminShop('store','store_edit',array('store_id'=>intval($_POST['store_id']))),
		...
		$default_rate = '';
		if(!empty($store_array['scid1'])&&$store_array['scid1']>0){
			$sc_info1=$model_store_class->getStoreClassInfo(array('sc_id'=>$store_array['scid1']));
			$showsc_info = $sc_info1['sc_name'];
			$default_rate = $sc_info1['commis_rate'];
		}
		if(!empty($store_array['scid2'])&&$store_array['scid2']>0){
			$sc_info2=$model_store_class->getStoreClassInfo(array('sc_id'=>$store_array['scid2']));
			$showsc_info = $showsc_info.' -> '. $sc_info2['sc_name'];
			$default_rate = $sc_info2['commis_rate'];
		}
		Tpl::output('showsc_info',$showsc_info.'(佣金比列:'.$default_rate.'%)');
        //店铺等级
        /* $model_grade = Model('store_grade');
    ...
    public function edit_save_joininWt() {
        if (chksubmit()) {
            ...
            if ($result) {
                showMessage(L('wt_common_op_succ'), urlAdminShop('store','store'));
            } else {
    ...
    public function wtshop_addWt()
    {
        if (chksubmit())
        {
            ...
            if (!$this->checkMemberName($memberName))
                showMessage('会员账号已被占用', '', 'html', 'error');
            ...
            {
                showMessage('会员账号新增失败', '', 'html', 'error');
            }
            ...
            showMessage('操作成功',urlAdminShop('store','store'));
            return;
        }
		Tpl::setDirquna('shop');
    ...
    public function get_joinin_xmlWt() {
        ...
        foreach ($store_list as $value) {
            ...
            if(in_array(intval($value['joinin_state']), array(STORE_JOIN_STATE_NEW, STORE_JOIN_STATE_PAY))) {
                $operation = "<a class='btn orange' href=\"".urlAdminShop('store','store_joinin_detail',array('sj_id'=>$value['sj_id']))."\"><i class=\"fa fa-check-bbs\"></i>审核</a>";
            } else {
                $operation = "<a class='btn green' href=\"".urlAdminShop('store','store_joinin_detail',array('sj_id'=>$value['sj_id']))."\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }
            $param['operation'] = $operation;
    ...
    public function get_bind_class_applay_xmlWt() {
        ...
        foreach ($store_bind_class_list as $value) {
            $param = array();
            if($value['state'] == '0') {
                $operation = "<a class='btn orange' href=\"javascript:if(confirm('确认审核吗？'))window.location = '".urlAdminShop('store','store_bind_class_applay_check',array('bid'=>$value['bid'],'store_id'=>$value['store_id']))."'\"><i class=\"fa fa-check-bbs\"></i>审核</a>";
            } else {
                $operation = "<a class='btn red' href=\"javascript:if(confirm('".($value['state'] == '1' ? '该类目已经审核通过，删除它可能影响到商家的使用，' : null)."确认删除吗？'))window.location = '".urlAdminShop('store','store_bind_class_applay_del',array('bid'=>$value['bid'],'store_id'=>$value['store_id']))."'\"><i class=\"fa fa-trash-o\"></i>删除</a>";
            }
            $param['operation'] = $operation;
            $param['store_id'] = $value['store_id'];
    ...
    public function store_joinin_verifyWt() {
        $model_store_joinin = Model('store_joinin');
        $joinin_detail = $model_store_joinin->getOne(array('sj_id'=>$_POST['sj_id']));
    ...
    private function store_joinin_verify_pass($joinin_detail) {
        ...
        $param['paying_amount'] = 0;//abs(floatval($_POST['paying_amount']));
        ...
        if ($param['paying_amount'] > 0) {
            showMessage('店铺入驻申请审核完成',urlAdminShop('store','store_joinin'));
        } else {
    ...
    private function store_joinin_verify_open($joinin_detail,$paying_amount = 0) {
        ...
        if($_POST['verify_type'] === 'pass') {
			...
			$shop_array['shop_phone']     = $joinin_detail['shop_phone'];
            ...
            if($state) {
				...
				//发送用户消息
				$param = array();
				$param['code'] = 'inviter_store_reg';
				$param['member_id'] = $shop_array['invite_bm'];
				$param['param'] = array();
                ...
                showMessage('店铺开店成功',urlAdminShop('store','store_joinin'));
            } else {
                showMessage('店铺开店失败',urlAdminShop('store','store_joinin'));
            }
        } else {
            showMessage('店铺开店拒绝',urlAdminShop('store','store_joinin'));
        }
    }
     ...
     * 验证店铺ID是否存在
     */
    public function ckeck_storeidWt() {
        /**
         * 实例化商家模型
         */
        $where = array();
