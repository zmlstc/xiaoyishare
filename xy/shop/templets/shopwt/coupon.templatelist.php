...
<script>
$(function(){
    var flexUrl = '<?php echo urlAdminShop('coupon','rptlist_xml');?>';
...
function fg_operate(name, bDiv) {
	if (name == 'add') {
		window.location.href = '<?php echo urlAdminShop('coupon','rptadd');?>';
    }
}
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除吗？')){
        $.getJSON('<?php echo urlAdminShop('coupon','rptdel');?>', {tid:id}, function(data){
            if (data.state) {
