...
$(function(){
    $("#flexigrid").flexigrid({
        url: '<?php echo urlAdminShop('store','get_xml');?>',
        colModel : [
            ...
            //{display: '会员账号', name : 'member_id', width : 120, sortable : true, align: 'left'},
           ...
           // {display: '店铺等级', name : 'grade_id', width : 80, sortable : true, align: 'center'},
            ...
            {display: '是否显示', name : 'isshow', width : 60, sortable : true, align: 'center'},
    ...
    $('#wtsubmit').click(function(){
        $("#flexigrid").flexOptions({url: '<?php echo urlAdminShop('store','get_xml');?>?'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });

    // 高级搜索重置
    $('#wtreset').click(function(){
        $("#flexigrid").flexOptions({url: '<?php echo urlAdminShop('store','get_xml');?>'}).flexReload();
        $("#formSearch")[0].reset();
    });
});
...
function fg_operations(name, bDiv) {
    if (name == 'wtshop_add') {
        window.location.href = '<?php echo urlAdminShop('store','wtshop_add');?>';
    }
}
