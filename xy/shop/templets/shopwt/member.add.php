		...
		(
			{
				...
				{
					if (data.status == 1){
						ajax_form('cutpic','<?php echo $lang['wt_cut'];?>','<?php echo urlAdmin('common','pic_cut',array('type'=>'member'));?>?x=120&y=120&resize=1&ratio=1&url='+data.url,690);
					}else{
						alert(data.msg);
					}
    ...
    $('#user_form').validate({
        ...
        rules : {
			member_name: {
				...
				maxlength: 20,
				remote   : {
                    url :'<?php echo urlAdminShop('member','ajax',array('branch'=>'check_user_name'));?>',
                    type:'get',
                    data:{
                        user_name : function(){
