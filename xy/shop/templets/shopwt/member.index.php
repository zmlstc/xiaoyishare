...
$(function(){
    $("#flexigrid").flexigrid({
        url: '<?php echo urlAdminShop('member','get_xml');?>',
        colModel : [
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = '<?php echo urlAdminShop('member','member_add');?>';
    }
    if (name == 'csv') {
...
function submit_delete(id){
	...
	if(confirm('确认删除这 ' + id.length + ' 项吗？')){
		id = id.join(',');
        window.location.href = '<?php echo urlAdminShop('member','member_del');?>?member_id='+id;
    }
}
