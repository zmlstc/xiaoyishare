  ...
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base wt-row">
        ...
        <li><a href="<?php echo urlAdminShop('chat_log','msg_log');?>">聊天内容</a></li>
      </ul>
    </div>
  </div>
  <!-- 操作说明 -->
...
$(function(){
    ...
    $('#wtsubmit').click(function(){
        $("#flexigrid").flexOptions({url: '<?php echo urlAdminShop('chat_log','get_xml');?>?'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });

    // 高级搜索重置
    $('#wtreset').click(function(){
        $("#flexigrid").flexOptions({url: '<?php echo urlAdminShop('chat_log','get_xml');?>'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
        url: '<?php echo urlAdminShop('chat_log','get_xml');?>?'+$("#formSearch").serialize(),
        colModel : [
