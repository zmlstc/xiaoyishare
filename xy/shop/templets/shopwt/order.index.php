...
$(function(){
    ...
    $('#wtsubmit').click(function(){
        $("#flexigrid").flexOptions({url: '<?php echo urlAdminShop('order','get_xml');?>?'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#wtreset').click(function(){
        $("#flexigrid").flexOptions({url: '<?php echo urlAdminShop('order','get_xml');?>'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
        url: '<?php echo urlAdminShop('order','get_xml');?>',
        colModel : [
			...
			{display: '订单金额(元)', name : 'order_amount', width : 80, sortable : true, align: 'left'},
			{display: '订单状态', name : 'order_state', width: 60, sortable : true, align : 'center'},   
			{display: '支付方式', name : 'payment_code', width: 80, sortable : true, align : 'center'},
...
function fg_cancel(id) {
	...
	$.ajax({
        type: "GET",
        dataType: "json",
        url: "<?php echo urlAdminShop('order','change_state',array('state_type'=>'cancel'));?>",
        data: "order_id="+id,
