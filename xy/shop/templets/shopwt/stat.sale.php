...
function update_flex(){
	$('.wtap-stat-general-single').load('<?php echo urlAdminShop('stat_trade','get_plat_sale');?>?'+$("#formSearch").serialize(),
		function(){
			$('.timer').each(count);
     	});
    $("#flexigrid").flexigrid({
        url: '<?php echo urlAdminShop('stat_trade','get_sale_xml');?>?'+$("#formSearch").serialize(),
        colModel : [
...
function getStatdata(type){
	$('#'+type+'_div').load('<?php echo urlAdminShop('stat_trade','sale_trend');?>?type='+type+'&'+$("#formSearch").serialize());
}
function fg_operation(name, bDiv){
    var stat_url = '<?php echo urlAdminShop('stat_trade','sale');?>';
