...
$(function(){
    ...
    $('#wtsubmit').click(function(){
        $("#flexigrid").flexOptions({url: '<?php echo urlAdminShop('store','get_bind_class_applay_xml');?>?'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });

    // 高级搜索重置
    $('#wtreset').click(function(){
        $("#flexigrid").flexOptions({url: '<?php echo urlAdminShop('store','get_bind_class_applay_xml');?>'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
        url: '<?php echo urlAdminShop('store','get_bind_class_applay_xml');?>',
        colModel : [
            ...
            {display: '会员账号', name : 'seller_name', width : 80, sortable : true, align: 'left'},
