...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="<?php echo urlAdminShop('coupon','rptlist');?>" title="返回列表"><i class="fa fa-arrow-bbs-o-left"></i></a>
      <div class="subject">
        <h3>平台优惠券 - 优惠券详情</h3>
        <h5>平台优惠券新增与管理</h5>
      </div>
    ...
    <div class="wtap-form-default">
      ...
      <dl class="row">
        ...
        <dd class="opt">
            <input type="text" onclick="oCopy(this)" value="<?php echo urlShop('coupon','getcoupon',array('tid'=>$output['t_info']['coupon_t_id']));?>" readonly class='w350'/>
<script>
	function oCopy(obj){
		obj.select();
    ...
    <div class="wtap-form-default">
        <div style="text-align: center;"><a href="<?php echo urlAdminShop('coupon','rptlist');?>" class="wtap-btn-big wtap-btn-green" id="submitBtn">返回列表</a></div>
    </div>
</div>
<script>
$(function(){
    var flexUrl = '<?php echo urlAdminShop('coupon','rplist_xml',array('tid'=>$output['t_info']['coupon_t_id'],'gtype'=>$output['t_info']['coupon_t_gettype_key']));?>';
...
function fg_csv(ids) {
    id = ids.join(',');
    window.location.href = '<?php echo urlAdminShop('coupon','export_step1',array('tid'=>$output['t_info']['coupon_t_id']));?>?rid=' + id;
}
</script>