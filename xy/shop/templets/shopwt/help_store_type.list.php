  ...
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base wt-row">
        <li><a href="<?php echo urlAdminShop('help_store','help_store');?>"><?php echo '帮助内容';?></a></li>
...
$(function(){
    $("#flexigrid").flexigrid({
        url: '<?php echo urlAdminShop('help_store','get_type_xml');?>',
        colModel : [
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = '<?php echo urlAdminShop('help_store','add_type');?>';
    }
}

function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('<?php echo urlAdminShop('help_store','del_type');?>', {id:id}, function(data){
            if (data.state) {
