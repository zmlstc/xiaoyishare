...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="<?php echo urlAdminShop('sns_strace');?>" title="返回动态列表"><i class="fa fa-arrow-bbs-o-left"></i></a>
      <div class="subject">
...
$(function(){
    $("#flexigrid").flexigrid({
        url: '<?php echo urlAdminShop('sns_strace','get_scomm_xml',array('id'=>$_GET['st_id']));?>',
        colModel : [
...
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('<?php echo urlAdminShop('sns_strace','scomm_del');?>', {id:id}, function(data){
            if (data.state) {
