  ...
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base wt-row">
        ...
        <li><a href="<?php echo urlAdminShop('predeposit','pd_cash_list');?>"><?php echo $lang['admin_predeposit_cashmanage']; ?></a></li>
...
$(function(){
    ...
    $('#wtsubmit').click(function(){
        $("#flexigrid").flexOptions({url: '<?php echo urlAdminShop('predeposit','get_xml');?>?'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#wtreset').click(function(){
        $("#flexigrid").flexOptions({url: '<?php echo urlAdminShop('predeposit','get_xml');?>'}).flexReload();
        $("#formSearch")[0].reset();
    });
    
    $("#flexigrid").flexigrid({
        url: '<?php echo urlAdminShop('predeposit','get_xml');?>',
        colModel : [
...
function fg_delete(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('<?php echo urlAdminShop('predeposit','recharge_del');?>', {id:id}, function(data){
            if (data.state) {
