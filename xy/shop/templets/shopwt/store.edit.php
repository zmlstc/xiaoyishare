...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="<?php echo urlAdminShop('store','store');?>" title="返回<?php echo $lang['manage'];?>列表"><i class="fa fa-arrow-bbs-o-left"></i></a>
      <div class="subject">
    ...
    <form id="store_form" method="post">
      ...
      <dl class="row">
        ...
        <dd class="opt">
          ...
          <input type="hidden" value="<?php echo $output['store_array']['city_id']; ?>" name="store_city_id" id="store_city_id">
          <input type="hidden" value="<?php echo $output['store_array']['area_id']; ?>" name="store_area_id" id="store_area_id">
          <span class="err"></span>
        </dd>
      </dl>
      <dl class="row">
        ...
        <dl class="row">
          ...
          <dd class="opt">
			...
			<div id="showsc_info"><?php echo $output['showsc_info'];?></div>
			<a href="javascript:;" id="btn_select_category" class="btn">编辑</a>
			<div id="gcategory" style="display:none;">
              <select id="gcategory_class1" name="scid1">
                <option value="0">请选择</option>
                <?php if(!empty($output['class_list']) && is_array($output['class_list']) ) {?>
                <?php foreach ($output['class_list'] as $gc) {?>
                <option value="<?php echo $gc['sc_id'];?>" data-explain="<?php echo $gc['commis_rate'];?>"><?php echo $gc['sc_name'];?></option>
                <?php }?>
                <?php }?>
              </select>
			  <div id="showrate"></div>
      ...
      <dl class="row">
        <dt class="tit">
          <label for="store_rate">佣金比列:</label>
        </dt>
        <dd class="opt">
          <input type="text" value="<?php echo $output['store_array']['store_rate'];?>" id="store_rate" name="store_rate" class="input-txt" />%
          <span class="err"></span>
		   <p class="notic">该值大于0有效,为0时默认读取店铺分类的佣金比列 </p>
        </dd>
      </dl>
        <!--dl class="row">
          <dt class="tit">
...
<?php if(!($output['store_zizhi'])){?>
    <form id="joinin_form" enctype="multipart/form-data" method="post" action="<?php echo urlAdminShop('store','edit_save_joinin');?>" style="display:none;">
      <input type="hidden" name="form_submit" value="ok" />
      <input type="hidden" name="sj_id" value="<?php echo $output['joinin_detail']['sj_id'];?>" />
...
<script type="text/javascript" src="<?php echo STATIC_SITE_URL;?>/js/storeclass_select.js" charset="utf-8"></script>
...
$(function(){
    ...
    $("#submitBtn").click(function(){
        if($("#store_form").valid()){
            ...
            $('#store_city_id').val($("#store_region").fetch('area_id_2'));
            $('#store_area_id').val($("#store_region").fetch('area_id_3'));
            $("#store_form").submit();
        }
    });
