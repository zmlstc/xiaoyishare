  ...
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base wt-row">
        ...
        <li><a href="<?php echo urlAdminShop('help_store','help_type');?>"><?php echo '帮助类型';?></a></li>
      </ul>
    </div>
  </div>
  <!-- 操作说明 -->
...
$(function(){
    $("#flexigrid").flexigrid({
        url: '<?php echo urlAdminShop('help_store','get_xml');?>',
        colModel : [
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = '<?php echo urlAdminShop('help_store','add_help');?>';
    }
}
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('<?php echo urlAdminShop('help_store','del_help');?>', {id:id}, function(data){
            if (data.state) {
