...
$(function(){
    $("#flexigrid").flexigrid({
        url: '<?php echo urlAdminMobile('mb_feedback','get_xml');?>',
        colModel : [
            ...
            {display: '反馈类型', name : 'fbclass', width : 60, sortable : true, align: 'left'},
            {display: '联系方式', name : 'lxway', width : 60, sortable : true, align: 'left'}
            ],
        buttons : [
...
function fg_del(ids) {
    ...
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('<?php echo urlAdminMobile('mb_feedback','del');?>', {id:id}, function(data){
            if (data.state) {
