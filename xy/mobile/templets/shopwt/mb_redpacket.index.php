  ...
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base">
        ...
        <li><a href="<?php echo urlAdminMobile('mb_redpacket','new');?>" ><span>新增</span></a></li>
      </ul>
    </div>
  </div>
    ...
    <table class="table tb-type2">
      ...
      <tbody id="treet1">
		...
		<td><a href="<?php echo WAP_SITE_URL; ?>/html/red_packet.html?id=<?php echo $v['id']; ?>" target="_blank"><?php echo WAP_SITE_URL; ?>/html/red_packet.html?id=<?php echo $v['id']; ?></a></td>
		  ...
		  <td class="align-center"><a href="<?php echo urlAdminMobile('mb_redpacket','uselist',array('id'=>$v['id']));?>"><?php echo $v['packet_numbered'];?></a></td>
		 
		  <td class="align-center"><?php echo $v['win_rate'];?>%</td>
		  <?php if(!empty($v['valid_date'])){echo date('Y-m-d H:i:s',$v['valid_date']);}else{if($v['valid_date2']==1){echo '自领取后当天内有效';}else{echo '自领取后'.$v['valid_date2'].'天内有效';}}?></td-->
          ...
          <td class="align-center">
		    <a href="<?php echo urlAdminMobile('mb_redpacket','list',array('id'=>$v['id']));?>">查看</a> | 
          	<a href="<?php echo urlAdminMobile('mb_redpacket','edit',array('id'=>$v['id']));?>">编辑</a> |
			<?php if($v['state']==1){ ?>
			<a href="javascript:if(confirm('确定关闭?'))window.location.href='<?php echo urlAdminMobile('mb_redpacket','state',array('id'=>$v['id'],'type'=>'close'));?>'">关闭</a>
			<?php }else{ ?>
			<a href="javascript:if(confirm('确定开启?'))window.location.href='<?php echo urlAdminMobile('mb_redpacket','state',array('id'=>$v['id'],'type'=>'open'));?>'">开启</a>
			<?php } ?>
			|
			<?php if($v['state']==1 && $v['start_time']<time() && $v['end_time']>time()){ ?>
			<a href="javascript:;"><span style="color:#666;">删除</span></a>
			<?php }else{ ?>
			<a href="javascript:if(confirm('确定删除?'))window.location.href='<?php echo urlAdminMobile('mb_redpacket','del',array('id'=>$v['id']));?>'">删除</a>
			<?php } ?>
          </td>
        </tr>
        <?php } ?>
