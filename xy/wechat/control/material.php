...
class materialControl extends SystemControl{
    ...
    public function material_manageWt() {
        ...
        $links = array(
			array('url'=>'material/material_manage','lang'=>'material_all'),
			array('url'=>'material/material_edit','lang'=>'material_add')
		);
    ...
    public function material_editWt() {
		...
		if(chksubmit()){
			...
			if ($result){
				showMessage('保存成功',urlAdminWechat($_GET['w'],'material_manage'));
			}else {
				showMessage('保存失败',urlAdminWechat($_GET['w'],'material_manage'));
			}
		}else{
			if(!empty($_GET['mid'])){
	...
	public function material_delWt(){
		$lang   = Language::getLangContent();
		if(empty($_GET['mid'])){
			showMessage('参数错误！',urlAdminWechat($_GET['w'],'material_manage'));
		}
		...
		if (intval($_GET['mid']) > 0){
			...
			if(empty($material_info)){
				showMessage($lang['info_not_exist'],urlAdminWechat($_GET['w'],'material_manage'));
			}
			...
			showMessage('删除成功',urlAdminWechat($_GET['w'],'material_manage'));
		}else {
			showMessage('删除失败',urlAdminWechat($_GET['w'],'material_manage'));
		}
	}
