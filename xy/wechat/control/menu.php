...
class menuControl extends SystemControl{
    ...
    public function menu_addWt() {
		...
		if (chksubmit()){
			...
			if($flag){
				$model_wechat->commit();
				showMessage('保存成功！',urlAdminWechat($_GET['w'],'menu_manage'));
			}else{
    ...
    public function menu_delWt() {
		...
		if(empty($_GET['mid']) && empty($_POST['mid'])){
			showMessage($lang['not_info_id'],urlAdminWechat($_GET['w'],'menu_manage'));
		}
		...
		if(!empty($menu_info)){
			...
			if($response['status']==0){
				showMessage($lang[$response['msg']],urlAdminWechat($_GET['w'],'menu_manage'));
			}else{
				showMessage($lang[$response['msg']],urlAdminWechat($_GET['w'],'menu_manage'));
			}
		}else{
			if($result){
				showMessage('删除成功',urlAdminWechat($_GET['w'],'menu_manage'));
			}else{
				showMessage('删除失败！',urlAdminWechat($_GET['w'],'menu_manage'));
			}
		}
    }
    ...
    public function menu_editWt() {
		...
		if (chksubmit()){
			...
			if(empty($menu_info)){
				showMessage('菜单信息不存在！',urlAdminWechat($_GET['w'],'menu_manage'));
			}
			...
			if($flag){
				...
				if($menu_info['menu_status']==1){
					...
					if($response['status']==1){
						showMessage('同步中，请到微信公众号查看',urlAdminWechat($_GET['w'],'menu_manage'));
					}else{
						showMessage('有个别菜单动作为空，请检查！',urlAdminWechat($_GET['w'],'menu_manage'));
					}
				}else{
					showMessage('保存成功！',urlAdminWechat($_GET['w'],'menu_manage'));
				}				
			...
			if(empty($menu_info)){
				showMessage($lang['info_not_exist'],urlAdminWechat($_GET['w'],'menu_manage'));
			}
	...
	public function menu_publishWt(){
		...
		if($response['status']==0){
			showMessage('同步数据失败！',urlAdminWechat($_GET['w'],'menu_manage'));
		}else{
			showMessage('同步数据成功！',urlAdminWechat($_GET['w'],'menu_manage'));
		}
	}
