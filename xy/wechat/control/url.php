...
class urlControl extends SystemControl{
    ...
    public function url_addWt() {
		...
		if (chksubmit()){
			...
			if ($result){
				showMessage('操作成功！',urlAdminWechat($_GET['w'],'url_manage'));
			}else {
				showMessage('操作失败！');
			}
		}else{
    ...
    public function url_editWt() {
		...
		if (chksubmit()){
			if(empty($_POST['urlid'])){
				showMessage('参数错误！',urlAdminWechat($_GET['w'],'url_manage'));
			}
			...
			if ($result === true){
				...
				showMessage('操作失败！',urlAdminWechat($_GET['w'],'url_manage'));
			}
		}else{
			$url_info = array();
			if(empty($_GET['urlid'])){
			...
			if(empty($url_info)){
				showMessage('记录不存在',urlAdminWechat($_GET['w'],'url_manage'));
			}
    ...
    public function url_delWt() {
		...
		if($result){
			showMessage('删除成功！',urlAdminWechat($_GET['w'],'url_manage'));
		}else{
			showMessage('删除失败！',urlAdminWechat($_GET['w'],'url_manage'));
		}
    }
