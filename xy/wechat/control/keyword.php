...
class keywordControl extends SystemControl{
    ...
    public function keyword_addWt() {
		...
		if (chksubmit()){
			...
			if ($result){
				showMessage('保存成功！',urlAdminWechat($_GET['w'],'keyword_manage'));
			}else {
				showMessage('保存失败！');
			}
		}else{
    ...
    public function keyword_editWt() {
		...
		if (chksubmit()){
			if(empty($_POST['rid'])){
				showMessage('参数错误！',urlAdminWechat($_GET['w'],'keyword_manage'));
			}
			...
			if ($result === true){
				...
				showMessage('保存失败！',urlAdminWechat($_GET['w'],'keyword_manage'));
			}
		}else{
			...
			if(empty($reply_info)){
				showMessage('记录不存在',urlAdminWechat($_GET['w'],'keyword_manage'));
			}
    ...
    public function keyword_delWt() {
		...
		if($result){
			showMessage('删除成功！',urlAdminWechat($_GET['w'],'keyword_manage'));
		}else{
			showMessage('删除失败！',urlAdminWechat($_GET['w'],'keyword_manage'));
		}
    }
