...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="<?php echo urlAdminSystem('area');?>" title="返回列表"><i class="fa fa-arrow-bbs-o-left"></i></a>
      <div class="subject">
        <h3>地区管理 - 新增</h3>
        <h5>地区新增与编辑</h5>
      </div>
  ...
  <form id="form" method="post" action="<?php echo urlAdminSystem('area','save',array('area_id'=>$_GET['area_id']));?>">
    ...
    <div class="wtap-form-default">
      <dl class="row">
        ...
        <dd class="opt">
        <?php echo $output['info']['area_name']?>
          <span class="err"></span>
          <p class="notic"></p>
        </dd>
      </dl>
      <dl class="row">
        <dt class="tit">
          <label for="ac_name"><em>*</em>简称</label>
        </dt>
        <dd class="opt">
          <input type="text" name="shortname" value="<?php echo $output['info']['shortname']?>" maxlength="10" id="shortname" class="input-txt">
          <span class="err"></span>
          <p class="notic"></p>
        </dd>
      </dl>
...
$(document).ready(function(){
	...
	$('#form').validate({
        ...
        messages : {
        	shortname : {
                required : '<i class="fa fa-exclamation-bbs"></i>请填写地区简称'
            }
        }
    });
});
