...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="<?php echo urlAdminSystem('show');?>" title="返回列表"><i class="fa fa-arrow-bbs-o-left"></i></a>
      <div class="subject">
...
$(function(){
    $("#flexigrid").flexigrid({
        url: '<?php echo urlAdminSystem('show','get_show_xml',array('ap_id'=>$_GET['ap_id']));?>',
        colModel : [
            {display: '操作', name : 'operation', width : 150, sortable : false, align: 'center', className: 'handle'},
            {display: '广告名称', name : 'show_title', width : 200, sortable : false, align: 'left'}, 
			{display: '所在广告位', name : 'ap_id', width : 200, sortable : true, align : 'left'},           
			{display: '类型', name : 'ap_class', width : 80, sortable : false, align: 'left'},
...
function fg_delete(id){
	...
	$.ajax({
        type: "GET",
        dataType: "json",
        url: "<?php echo urlAdminSystem('show','show_delete');?>",
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = '<?php echo urlAdminSystem('show','show_add',array('ap_id'=>$_GET['ap_id']));?>';
    }else if (name == 'delete') { 
