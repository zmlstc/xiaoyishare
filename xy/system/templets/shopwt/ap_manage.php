...
$(function(){
    $("#flexigrid").flexigrid({
        url: '<?php echo urlAdminSystem('show','get_ap_xml');?>',
        colModel : [
			...
			{display: '展示方式', name : 'ap_display', width : 80, sortable : true, align: 'left'},
			{display: '宽度/字数', name : 'ap_width', width : 80, sortable : true, align: 'left'},
...
function fg_delete(id){
	...
	$.ajax({
        type: "GET",
        dataType: "json",
        url: "<?php echo urlAdminSystem('show','delete');?>",
        data: "del_id="+id,
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = '<?php echo urlAdminSystem('show','ap_add');?>';
    }else if (name == 'delete') { 
...
{
   ajax_form('copy_show', '<?php echo $lang['ap_get_js'];?>', '<?php echo urlAdminSystem('show','ap_copy');?>?id='+id);
}
</script> 
