...
$(function(){
    ...
    $('#wtsubmit').click(function(){
        $("#flexigrid").flexOptions({url: '<?php echo urlAdminSystem('admin_log','get_xml');?>?'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#wtreset').click(function(){
        $("#flexigrid").flexOptions({url: '<?php echo urlAdminSystem('admin_log','get_xml');?>'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
        url: '<?php echo urlAdminSystem('admin_log','get_xml');?>',
        colModel : [
    ...
    if (name == 'csv') {
    	...
    	if(confirm('删除后将不能恢复，确认删除吗？')){
    		$.ajax({
    	        type: "GET",
    	        dataType: "json",
    	        url: "<?php echo urlAdminSystem('admin_log','list_del');?>",
    	        data: "type=ago",
