...
$(function(){
    $("#flexigrid").flexigrid({
        url: '<?php echo urlAdminSystem('admin','get_admin_xml');?>',
        colModel : [
...
function fg_operation(name, grid) {
    if (name == 'add') {
        window.location.href = '<?php echo urlAdminSystem('admin','admin_add');?>';
    }
}
function fg_operation_del(admin_id){
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        var _url = '<?php echo urlAdminSystem('admin','admin_del');?>?admin_id='+admin_id;
