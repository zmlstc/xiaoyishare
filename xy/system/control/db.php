...
class dbControl extends SystemControl{
	...
	public function dbWt(){
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			showMessage($lang['db_index_backup_to_wait'],urlAdminSystem('db','db',array('step'=>1)));
		}
		/**
		 * 数据库备份步骤
		 */
		if (intval($_GET['step']) >= 1){
			...
			if (
				...
				$url = array(
					array(
						'url'=>urlAdminSystem('db','db',array('step'=>(intval($_GET['step'])+1))),
	...
	public function db_importWt(){
		...
		if ($_GET['dir_name'] != '' && file_exists(BASE_DATA_PATH.DS.'backup'.DS.$_GET['dir_name'])){
			...
			if ($result == 'succ'){
				$url = array(
					array(
						'url'=>urlAdminSystem('db','db_restore'),
			...
			if ($result == 'continue'){
				$url = array(
					array(
						'url'=>urlAdminSystem('db','db_import',array('step'=>(intval($_GET['step'])+1))).'?dir_name='.$_GET['dir_name'],
