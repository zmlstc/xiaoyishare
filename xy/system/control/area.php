...
class areaControl extends SystemControl{
    ...
    public function get_xmlWt() {
        ...
        foreach ($list as $k => $info) {
            ...
            $operation_detail .= "<li><a href=\"".urlAdminSystem('area','edit',array('area_id'=>$info['area_id']))."\">编辑地区</a></li>";
            if ($info['area_deep'] <= 3) {
                //$operation_detail .= "<li><a href=\"".urlAdminSystem('area','add',array('parent_id'=>$info['area_id']))."\">新增下级</a></li>";
	...
	/* 	else {
        ...
        if ($result) {
            showMessage('操作成功',$_GET['area_id'] ? urlAdminSystem('area'): '');
        } else {
