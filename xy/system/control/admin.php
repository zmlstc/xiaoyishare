...
class adminControl extends SystemControl{
    private $links = array(
        array('url'=>'admin/admin','lang'=>'limit_admin'),
        array('url'=>'admin/gadmin','lang'=>'limit_gadmin'),
    );
        ...
        if (!empty($list) && is_array($list)){
            ...
            foreach ($list as $k => $v){
                ...
                if ($v['admin_is_super'] != 1) {
                    ...
                    $operation .= '<a class="btn blue" href="'.urlAdminSystem('admin','admin_edit',array('admin_id'=>$v['admin_id'])).'"><i class="fa fa-pencil-square-o"></i>'.L('wt_edit').'</a>';
                }else {
                    $operation = '--';
                }
    ...
    public function admin_addWt(){
        if (chksubmit()){
            ...
            if ($rs){
                ...
                showMessage(L('wt_common_save_succ'),urlAdminSystem('admin','admin'));
            }else {
    ...
    public function admin_editWt(){
        if (chksubmit()){
            ...
            if ($result){
                ...
                showMessage(Language::get('admin_edit_success'),urlAdminSystem('admin','admin'));
            }else{
                showMessage(Language::get('admin_edit_fail'),urlAdminSystem('admin','admin'));
            }
        }else{
            //查询用户信息
            $admin_model = Model('admin');
            ...
            if (!is_array($admininfo) || count($admininfo)<=0){
                showMessage(Language::get('admin_edit_admin_error'),urlAdminSystem('admin','admin'));
            }
    ...
    public function gadmin_addWt(){
        if (chksubmit()){
            ...
            if ($model->insert($data)){
                ...
                showMessage(L('wt_common_save_succ'),urlAdminSystem('admin','gadmin'));
            }else {
                showMessage(L('wt_common_save_fail'));
            }
        }
    ...
    public function gadmin_delWt(){
        if (is_numeric($_GET['gid'])){
            ...
            showMessage(Language::get('wt_common_op_succ'),urlAdminSystem('admin','gadmin'));
        }else {
