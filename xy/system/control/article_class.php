        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    $url = array(
                        array(
                            'url'=>urlAdminSystem('article_class','article_class_add',array('ac_parent_id'=>intval($_POST['ac_parent_id']))),
                            'msg'=>$lang['article_class_add_class'],
                        ),
                        array(
                            'url'=>urlAdminSystem('article_class','article_class'),
...
//              $update_array['ac_parent_id'] = intval($_POST['ac_parent_id']);
                ...
                if ($result){
                    $url = array(
                        ...
                        array(
                            'url'=>urlAdminSystem('article_class','article_class_edit',array('ac_id'=>intval($_POST['ac_id']))),
                            'msg'=>$lang['article_class_edit_again'],
                        ),
                    );
                    $this->log(l('wt_edit,article_class_index_class').'['.$_POST['ac_name'].']',1);
                    showMessage($lang['article_class_edit_succ'],urlAdminSystem('article_class','article_class'));
                }else {
    ...
    public function article_class_delWt(){
        ...
        if (intval($_GET['ac_id']) > 0){
            ...
            showMessage($lang['article_class_index_del_succ'],urlAdminSystem('article_class','article_class'));
        }else {
            showMessage($lang['article_class_index_choose'],urlAdminSystem('article_class','article_class'));
        }
    }
    /**
     * ajax操作
