...
class showControl extends SystemControl{
	private $links = array(
        array('url'=>'show/ap_manage','lang'=>'show_index_manage'),
    );
        ...
        if(!chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result&&$result2){
                    ...
                    showMessage($lang['show_add_succ'],urlAdminSystem('show','show',array('ap_id'=>$_POST['ap_id'])));
                }else {
                    showMessage($lang['show_add_fail'],urlAdminSystem('show','show',array('ap_id'=>$_POST['ap_id'])));
                }
        }
      }
    }
    ...
    public function get_ap_xmlWt(){
        ...
        foreach ((array)$ap_list as $k => $ap_info) {
            ...
            $operation_detail = "<li><a href=\"".urlAdminSystem('show','show',array('ap_id'=>$ap_info['ap_id']))."\"></i>管理广告</a></li>";
            $operation_detail .= "<li><a href='".urlAdminSystem('show','ap_edit',array('ap_id'=>$ap_info['ap_id']))."'>编辑属性</a></li>";
        ...
        if($_POST['form_submit'] != 'ok'){
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    showMessage($lang['ap_add_succ'],urlAdminSystem('show','ap_manage'),'html','succ',1,4000);
                }else {
    ...
    public function get_show_xmlWt(){
        ...
        foreach ((array)$show_list as $k => $show_info) {
            $list = array();$operation_detail = '';
            $list['operation'] = "<a class='btn red' onclick=\"fg_delete({$show_info['show_id']})\"><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='".urlAdminSystem('show','show_edit',array('show_id'=>$show_info['show_id']))."'><i class='fa fa-pencil-square-o'></i>编辑</a>";
