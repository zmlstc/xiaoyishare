...
class wtControl extends SystemControl{
	 private $links = array(
	    array('url'=>'setting/lc','lang'=>'wt_set'),
		//array('url'=>'wt/sms','lang'=>'sms_set'),
		//临时注释
		//array('url'=>'wt/rc','lang'=>'rc_set'),
		//array('url'=>'wt/webchat','lang'=>'webchat_set'),
...
/*            $update_array = array();
    ...
    public function rc_addWt() {
        ...
        if (chksubmit()) {
            if (count($rc_list) >= 8) {
                showMessage('最多可设置8个楼层',urlAdminSystem('wt','rc'));
            }
            ...
            if ($result){
                showMessage('保存成功',urlAdminSystem('wt','rc'));
            }else {
                showMessage('保存失败');
            }
        }
    ...
    public function rc_editWt() {
        ...
        if (!chksubmit()) {
            ...
            $result = $model_setting->updateSetting(array('wt_rc'=>serialize($rc_list)));
            if ($result){
                showMessage('编辑成功',urlAdminSystem('wt','rc'));
            }
            showMessage('编辑失败');
        }
