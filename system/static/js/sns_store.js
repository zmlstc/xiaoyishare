$(function(){
		...
		$.get('index.php/index/login', function(result){
		    if(result=='0'){
				...
				if($('#tracereply_'+data.txtid).css("display")=='none'){
					//加载评论列表
			        $("#tracereply_"+data.txtid).load('index.php/store_snshome/commenttop?id='+data.txtid);
	...
	$("[wt_type='scommentbtn']").live('click',function(){
		...
		if($("#commentform_"+data.txtid).valid()){			
			...
			if(cookienum >= MAX_RECORDNUM && $("#commentseccode"+data.txtid).css('display')=="none"){
				...
				$("#commentseccode"+data.txtid).find("[name='codeimage']").attr('src','index.php/vercode?type=30x92&c=' + Math.random());
	...
	$("[wt_type='scomment_del']").live('click',function(){
		...
		showDialog('您确定要删除该信息吗？','confirm', '', function(){
	        ...
	        ajax_get_confirm('','index.php/store_snshome/delcomment?scid='+data_str.scid+'&stid='+data_str.stid);
			return false;
		});
	});
		...
		if(form.valid()){
			...
			if(!isNaN(cookienum) && cookienum >= MAX_RECORDNUM){
			    if (vercode.css('display') == 'none') {
	                ...
	                vercode.find("[name='codeimage']").attr('src','index.php/vercode?type=30x92&c=' + Math.random());
	...
	$("[wt_type='sd_del']").live('click',function(){
        ...
        eval("data_str = "+data_str);
        var url = "index.php/store_snshome/deltrace?id="+data_str.txtid;
		showDialog('您确定要删除该信息吗？','confirm', '', function(){
			ajaxget(url);
			return false;
		});
