...
function gcategoryChange()
{
    ...
    for (i = 0; i < selects.length; i++)
    {
        ...
        if (sel.value > 0)
        {
			...
			//console.log( sel.options[sel.selectedIndex].dataset.explain);
			crate = sel.options[sel.selectedIndex].dataset.explain;
            names.push(name);
        }
    }
	...
	$(this).parent().find("#showrate").html('佣金比例:'+crate+'%');

    // ajax请求下级分类
    if (this.value > 0)
    {
        var _self = this;
        var url = SITEURL + '/wt/index.php/storeclass/josn_sc?callback=?';
        $.getJSON(url, {'sc_id':this.value}, function(data){
            if (data)
            {
                if (data.length > 0)
                {
                    $("<select class='class-select' name='scid2'><option value=''>-请选择-</option></select>").change(gcategoryChange).insertAfter(_self);
                    var data  = data;
                    for (i = 0; i < data.length; i++)
                    {
                        $(_self).next("select").append("<option data-explain='" + data[i].commis_rate + "' value='" + data[i].sc_id + "'>" + data[i].sc_name + "</option>");
                    }
                }
                else
                {
	...
	if(gc_json){
		for(var i in gc_json){
			if(gc_json[i].depth == 1){
				html += ('<option value="'+gc_json[i].sc_id+'">'+gc_json[i].sc_name+'</option>');
			}
		}
	}
	html += '</select>';
