...
function collect_store(fav_id,jstype,jsobj){
    $.get('index.php/index/login', function(result){
        if(result=='0'){
            login_dialog();
        }else{
            var url = 'index.php/member_favorite_store/favoritestore';
...
function collect_goods(fav_id,jstype,jsobj){
    $.get('index.php/index/login', function(result){
        if(result=='0'){
            ...
            var url = 'index.php/member_favorite_goods/favoritegoods';
            $.getJSON(url, {'fid':fav_id}, function(data){
                if (data.done)
            {
                showDialog(data.msg, 'succ','','','','','','','','',2);
...
function load_cart_information(){
	$.getJSON(SITEURL+'/index.php/cart/ajax_load?callback=?', function(result){
...
function drop_topcart_item(cart_id){
    $.getJSON(SITEURL+'/index.php/cart/del?cart_id='+cart_id+'&callback=?', function(result){
        if(result.state){
            var obj = $('.head-my-menu .my-cart');
            //删除成功
            if(result.quantity == 0){
    	        ...
    	        if ($('#rtoolbar_cartlist > ul').children().size() != result.quantity) {
    	        	$("#rtoolbar_cartlist").load(BASE_SITE_URL+ '/index.php/cart/ajax_load?type=html');return ;
    	        }
            }
        }else{
            alert(result.msg);
...
function load_history_information(){
    $.getJSON(SITEURL+'/index.php/index/viewed_info', function(result){
...
function weixin_login(){
    ...
    $.getScript("https://res.wx.qq.com/connect/zh_CN/htmledition/js/wxLogin.js", function(){
        var obj = new WxLogin({
            ...
            scope: "snsapi_login", 
            redirect_uri: MEMBER_SITE_URL+"/index.php/connect_wx/get_info",
            state: "",
            style: "",
            href: ""
        });
...
(function($) {
    $.show_wt_login = function(options) {
        var settings = $.extend({}, {action:'/index.php/login/login?inajax=1', wthash:'', formhash:'' ,anchor:''}, options);
        var login_dialog_html = $('<div class="quick-login"></div>');
        var ref_url = document.location.href;
        var add_html = '<span class="other">';
        if (connect_qq == '1'){
            add_html += '<a href="'+MEMBER_SITE_URL+'/index.php/connect_qq" onclick="ref_url_login()" title="QQ账号登录" class="qq"><i></i></a>';
        }
        if (connect_sn == '1'){
            add_html += '<a href="'+MEMBER_SITE_URL+'/index.php/connect_sina" onclick="ref_url_login()" title="新浪微博账号登录" class="sina"><i></i></a>';
        }
        if (connect_wx == '1'){
        ...
        login_dialog_html.append('<form class="bg" method="post" id="ajax_login" action="'+LOGIN_SITE_URL+settings.action+'"></form>').find('form')
        	...
        	.append('<dl><dt>验证码</dt><dd><input type="text" size="10" maxlength="4" class="text fl w60" name="captcha"><img border="0" onclick="this.src=\'index.php/vercode?c=\' + Math.random()" name="codeimage" title="看不清，换一张" src="index.php/vercode" class="fl ml10"><span>不区分大小写</span></dd></dl>')
        	.append('<ul><li>› 如果您没有登录账号，请先<a class="register" href="'+LOGIN_SITE_URL+'/index.php/login/register">注册会员</a>然后登录</li><li>› 如果您<a class="forget" href="'+LOGIN_SITE_URL+'/index.php/login/forget_password">忘记密码</a>？，申请找回密码</li></ul>')
...
(function($){
	$.fn.membershipCard = function(options){
		...
		return this.each(function(){
			...
			var _uri = SITEURL+'/index.php/member_card?callback=?&uid='+data_str.id+'&from='+options.type;
			var _dl = '';
			$this.qtip({
	            content: {
	                text: 'Loading...',
	                ajax: {
		                ...
		                success: function(data) {
		                	if(data){
	                			...
	                			.append('<a href="'+BASE_SITE_URL+'/index.php/member_snshome?mid='+data.id+'" target="_blank">'+data.name+'</a>'+(data.truename != ''?'('+data.truename+')':''));
		                		//show membergrade
		                		...
		                		// avatar
		                		$('<dd class="avatar"><a href="'+BASE_SITE_URL+'/index.php/member_snshome?mid='+data.id+'" target="_blank"><img src="'+data.avatar+'" /></a><dd>')
		                			.appendTo(_dl);
		                		// info
		                		var _info = '';
		                		...
		                		// ajax info
		                		if(data.url != ''){
			                		$.getJSON(data.url+'/index.php/member_card/mcard_info?uid='+data.id, function(d){
			                			if(d){
		                		...
		                		if(data.follow != 2){
		                			...
		                			$('<div class="send-msg"> <a href="'+MEMBER_SITE_URL+'/index.php/member_message/sendmsg?member_id='+data.id+'" target="_blank"><i></i>站内信</a> </div>').appendTo(_bottom);
		                		}
		...
		function follow(o){
			...
			$.getJSON(MEMBER_SITE_URL+'/index.php/member_snsfriend/addfollow?callback=?&mid='+data_str.mid, function(data){
				if(data){
		...
		function onfollow(o){
			var data_str = o.parent().attr('data-param');
			eval( "data_str = "+data_str);
			$.getJSON(MEMBER_SITE_URL+'/index.php/member_snsfriend/delfollow?callback=?&mid='+data_str.mid, function(data){
				if(data){
		...
		function bbs_function(d){
			...
			$.each(d,function(i, n){
				rs.append('<div class="rank-div" title="'+n.bbs_name+'圈等级'+n.cm_level+'，经验值'+n.cm_exp+'"><a href="'+BBS_SITE_URL+'/index.php/group?c_id='+n.bbs_id+'" target="_blank">'+n.bbs_name+'</a><i></i><em class="rank-em rank-'+n.cm_level+'">'+n.cm_level+'</em></div>');
			});
			return rs;
		}
	...
	$.fn.wt_region = function(options) {
		...
		this.each(function() {
			...
			if ($inputArea.val() && /^\d+$/.test($inputArea.val())) {
				$.getJSON("/api/mobile/index.php/area/json_area_show/area_id/" + $inputArea.val() + "?callback=?", function(data) {
		...
		function loadAreaArray(callback) {
			if (typeof wt_a === "undefined") {
				$.getJSON("/api/mobile/index.php/area/json_area/src/" + settings.src + "?callback=?", function(data) {
					wt_a = data;
					callback()
				})
			} else {
    ...
    $.getJSON(url, {'goods_id':goods_id, 'quantity':quantity}, function(data) {
        if (data != null) {
            if (data.state) {
                ...
                $("#rtoolbar_cartlist").load(BASE_SITE_URL + '/index.php/cart/ajax_load?type=html');
            } else {
                alert(data.msg);
            }
        }
