...
if (isset($_FILES["Filedata"]) || !is_uploaded_file($_FILES["Filedata"]["tmp_name"]) || $_FILES["Filedata"]["error"] != 0) {
	if($config['oss']['open']){
		$path = 'shop/store/goods/'.trim($_POST['storeid']).'/';//取得上传图片的绝对路径
	}else{
	...
	if($config['oss']['open']){
		...
		$oss_path = __DIR__ . '/../../../apid/oss/sdk.class.php';
		if (file_exists($oss_path)){
			require_once($oss_path);
		}
		$result = oss::upload($name,$save);
		if ($result == false) {
			$this->error = '上传失败';
		}
