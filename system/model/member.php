...
class memberModel extends Model
{
    ...
    public function login($login_info)
    {
        ...
        if (!empty($member_info)) {
            if ($member_info['lock_pwd_num'] >= 10 && $member_info['lock_pwd_time'] >= time()) {
                return array('error' => '密码输入错误10次，你的账号被锁定30分钟！');
            }
            //Log::record(json_encode($member_info));
            if ($member_info['member_passwd'] != md5($password)) {
                //$error_num = intval($member_info['lock_pwd_num']+1);
                $where = array();
                $where['member_id'] = $member_info['member_id'];
                $where['addtime'] = array('gt', time() - 30 * 60);
                $error_num = $this->table('member_login_log')->where($where)->count();
                $update = array();
                $update['lock_pwd_num'] = array('exp', "lock_pwd_num + 1");
                if ($error_num == 0) {
                    $update['lock_pwd_num'] = 1;
                }
                $update['lock_pwd_time'] = time() + 60 * 30;
                $updateSuccess = $this->editMember(array('member_id' => $member_info['member_id']), $update);
                $insert = array();
                $insert['member_id'] = $member_info['member_id'];
                $insert['addtime'] = time();
                $insert['is_error'] = 1;
                $rs = $this->table('member_login_log')->insert($insert);
                if ($error_num + 1 >= 7) {
                    return array('error' => '登陆密码不正确！还剩' . (9 - $error_num) . '次数机会');
                } else {
                    return array('error' => '登陆密码不正确！多次输入错误将会锁定登陆!');
                }
            } else {
                $update = array();
                $update['lock_pwd_num'] = 0;
                $update['lock_pwd_time'] = 0;
                $updateSuccess = $this->editMember(array('member_id' => $member_info['member_id']), $update);
            ...
            //刪除登录记录
            $member_login_log = Model('member_login_log');
            $member_login_log->deleteOneLog($member_info['member_id']);
    ...
    public function register($register_info)
    {
        ...
        $member_info['nickname'] = $register_info['nickname'];
        //shopwt 5.2 分销
    ...
    public function addMember($param)
    {
        ...
        try {
            ...
            $member_info['nickname'] = $param['nickname'];
            //添加邀请人(推荐人)会员积分
            ...
            if (C('points_isuse')) {
                ...
                if ($today_num < 2) {
                    ...
                    if (!empty($inviter_name) && $inviter_name != '') {
                        Model('points')->savePointsLog('inviter', array('pl_memberid' => $member_info['inviter_id'], 'pl_membername' => $inviter_name, 'invited' => $insert_id));
                    }
                }
            }
            ...
            $param['member_id'] = $insert_id;
            $param['param'] = array();
            QueueClient::push('sendMemberMsg', $param);

            //发送用户消息(邀请人)
            $param = array();
            $param['code'] = 'inviter_member_reg';
            $param['member_id'] = $member_info['inviter_id'];
            $param['param'] = array();
            QueueClient::push('sendMemberMsg', $param);
