...
/*          $param  = array();
        ...
        $message_list       = Db::select($param,$page);  */
	 	...
	 	if($condition_str!=''){
			$condition_str = substr(trim($condition_str),3,strlen(trim($condition_str)));
			$condition_str = str_replace('message.','',$condition_str);
		}
		$message_list = $this->table('message')->where($condition_str)->page($page)->order('message_id DESC')->select();
