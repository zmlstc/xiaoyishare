...
class pointsModel extends Model {
    ...
    function savePointsLog($stage,$insertarr,$if_repeat = true){
        ...
        switch ($stage){
            ...
            case 'order':
                if (!$insertarr['pl_desc']){
                    $insertarr['pl_desc'] = '订单'.$insertarr['order_sn'].'消费获得积分';
                }
                break;
            case 'system':
                break;
    ...
    public function getPointsLogList($where, $field = '*', $limit = 0, $page = 0, $order = '', $group = ''){
        ...
        if ($list && is_array($list)){
            foreach ($list as $k=>$v){
                ...
                $v['addtimetext'] = @date('Y-m-d H:i:s',$v['pl_addtime']);
                $list[$k] = $v;
            }
        }
        return $list;
	...
	*
	*/
	public function hidePoints($where)
    {
		return $this->table('points_log')->where($where)->update(array('is_del'=>1));
    }	
