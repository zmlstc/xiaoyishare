...
class voucherModel extends Model {
    ...
    public function delVoucher($conditoin) {
        return $this->table('voucher')->where($conditoin)->delete();
    }
    /**
    ...
    public function getCurrentAvailableVoucher($condition = array(), $goods_total = 0,$order = '') {
       ...
       foreach ($voucher_list as $key => $voucher) {
           if ($goods_total < $voucher['voucher_limit']) {
               ...
               $voucher_list[$key]['desc'] = sprintf('面额%s元 有效期至 %s ',floatval($voucher['voucher_price']),date('Y-m-d',$voucher['voucher_end_date']));
               if ($voucher['voucher_limit'] > 0) {
                   $voucher_list[$key]['desc'] .= sprintf(' 消费满%s可用',floatval($voucher['voucher_limit']));
               }
			   $voucher_list[$key]['valid_time'] =date('Y-m-d',$voucher['voucher_end_date']);
			   $voucher_list[$key]['mtitle'] =sprintf('优惠%s元 满%s可用',floatval($voucher['voucher_price']),floatval($voucher['voucher_limit']));
			   $voucher_list[$key]['voucher_price'] = floatval($voucher['voucher_price']);
			   $voucher_list[$key]['voucher_limit'] = floatval($voucher['voucher_limit']);
           }
       }
       return $voucher_list;
    }
	
	public function getCurrentAvailableVoucherInfo($condition = array(), $order = '') {
       $condition['voucher_end_date'] = array('gt',TIMESTAMP);
       $condition['voucher_state'] = 1;
       $voucher = $this->table('voucher')->where($condition)->order($order)->find();
	   if(!empty($voucher)&&is_array($voucher)){
     /*   foreach ($voucher as $key => $voucher) {
           if ($goods_total < $voucher['voucher_limit']) {
               unset($voucher[$key]);
           } else { */
               $voucher['desc'] = sprintf('面额%s元 有效期至 %s ',$voucher['voucher_price'],date('Y-m-d',$voucher['voucher_end_date']));
               if ($voucher['voucher_limit'] > 0) {
                   $voucher['desc'] .= sprintf(' 消费满%s可用',$voucher['voucher_limit']);
               }
			   $voucher['valid_time'] =date('Y-m-d',$voucher['voucher_end_date']);
			   $voucher['mtitle'] =sprintf('优惠%s元 ',$voucher['voucher_price']);
/*            }
       } */
	   }
       return $voucher;
	...
	public function getCurrentAvailableVoucherInfo_index($condition = array(), $order = '') {
       $condition['voucher_end_date'] = array('gt',TIMESTAMP);
       $condition['voucher_state'] = 1;
       $voucher_list = $this->table('voucher')->where($condition)->group('voucher_t_id')->order($order)->select();
	   //echo json_encode($voucher_list).'<br>';
	   if(!empty($voucher_list)&&is_array($voucher_list)){
			foreach ($voucher_list as $key => $voucher) {				
				$voucher_list[$key]['desc'] = sprintf('面额%s元 有效期至 %s ',floatval($voucher['voucher_price']),date('Y-m-d',$voucher['voucher_end_date']));
				if ($voucher['voucher_limit'] > 0) {
				   $voucher_list[$key]['desc'] .= sprintf(' 消费满%s可用',floatval($voucher['voucher_limit']));
					$voucher_list[$key]['mtitle'] =sprintf('满 %s 减%s元 ',floatval($voucher['voucher_limit']),floatval($voucher['voucher_price']));
				}else{
					$voucher_list[$key]['mtitle'] =sprintf('减%s元 ',floatval($voucher['voucher_price']));
				}
				$voucher_list[$key]['valid_time'] =date('Y-m-d',$voucher['voucher_end_date']);
    ...
    public function getMemberVoucherList($member_id, $voucher_state, $page = null, $order = 'voucher_id desc') {
        ...
        $field = 'voucher_id,voucher_code,voucher_title,voucher_desc,voucher_start_date,voucher_end_date,voucher_price,voucher_limit,voucher_state,voucher_order_id,voucher_store_id,store_name,store_id,store_domain,store_label,store_avatar,is_own_shop,voucher_t_customimg,voucher_active_date';
