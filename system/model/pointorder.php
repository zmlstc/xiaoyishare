...
class pointorderModel extends Model{
     ...
     * 兑换订单确认兑换（商家扫码）
     */
    public function dhPointOrder($point_ordersn, $member_id = 0,$store_id){
        //$point_orderid = intval($point_orderid);
        if($point_ordersn ==''||$member_id<=0) {
		...
		$order_info = $this->table('points_order')->where($where)->find();
		if(!empty($order_info)&& is_array($order_info)){
			//更新
			$result = $this->editPointOrder($where,array('point_orderstate'=>50,'point_finnshedtime'=>time()));
			if ($result){
			...
			return array('state'=>false,'msg'=>'兑换信息不存在');
		}
    }
     ...
     * @return bool
     */
    public function getPointOrderGoodsInfo($where) {
        $ordergoods_list = array();
        $ordergoods = $this->table('points_ordergoods')->where($where)->find();
        
        if ($ordergoods){
            
			$ordergoods['point_goodsimage_old'] = $ordergoods['point_goodsimage'];
			$ordergoods['point_goodsimage_small'] = pointprodThumb($ordergoods['point_goodsimage'], 'small');
			$ordergoods['point_goodsimage'] = pointprodThumb($ordergoods['point_goodsimage'], 'mid');
     ...
     * 创建积分礼品兑换订单
     */
    public function createOrder($post_arr, $pointprod_info, $member_info){
        ...
        $order_array['point_allpoint']      = $pointprod_info['pgoods_points'];
        $order_array['point_ordermessage']  = '';//trim($post_arr['pcart_message']);
        $order_array['point_orderstate']    = $pointorderstate_arr['waitship'][0];
        $order_array['store_id']      = $pointprod_info['store_id'];
        $order_array['store_name']      = $pointprod_info['store_name'];
        ...
        $insert_arr['pl_points'] = -$pointprod_info['pgoods_points'];
		...
		$tmp['point_goodsid']       = $pointprod_info['pgoods_id'];
		$tmp['point_goodsname']     = $pointprod_info['pgoods_name'];
		$tmp['point_goodspoints']   = $pointprod_info['pgoods_points'];
		$tmp['price']   = $pointprod_info['pgoods_price'];
		$tmp['point_goodsnum']      =1;
		$tmp['point_goodsimage']    = $pointprod_info['pgoods_image_old'];
        $tmp['store_id']      = $pointprod_info['store_id'];
        $tmp['store_name']      = $pointprod_info['store_name'];

		$tmp['checkcode']      	= createUserPointsCode(time());
		$tmp['endtime']      	= $pointprod_info['pgoods_endtime'];
		...
		$tmp['pgoods_storage'] = array('exp',"pgoods_storage-1");
		$tmp['pgoods_salenum'] = array('exp',"pgoods_salenum+1");
		
		$model_pointprod->editPointProd($tmp,array('pgoods_id'=>$pointprod_info['pgoods_id']));
