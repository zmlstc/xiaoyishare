...
class predepositModel extends Model {
    ...
    public function getPdLogSum($condition = array()) {
    ...
    public function changePd($change_type, $data = array()) {
        ...
        switch ($change_type) {
            ...
            case 'member_invite':
                $data_log['lg_av_amount'] = +$data['amount'];
                $data_log['lg_desc'] = '获得推广会员佣金，订单号: ' . $data['order_sn'];
                $data_log['lg_invite_member_id'] = $data['invite_member_id'];
                $data_log['lg_invite_member_name'] = $data['invite_member_name'];
                $data_log['is_tg'] = 1;
                $data_pd['available_predeposit'] = array('exp', 'available_predeposit+' . $data['amount']);
                $data_pd['tg_predeposit'] = array('exp', 'tg_predeposit+' . $data['amount']);
            ...
            case 'store_invite':
                $data_log['lg_av_amount'] = +$data['amount'];
                $data_log['lg_desc'] = '获得推广店铺佣金，订单号: ' . $data['order_sn'];
                $data_log['lg_invite_store_id'] = $data['invite_store_id'];
                $data_log['lg_invite_store_name'] = $data['invite_store_name'];
                $data_log['is_tg'] = 1;
                $data_log['lg_invite_member_id'] = 0;
                $data_pd['available_predeposit'] = array('exp', 'available_predeposit+' . $data['amount']);
                $data_pd['tg_predeposit'] = array('exp', 'tg_predeposit+' . $data['amount']);
