...
class store_orderModel extends Model {
    ...
    public function getOrderInfo($condition = array(), $extend = array(), $fields = '*', $order = '',$group = '') {
        $order_info = $this->table('store_orders')->field($fields)->where($condition)->group($group)->order($order)->find();
        if (empty($order_info)) {
            return array();
        }
 ...
 */
    ...
    public function getOrderList($condition, $pagesize = '', $field = '*', $order = 'order_id desc', $limit = '', $extend = array(), $master = false){
        $list = $this->table('store_orders')->field($field)->where($condition)->page($pagesize)->order($order)->limit($limit)->master($master)->select();
...
/*         if (in_array('order_goods',$extend)) {
    ...
    public function getOrderCount($condition) {
        return $this->table('store_orders')->where($condition)->count();
    }
    ...
    public function getOrderSum($condition) {
        ...
        return $this->table('store_orders')->where($condition)->sum('order_amount');
    }
	
	public function getOrderCountByMember($condition) {
        //return $this->table('store_orders')->where($condition)->group('buyer_id')->count();
		return $this->table('store_orders')->where($condition)->group('')->count1("distinct CONCAT(buyer_id)");
    }
    ...
    public function addOrder($data) {
        $insert = $this->table('store_orders')->insert($data);
        if ($insert) {
            //更新缓存
            if (C('cache_open')) {
    ...
    public function editOrder($data,$condition,$limit = '') {
        $update = $this->table('store_orders')->where($condition)->limit($limit)->update($data);
        if ($update) {
            //更新缓存
            if (C('cache_open')) {
    ...
    public function getOrderOperateState($operate,$order_info){
        ...
        switch ($operate) {
           ...
           case 'payment':
            ...
            //评价
            case 'evaluation':
                $state = !$order_info['lock_state'] && !$order_info['evaluation_state'] && $order_info['order_state'] == 20;
                break;
            ...
            //放入回收站
            case 'delete':
                $state = $order_info['delete_state'] == 0;
                break;
