...
final class Base{
	...
	private static function routeCheck()
    {
        ...
        if('' != $_SERVER['PATH_INFO']) {
			...
			// 去除正常的URL后缀
			$path = preg_replace('/\.(' . ltrim('html', '.') . ')$/i', '', $pathinfo);
			//echo $path;exit;
            //$paths  =   explode($depr,trim($_SERVER['PATH_INFO'],$depr));
			$paths  =   explode($depr,trim($path,$depr));
			// 控制器
			$controller = !empty($paths) ? array_shift($paths) : 'index';
            
            // 操作
            $action = !empty($paths) ? array_shift($paths) : 'index';
			//echo $controller.'/'.$action.'---'.$depr;exit;
			// 剩余的URL参数
            $var  =  array();
