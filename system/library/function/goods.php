...
function thumb($goods = array(), $type = ''){ 
    ...
    if (array_key_exists('ap_cover', $goods)) {
        $goods['goods_image'] = $goods['ap_cover'];
    }
 ...
 * @return string
 */
function storePayQRCode($store_id) {
    if (!file_exists(BASE_UPLOAD_PATH. '/' . ATTACH_STORE . '/' . $store_id . '/' . $store_id . '_pay.png' )) {
        return UPLOAD_SITE_URL.DS.ATTACH_STORE.DS.'default_qrcode.png';
    }
    return UPLOAD_SITE_URL.DS.ATTACH_STORE.DS.$store_id.DS.$store_id.'_pay.png';
}
...
function feedbackThumb($image_name = '', $type = ''){
	if (!in_array($type, array('240','1024'))) $type = '240';
	if (empty($image_name)){
		return UPLOAD_SITE_URL.'/'.defaultGoodsImage('240');
    }
    ...
    $file_path = ATTACH_MALBUM.DS.'feedback'.DS.str_ireplace('.', '_'.$type.'.', $image_name);
    if(!file_exists(BASE_UPLOAD_PATH.DS.$file_path)) {
		return UPLOAD_SITE_URL.'/'.defaultGoodsImage('240');
	}
	return UPLOAD_SITE_URL.DS.$file_path;
...
function orderPaymentName($payment_code) {
    return str_replace(
            array('offline','online','ali_native','alipay','tenpay','chinabank','predeposit','wxpay','wx_jsapi','wx_saoma','chain','wxpay_h5','wxminipay','paypal','alipay_native','wxpay_jsapi'),
            array('货到付款','在线付款','支付宝移动支付','支付宝','财付通','网银在线','余额支付','微信支付[客户端]','微信支付[WAP]','微信支付[扫码]','门店支付','微信支付[H5]','微信支付','PayPal支付','支付宝移动支付','微信支付[jsapi]'),
            $payment_code);
}
