...
function getNEWSArticleUrl($article_id) {
    if (URL_MODEL) {
        ...
        return urlNews('article', 'article_detail', array('article_id' => $article_id));
    }
}
...
function getNEWSPictureUrl($picture_id) {
    if (URL_MODEL) {
        ...
        return urlNews('picture', 'picture_detail', array('picture_id' => $picture_id));
    }
}
...
function getNEWSSpecialUrl($special_id) {
    return urlNews('special', 'special_detail', array('special_id' => $special_id));
}
...
function getShopSpecialUrl($special_id) {
    return urlShop('special', 'special_detail', array('special_id' => $special_id));
}
...
function wcache($key = null, $data = array(), $prefix, $period = 0) {
    ...
    if (!in_array($prefix, array('show', 'all_categories', 'area', 'channel', 'class_tag', 'contractitem', 'gc_class', 'goods_class_seo', 'index/article', 'nav', 'own_shop_ids', 'seo', 'setting', 'setting_updates'))) {
        return true;
    }
 ...
 *
 * 若指向网站首页，可以传空:
 * url() => 表示w和t均为index，返回当前站点网址
 *
 * url('search,'index','array('cate_id'=>2));
 * 传递数组参数时，若w（或t）值为index,则可以省略
 * 上面示例等同于
...
//    $site_url = empty($site_url) ? BASE_SITE_URL : $site_url;
    ...
    if ($model) {
        $show_url = $site_url . '/' . $w . '/' . $t . $url;
    } else {
        $show_url = $site_url . '/' . $file . '/' . $w . '/' . $t . $url;
    }
 ...
 * @return string
 */
function urlSeller($w = '', $t = '', $args = array()) {
    return url($w, $t, $args, false, SELLER_SITE_URL);
}
...
function urlAdminSystem($w = '', $t = '', $args = array()) {
    return url($w, $t, $args, false, ADMIN_SITE_URL . DS . ADMIN_MODULES_SYSTEM);
}

function urlAgent($w = '', $t = '', $args = array()) {
    return url($w, $t, $args, false, AGENTS_SITE_URL);
}

function urlAgentAgents($w = '', $t = '', $args = array()) {
    return url($w, $t, $args, false, AGENTS_SITE_URL . '/agents');
}
 ...
 * @return string
 */
function urlSince($w = '', $t = '', $args = array()) {
    return url($w, $t, $args, false, DELIVERY_SITE_URL);
}

function urlApiMobile($w = '', $t = '', $args = array()) {
    return url($w, $t, $args, false, MOBILE_SITE_URL);
}
 ...
 */
function substr_cut_Show($user_name) {
 ...
 * @return string
 */
function createUserInviteCode($user_id) {
...
//    return $code;
    $member = Model('member');
    $member_info = $member->getMemberInfoByID($user_id);
    //如果已经生成过邀请码 ，就直接调用
    if ($member_info['invite_code']) {
        $code = $member_info['invite_code'];
        return $code;
    } else {
        $eng_string = '';
        $num_string = '';

        $num_string_length = rand(0, 3);
        $eng_string_length = 6 - $num_string_length;

        for ($i = 0; $i < $eng_string_length; $i++) {
            $eng_string .= chr(rand(65, 90));
        }

        for ($k = 0; $k < $num_string_length; $k++) {
            $eng_string .= rand(0, 9);
        }

        $code = $eng_string . $num_string;
        //校验邀请码唯一性
        $result = $member->getMemberInfo(array('invite_code' => $code));
        if ($result) {
            return createUserInviteCode($user_id);
        } else {
            $member->editMember(array('member_id' => $user_id), array('invite_code' => $code));
            return $code;
        }
    }
}
 ...
 * @return string
 */
function getUserInviteCode($code) {
//    static $source_string = 'E5FCDG3HQA4B1NPIJ2RSTUV67MWX89KLYZ';
...
//    return $num;
    $member = Model('member');
    $member_info = $member->getMemberInfo(array('invite_code' => $code));
    if ($member_info) {
        $num = $member_info['member_id'];
    } else {
        $num = '00000000';
    }
 ...
 * @return string
 */
function createUserPointsCode($num) {
    static $source_string = 'E5FCDG3HQA4B1NPIJ2RSTUV67MWX89KLYZ';
...
//身份证号验证
function checkMIdCard($idcard = '') {

    if (empty($idcard) || $idcard == '' || strlen($idcard) != 18) {
        return false;
    }
