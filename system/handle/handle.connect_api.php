...
class connect_apiHandle {
    ...
    public function getWxOAuth2Url() {
        echo time();
        $weixin_appid = C('h5_weixin_appid');

        $api_url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=' . $weixin_appid .
            '&redirect_uri=' . urlencode(MOBILE_SITE_URL . '/index.php/connect/wxlogin_h5') .
    ...
    public function getWxminiOAuth2Url() {
        ...
        $api_url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=' . $weixin_appid .
            '&redirect_uri=' . urlencode(MOBILE_SITE_URL . '/index.php/connect/get_wxmini') .
            '&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect';
        return $api_url;
    }
        ...
        if ($state == true) {
            ...
            switch ($log_type) {
                case '1':
                    ...
                    $s_data['apicodeid'] = 'SMS_183840516';
                    break;
                case '2':
                    $model_member = Model('member');
                    $member = $model_member->getMemberInfo(array('member_mobile' => $phone));
                    ...
                    $s_data['apicodeid'] = 'SMS_183840518';
                    break;
                case '3':
                    $model_member = Model('member');
                    $member = $model_member->getMemberInfo(array('member_mobile' => $phone));
                    ...
                    $log_array['member_id'] = $member['member_id'];
                    $log_array['member_name'] = $member['member_name'];
                    $s_data['apicodeid'] = 'SMS_183840519';
                    break;
                case '4':
                    $model_member = Model('member');
                    $member = $model_member->getMemberInfo(array('member_mobile' => $phone));
                    ...
                    break;
                case '5':
                    $log_msg .= '您的证码是：' . $captcha . '，如非本人操作，请忽略本短信。';
                    $s_data['apicodeid'] = 'SMS_183840519';
                    break;
                case '6':
                    $log_msg .= '恭喜您！您提交的实名认证已审核通过，现在可以使用平台的推广分享功能。';
                    $s_data['apicodeid'] = 'SMS_184616033';
                    break;
                default:
                    $state = false;
                    $msg = '参数错误';
     ...
     * @return array
     */
    public function smsRegister($phone, $captcha, $password, $client, $username, $password_confirm, $invite_member_id, $nickname) {
        if ($password != $password_confirm) {
            $state_data['state'] = false;
            $state_data['msg'] = '两次输入的登陆密码不一致。';
            return $state_data;
        }
        /* 	if($puserpwd != $ppassword_confirm) {
                    $state_data['state'] = false;
                    $state_data['msg'] = '两次输入的支付密码不一致。';
                    return $state_data;
            } */
        // 注册验证
...
//            )
        ...
        if ($state == true) {
            ...
            /*             $member2 = $model_member->getMemberInfo(array('member_name'=> $username));
                        if(!empty($member2)) {//检查是否已被注册
                            $state_data['state'] = false;
                            $state_data['msg'] = '当前用户名已被注册，请更换其他用户名。';
                            return $state_data;
                        } */
            /*             $member3 = $model_member->getMemberInfo(array('member_email'=> $email));
                        if(!empty($member3)) {//检查是否已被注册
                            $state_data['state'] = false;
                            $state_data['msg'] = '当前电子邮箱已被注册，请更换其他电子邮箱。';
                            return $state_data;
                        } */
            //$num = substr($phone,-4);
            $member_name = $username;// $this->getMemberName('hao', $num);
            ...
            $member['member_paypwd'] = time();
            $member['nickname'] = $nickname;
            //添加奖励积分33hao v5.1
            $member['inviter_id'] = $invite_member_id;
            $result = $model_member->addMember($member);
            if ($result) {
                $model_member->editMember(array('member_id' => $result), array('member_name' => $result));
     ...
     * @return array
     */
    public function quickRegister($phone, $weixin_unionid, $username, $invite_member_id, $nickname) {
...
//            $member['member_email'] = $email;
            $member['member_mobile'] = $phone;
            $member['member_mobile_bind'] = 1;
            $member['weixin_unionid'] = $weixin_unionid;
            $member['member_paypwd'] = time();
            $member['nickname'] = $nickname;
            //添加奖励积分33hao v5.1
            $member['inviter_id'] = $invite_member_id;
     ...
     * @return array
     */
    public function wxMiniRegister($phone, $captcha, $password, $client, $username, $password_confirm, $invite_member_id, $nickname, $unionid) {
        if ($password != $password_confirm) {
            $state_data['state'] = false;
            $state_data['msg'] = '两次输入的登陆密码不一致。';
            return $state_data;
        ...
        if ($state == true) {
            ...
            $member2 = $model_member->getMemberInfo(array('weixin_unionid' => $unionid));
            if (!empty($member2)) {//检查是否已被注册
                $state_data['state'] = false;
                $state_data['msg'] = '当前微信已绑定账号。';
                return $state_data;
            }
    ...
    /***未用到***/
    public function smsRegister_back($phone, $captcha, $password, $client) {
        $state_data = $this->checkSmsCaptcha($phone, $captcha, 1);//再次进行验证码验证
        $state = $state_data['state'];
        if ($state == true) {
            $model_member = Model('member');
    ...
    public function smsPassword($phone, $captcha, $password, $client) {
        ...
        if ($state == true) {
            ...
            if (!empty($member)) {
                ...
                $ret = $model_member->editMember(array('member_id' => $member['member_id']), array('member_passwd' => $new_password));
                /* $member_id = $member['member_id'];
     ...
     * @return array
     */
    public function smsSellerPwd($phone, $captcha, $password, $client) {
        $state_data = $this->checkSmsCaptcha($phone, $captcha, 5);//再次进行验证码验证
        $state = $state_data['state'];
        if ($state == true) {
            $model_store = Model('store');
            $store = $model_store->getStoreInfo(array('shop_phone' => $phone));//检查手机号是否已被注册
            if (!empty($store)) {
                $new_password = md5($password);
                $ret = Model('seller')->editSeller(array('seller_pwd' => $new_password), array('store_id' => $store['store_id']));
     ...
     * @return array
     */
    public function getWxMiniUserInfo($code) {
        $weixin_appid = C('app_weixin_appid');
        $weixin_secret = C('app_weixin_secret');

        $url = 'https://api.weixin.qq.com/sns/jscode2session?appid=' . $weixin_appid . '&secret=' . $weixin_secret .
        ...
        //$user_info = array();
        if (!empty($user_info['session_key'])) {
    ...
    public function getWxUserInfo($code, $api_type = '') {
        ...
        if ($api_type == 'h5') {
            $weixin_appid = C('h5_weixin_appid');
            $weixin_secret = C('h5_weixin_secret');
        }
