...
/*        $order_goods = $model_order->getOrderGoodsList(array('order_id'=>$order_id));
     ...
     * @return Ambigous <PHPUnit_Framework_Constraint_Callback, multitype:unknown >
     */
    public function save($date, $order_info, $store_info,$member_id, $member_name) {
        $model_order = Model('store_order');
        $model_evaluate_store = Model('evaluate_store');
		
		//如果未评分，默认为5分
		$evaluate_score = intval($date['score']);
		if($evaluate_score <= 0 || $evaluate_score > 5) {
			$evaluate_score = 5;
		}
		//默认评语
		$evaluate_comment = $date['comment'];
		if(empty($evaluate_comment)) {
			$evaluate_comment = '不错哦';
		}
	
		$geval_image = '';
	/* 	if (isset($date['evaluate_image']) && is_array($date['evaluate_image'])) {
			foreach ($date['evaluate_image'] as $val) {
				if(!empty($val)) {
					$geval_image .= $val . ',';
				}
			}
		} */
		$geval_image = $date['evaluate_image'];
		...
		$evaluate_store_info['seval_scores'] = $evaluate_score;
		$evaluate_store_info['seval_content'] = $evaluate_comment;
		$evaluate_store_info['seval_isanonymous'] = $date['anony']?1:0;
