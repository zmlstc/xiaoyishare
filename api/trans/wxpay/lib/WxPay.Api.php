...
class WxPayApi
{
	...
	{		
		...
		if($data){
			...
			curl_close($ch);
			Log::record('--error---'.$error.'===='.$error_info);
			return false;
	 ...
	 * 企业账户转账到个人账户
	 */
	public static function transfers($config, $transfer_info, $timeOut = 6)
	{
		...
		$pars = array();
		$pars['mch_appid'] =$config->GetAppId();//商户账号appid
		$pars['mchid']=$config->GetMerchantId();//商户号
		$pars['nonce_str'] = self::getNonceStr();
