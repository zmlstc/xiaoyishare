...
class photosControl extends mobileSellerControl
{
...
//    }
    ...
    public function cateWt()
    {
         ...
         */
        $param = array();
        $param['photos_aclass.store_id'] = $this->store_info['store_id'];
        ...
        foreach ($aclass_info as $key => $value) {
            //echo $value['ac_cover'];
            if ($value['ac_cover'] == '') {
                $aclass_info[$key]['ac_cover'] = 'https://www.fhlego.com/system/upfiles/shop/store/camera.png';
            } else {
                $aclass_info[$key]['ac_cover'] = 'https://www.fhlego.com/system/upfiles/shop/store/goods/' . $value['store_id'] . '/' . $value['ac_cover'];
            }

        }

        output_data(array('aclass_info' => $aclass_info));
    }
    ...
    public function album_addWt()
    {
         ...
         */
        $model_album = Model('photos');
        $class_count = $model_album->countClass($this->store_info['store_id']);
...
//        showDialog(Language::get('album_class_save_lose'));
    ...
    public function album_edit_saveWt()
    {
         ...
         * 验证
         */
        $return = $model_album->checkAlbum(array('photos_aclass.store_id' => $this->store_info['store_id'], 'photos_aclass.ac_id' => intval($_POST['id'])));
        if ($return) {
            /**
             * 更新
             */
            $re = $model_album->updateClass($param, intval($_POST['id']));
            output_data(array('re' => $re));
//            if ($re) {
//
...
//            showDialog(Language::get('album_class_edit_lose'));
    ...
    public function album_delWt()
    {
         ...
         * 验证
         */
        $return = $model_album->checkAlbum(array('photos_aclass.store_id' => $this->store_info['store_id'], 'photos_aclass.ac_id' => intval($_POST['id']), 'is_default' => '0'));
        //if (!$return) {
        //output_data(array('re' => $return));
         ...
         * 删除分类
         */
        $return = $model_album->delClass(intval($_POST['id']));
        // if (!$return) {
        //output_data(array('re' => $return));
//            showDialog(Language::get('album_class_file_del_lose'));
        //}
        ...
        $param['value'] = array('1', $this->store_info['store_id']);
        $class_info = $model_album->getOneClass($param);
        $param = array();
        $param['ac_id'] = $class_info['ac_id'];
        $return = $model_album->updatePic($param, array('ac_id' => intval($_POST['id'])));
...
//        }
        ...
        $param['photos_pic.store_id'] = $this->store_info['store_id'];
        if ($_GET['sort'] != '') {
            switch ($_GET['sort']) {
                case '0':
                    $param['order'] = 'add_time desc';
...
//        Tpl::output('class_list', $class_list);
        ...
        $param = array();
        $param['field'] = array('ac_id', 'store_id');
        $param['value'] = array(intval($_POST['id']), $this->store_info['store_id']);
        $class_info = $model_album->getOneClass($param);
...
//        }
        ...
        $return = $model_album->checkAlbum(array('photos_aclass.store_id' => $this->store_info['store_id'], 'photos_aclass.ac_id' => $pic_info['ac_id']));
    ...
    function album_pic_delWt()
    {
        ...
        $return = $model_album->checkAlbum(array('photos_pic.store_id' => $this->store_info['store_id'], 'in_ap_id' => $id));
        if (!$return) {
            //showDialog(Language::get('album_class_pic_del_lose'));
        }

        //删除图片
        $return = $model_album->delPic($id, $this->store_info['store_id']);
        output_data(array('re' => $return));
//        if ($return) {
...
//        }
     ...
     */
    public
    function replace_image_uploadWt()
    {
        $file = $_GET['id'];
     ...
     */
    public
    function album_pic_watermarkWt()
    {
        if (empty($_POST['id']) && !is_array($_POST['id'])) {
            showDialog(Language::get('album_parameter_error'));
        }
    ...
    public function image_uploadWt()
    {
        ...
        if (!empty ($_GET['category_id'])) {
            ...
            $data['state'] = 'false';
            $data['message'] = $error;
            $data['origin_file_name'] = $_FILES["pic"]["name"];
...
//        if ($width >= 600) {
//            $per = 600 / $width;//计算比例
//            $thumb_width = $width * $per;
//            $thumb_height = $height * $per;
//        }
        $upload = new UploadFile();
...
//        $upload->set('thumb_ext', GOODS_IMAGES_EXT);
        ...
        $insert_array['ap_cover'] = $pic_thumb;
        $insert_array['ap_size'] = intval($_FILES['pic']['size']);
        ...
        $data = array();
        $data['file_id'] = $result;
        $data['file_name'] = $pic_thumb;
        $data['origin_file_name'] = $_FILES['pic']["name"];
        $data['file_path'] = $pic_thumb;
     ...
     */
    private
    function profile_menu($menu_type, $menu_key = '')
    {
        $menu_array = array();
        switch ($menu_type) {
            case 'photos':
     ...
     */
    public
    function ajax_change_imgmessageWt()
    {
     ...
     */
    public
    function ajax_check_class_nameWt()
    {
