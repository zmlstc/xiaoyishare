...
class seller_loginControl extends mobileHomeControl {
    ...
    public function indexWt(){
		...
		if(empty($seller_info)&&strlen($seller_name)==11 && preg_match('/^0?(13|15|17|18|14)[0-9]{9}$/i', $seller_name)) {//根据会员名没找到时查手机号
            $condition = array();
            $condition['shop_phone'] = $seller_name;
            $_info = Model('store')->getStoreInfo($condition);
			if(!$_info) {
				output_error('登录失败');
			}
			$seller_info = $model_seller->getSellerInfo(array('store_id' => $_info['store_id']));
        }
        if(!$seller_info) {
            output_error('登录失败');
        }
       ...
       /*  $model_member = Model('member');
        ...
        if($seller_info['seller_pwd']!==md5($_POST['password'])) {
        ...
        if($token) {
			...
			output_data(array('seller_name' => $seller_info['seller_name'], 'store_name' => $store_info['store_name'], 'stoken' => $token));
        } else {
            output_error('登录失败');
        }
    }
	...
	public function getPhoneWt(){
		$store_id = $this->getStoreIdIsLogin();
		if($store_id>0){
