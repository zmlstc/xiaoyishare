...
class paymentControl extends mobileHomeControl{
    ...
    public function returnWt() {
       ...
       //papal支付
        ...
        if($callback_info) {
			 if ($this->payment_code == 'wxpay_jsapi' || $this->payment_code == 'wxpay_h5') {
...
Log::record('=======');
		//小程序支付
	    if ($this->payment_code == 'wxminipay') { Log::record('===2====');
			...
			Log::record('===notifyWt=5====result==='.json_encode($result));
            if ($result) {
			...
			Log::record('===notifyWt=5====internalSn==='.$internalSn.'----'.json_encode($result));
...
Log::record('===r5555=='.json_encode($result));			
