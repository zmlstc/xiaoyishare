    ...
    public function indexWt(){
        ...
        if(isset($member_info['error'])) {
			...
			if($seller_info) {
				...
				$sellerinfo=array('seller_name' => $seller_info['seller_name'],'store_name' => $store_info['store_name'],'token' => $token);
			} */
            $token = $this->_get_token($member_info['member_id'], $member_info['member_name'], $_POST['client']);
            if($token) {
                output_data(array('username' => $member_info['member_name'],'nickname' => $member_info['nickname'], 'userid' => $member_info['member_id'], 'token' => $token,'sell'=>$sellerinfo));
            } else {
                output_error('登录失败');
            }
        }
