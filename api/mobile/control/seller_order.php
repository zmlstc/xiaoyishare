...
class seller_orderControl extends mobileSellerControl {
    ...
    public function order_listWt() {
        ...
        //$condition['seller_del'] = 0;

        if (!empty($_POST['keyword']) && trim($_POST['keyword']) != '') {
            $keyword = trim($_POST['keyword']);
            $condition['buyer_name|order_sn'] = array('like', '%' . $keyword . '%');
        }
        ...
        if (!empty($_POST['date']) && trim($_POST['date']) != '' && $_POST['stype'] == 1) {
            ...
            $condition['add_time'] = array('between', array($today_start, $today_end));
        }
        ...
        $order_list_array = $model_order->getNormalOrderList($condition, 12, '*', 'order_id desc', '', array('store'));
        ...
        foreach ($order_list_array as $key => $val) {
            ...
            $value['mothday'] = date('m月d日', $val['add_time']);
            $value['hourmin'] = date('H:i:s', $val['add_time']);
            $value['paycode_txt'] = '---';
            if ($value['payment_code'] == 'pdpay') {
                $value['paycode_txt'] = '余额支付';
            } elseif ($value['payment_code'] == 'wxpay' || $value['payment_code'] == 'wxminipay') {
                $value['paycode_txt'] = '微信支付';
            } elseif ($value['payment_code'] == 'alipay') {
                $value['paycode_txt'] = '支付宝支付';
            }
            $value['member_avatar_url'] = getMemberAvatarForID($val['buyer_id']);
            $value['order_sn'] = $val['order_sn'];
            $memberInfo = Model('member')->getMemberInfoByID($val['buyer_id']);
            $value['buyer_name'] = $memberInfo['nickname'];//$val['buyer_name'];
            $value['order_amount'] = $val['order_amount'];
            $value['pay_comment'] = $val['pay_comment'];
            $value['tx_error'] = $val['tx_error'];
            $value['payment_name'] = $val['payment_name'];
            $value['seller_money'] = sprintf("%.2f", $val['seller_money']);
            $value['tx_api_state'] = $val['tx_api_state'];
            $value['tx_trade_time'] = $val['tx_trade_time'];
            $value['yj_money'] = sprintf("%.2f", ($val['order_amount'] - $val['seller_money']));

            $new_order_group_list[] = $value;
        }


        output_data(array('list' => $new_order_group_list), mobile_page($page_count));


    }

    //发起重新提现
    public function order_retxWt() {
        ...
        if ($order_info) {
            ...
            //判断是微信提现还是支付宝提现
            if ($order_info['payment_code'] == 'wxminipay' || $order_info['payment_code'] == 'wx_jsapi') {
                //微信提现的方式
                $txinfo = Model('txway')->getInfo(array('store_id' => $order_info['store_id'], 'bill_type_code' => 'wxpay', 'type' => 2));

                //定义提现数据
                $t_data = array();
                $t_data['order_sn'] = $order_sn;
                $t_data['pay_account'] = $txinfo['bill_openid'];
                $t_data['amount'] = $order_info['seller_money'];
                $t_data['real_name'] = $txinfo['bill_user_name'];
                $t_data['remark'] = '商家交易单号:' . $order_sn;
                ...
                if (!empty($trans_data) && $trans_data['code'] == 10000) {
                    $updata = Model('store_order')->editOrder(array('tx_api_state' => 1, 'tx_trade_no' => $trans_data['pay_order_id'], 'tx_trade_time' => $trans_data['pay_date']), array('order_sn' => $order_sn));
                    output_data(array('res' => '提现成功！'));
                } else {
                    //$insert2 = $this->table('store_pd_api')->insert($data_api);
                    Model('store_order')->editOrder(array('tx_error' => $trans_data['sub_msg']), array('order_sn' => $order_sn));
                    output_error($trans_data['sub_msg']);
                }

            } else {
                //阿里提现的方式
                $txinfo = Model('txway')->getInfo(array('store_id' => $order_info['store_id'], 'bill_type_code' => 'alipay', 'type' => 2));
                //定义提现数据
                $t_data = array();
                $t_data['order_sn'] = $order_sn;
