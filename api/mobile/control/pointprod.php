...
class pointprodControl extends mobileHomeControl {
    ...
    public function plistWt() {
        ...
        if (intval($_POST['isable']) == 1) {
            if ($memberid = $this->getMemberIdIfExists()) {
                ...
                $this->member_info = $member_infotmp;//array_merge($member_infotmp,$membergrade_info);
        ...
        //查询仅我能兑换和所需积分
        $points_filter = array();
        if (intval($_POST['isable']) == 1 && !empty($this->member_info)) {
        ...
        $filed = 'store_id,store_name,pgoods_id,pgoods_name,pgoods_price,pgoods_points,pgoods_image,pgoods_tag,pgoods_serial,pgoods_storage,pgoods_commend,pgoods_keywords,pgoods_salenum,pgoods_view,pgoods_limitnum';
        $pageSize = 10;
        $pointprod_list = $model_pointprod->getPointProdList($where, $filed, $orderby, '', $pageSize);
        $page_count = $model_pointprod->gettotalpage();

        output_data(array('list' => $pointprod_list), mobile_page($page_count));
    ...
    public function pinfoWt() {
        ...
        $prodinfo['pgoods_starttime'] = date('yy年m月d日', $prodinfo['pgoods_starttime']);
        $prodinfo['pgoods_endtime'] = date('yy年m月d日', $prodinfo['pgoods_endtime']);
