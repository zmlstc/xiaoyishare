$(function() {
    ...
    $(".submit").click(function(){
        if($.sValid()){
            ...
            $.ajax({
                url: ApiUrl + "/index.php/seller_apply/do_apply",
                type: 'POST',
                dataType: 'json',
...
$(function() {
            ...
            if (data.join_state.step == 5) {
                ...
                jQuery.sValid.init({
                    ...
                    callback:function (eId,eMsg,eRules){
                        if(eId.length >0){
                            var errorHtml = "";
                            jQuery.map(eMsg,function (idx,item){
                                errorHtml += "<p>"+idx+"</p>";
                            });
                            errorTipsShow(errorHtml);
                        }else{
    ...
    $('input[name="paying_money_certificate"]').fileupload({
        dataType: 'json',
        url: ApiUrl + '/index.php/seller_apply/ajax_upload_image',
    ...
    $('#btn_select_category').on('click', function() {
        $(".gc-wrap-pre").remove();
        $.ajax({
            url: ApiUrl + "/index.php/seller_apply/get_goods_category",
            type: 'POST',
            dataType: 'json',
            data: {client: Client},
            success: function(result){
